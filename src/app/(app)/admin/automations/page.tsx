...
export default function AutomationsPage() {
  ...
  if (catalog.isLoading || templates.isLoading || overview.isLoading || rules.isLoading || executions.isLoading) return <AsyncState state="loading" title={uiText("Preparando el estudio de automatización")} />;
  if (catalog.isError || templates.isError || overview.isError || rules.isError || executions.isError || !catalog.data || !templates.data || !overview.data || !rules.data || !executions.data) {
    return <AsyncState state="error" title={uiText("No fue posible cargar las automatizaciones")} onRetry={() => { void catalog.refetch(); void templates.refetch(); void overview.refetch(); void rules.refetch(); void executions.refetch(); }} />;
  }

  return <div className="space-y-8">
    <PageHeader eyebrow={uiText("Administración")} title={uiText("Automatización no-code")} description={uiText("Diseña reglas y opera grandes volúmenes con procesamiento durable, lotes y trazabilidad.")} actions={<div className="flex gap-2"><Button variant="secondary" onClick={() => void refresh()}><RefreshCw className="size-4" />{uiText("Actualizar")}</Button><Button onClick={() => setEditing("new")}><Plus className="size-4" />{uiText("Nueva automatización")}</Button></div>} />
    <div className="grid gap-4 md:grid-cols-4">
      <MetricCard label={uiText("Reglas activas")} value={String(overview.data.rules.active)} detail={`${overview.data.rules.total} configuradas`} period="En tiempo real" />
      <MetricCard label={uiText("Volumen 24 h")} value={String(overview.data.executions.total)} detail={`${overview.data.executions.inProgress} en curso`} period="Últimas 24 horas" />
      <MetricCard label={uiText("Tasa de éxito")} value={`${overview.data.executions.successRate}%`} detail={`${overview.data.executions.completed} completadas`} period="Últimas 24 horas" />
      <MetricCard label={uiText("Incidencias")} value={String(overview.data.executions.failed)} detail="Fallidas o parciales" period="Últimas 24 horas" />
    </div>
    <Card><CardContent className="grid gap-5 pt-6 lg:grid-cols-[1.2fr_1fr]"><div><div className="flex items-center gap-2"><Bot className="size-5 text-brand" /><h2 className="font-semibold">{uiText("Capacidad operativa")}</h2></div><p className="mt-2 text-sm text-muted-foreground">{uiText("Procesamiento mediante outbox durable, workers y recuperación automática de eventos bloqueados.")}</p><div className="mt-4 flex flex-wrap gap-2"><Badge variant="secondary">{overview.data.capacity.outboxMaxAttempts} {uiText(" intentos por evento")}</Badge><Badge variant="secondary">{overview.data.capacity.ruleBatchLimit} {uiText(" reglas por lote")}</Badge><Badge variant="secondary">{overview.data.capacity.retryBatchLimit} {uiText(" reintentos por lote")}</Badge>{overview.data.executions.oldestPendingAt ? <Badge variant="warning">{uiText("Pendiente desde ")}{formatDate(overview.data.executions.oldestPendingAt)}</Badge> : <Badge variant="success">{uiText("Sin acumulación pendiente")}</Badge>}</div></div><div><p className="text-sm font-semibold">{uiText("Reglas con mayor volumen, últimas 24 h")}</p><div className="mt-3 space-y-2">{overview.data.topRules.map((item) => <div key={item.id} className="flex items-center justify-between rounded-lg bg-muted/45 px-3 py-2 text-sm"><span className="truncate">{item.name}</span><strong>{item.executions}</strong></div>)}{!overview.data.topRules.length ? <p className="text-sm text-muted-foreground">{uiText("Sin ejecuciones en el periodo.")}</p> : null}</div></div></CardContent></Card>
    <section className="space-y-3"><div><h2 className="font-semibold">{uiText("Plantillas listas para usar")}</h2><p className="text-sm text-muted-foreground">{uiText("Crea un borrador desde escenarios habituales, revísalo y simúlalo antes de activarlo. Nunca se ejecuta una plantilla al crearla.")}</p></div><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">{templates.data.map((template) => <Card key={template.key}><CardContent className="space-y-3 pt-5"><div><p className="font-semibold">{template.name}</p><p className="mt-1 text-sm text-muted-foreground">{template.description}</p></div><div className="rounded-xl bg-muted/50 p-3 text-xs text-muted-foreground"><p><strong className="text-foreground">{uiText("Disparador:")}</strong> {catalog.data.triggers.find((item) => item.value === template.triggerEvent)?.label ?? template.triggerEvent}</p><p className="mt-1"><strong className="text-foreground">{uiText("Impacto previsto:")}</strong> {template.consequences.length} {uiText(" acciones configurables.")}</p></div><Button className="w-full" variant="secondary" disabled={createTemplate.isPending} onClick={() => createTemplate.mutate(template.key)}><Plus className="size-4" />{uiText("Usar como borrador")}</Button></CardContent></Card>)}</div></section>
    <Tabs defaultValue="rules">
      <TabsList><TabsTrigger value="rules">{uiText("Reglas")}</TabsTrigger><TabsTrigger value="executions">{uiText("Ejecuciones")}</TabsTrigger></TabsList>
      <TabsContent value="rules" className="pt-5">
        <div className="mb-4 flex flex-col gap-3 rounded-2xl border bg-card/80 p-4 lg:flex-row lg:items-center lg:justify-between"><div className="relative max-w-xl flex-1"><Search className="absolute left-3 top-2.5 size-4 text-muted-foreground" /><Input value={ruleSearch} onChange={(event) => { setRuleSearch(event.target.value); setRulePage(1); }} className="pl-9" placeholder={uiText("Buscar reglas por nombre")} /></div><div className="flex flex-wrap gap-2"><Button size="sm" variant="secondary" disabled={!selectedRules.length || bulkRules.isPending} onClick={() => bulkRules.mutate({ action: "ENABLE" })}><Play className="size-4" />{uiText("Activar (")}{selectedRules.length})</Button><Button size="sm" variant="secondary" disabled={!selectedRules.length || bulkRules.isPending} onClick={() => bulkRules.mutate({ action: "DISABLE" })}><Pause className="size-4" />{uiText("Desactivar")}</Button><Button size="sm" variant="ghost" disabled={!selectedRules.length || bulkRules.isPending} onClick={() => void confirmAction({ title: selectedRules.length === 1 ? "¿Eliminar la regla seleccionada?" : `¿Eliminar las ${selectedRules.length} reglas seleccionadas?`, description: "Dejan de existir y de poder ejecutarse.", consequence: "El historial de ejecuciones que ya generaron se conserva, pero las reglas no se pueden recuperar.", confirmLabel: "Eliminar", irreversible: true }).then((ok) => ok && bulkRules.mutate({ action: "DELETE" }))}><Trash2 className="size-4 text-destructive" />{uiText("Eliminar")}</Button></div></div>
        <div className="grid gap-4 xl:grid-cols-2">{rules.data.data.map((rule) => {
          ...
          return <Card key={rule.id} className="overflow-hidden"><CardHeader className="border-b bg-card/80"><div className="flex items-start justify-between gap-3"><div className="flex items-start gap-3"><input type="checkbox" disabled={!selectedRules.includes(rule.id) && selectedRules.length >= overview.data.capacity.ruleBatchLimit} className="mt-1 size-4 accent-primary" checked={selectedRules.includes(rule.id)} onChange={() => setSelectedRules((items) => items.includes(rule.id) ? items.filter((id) => id !== rule.id) : [...items, rule.id])} aria-label={`Seleccionar ${rule.name}`} /><div><div className="flex flex-wrap items-center gap-2"><CardTitle>{rule.name}</CardTitle><Badge variant={rule.enabled ? "success" : "secondary"}>{rule.enabled ? uiText("Activa") : uiText("Borrador")}</Badge></div><p className="mt-1 text-sm text-muted-foreground">{uiText("Versión ")}{rule.version} · {rule.scope === "TENANT" ? uiText("Toda la empresa") : uiText("Sucursal")}</p></div></div><Zap className="size-5 text-brand" /></div></CardHeader><CardContent className="space-y-4 pt-5"><div className="rounded-xl bg-muted/45 p-4"><p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">{uiText("Cuando")}</p><p className="mt-1 font-medium">{trigger?.label ?? rule.triggerEvent}</p><p className="text-sm text-muted-foreground">{trigger?.description}</p></div><div className="grid gap-3 sm:grid-cols-2"><div><p className="text-xs text-muted-foreground">{uiText("Condiciones")}</p><p className="font-semibold">{rule.conditions?.length ?? 0}</p></div><div><p className="text-xs text-muted-foreground">{uiText("Acciones en secuencia")}</p><p className="font-semibold">{rule.consequences.length}</p></div></div><div className="flex flex-wrap gap-2"><Button size="sm" onClick={() => setEditing(rule)}>{uiText("Editar")}</Button><Button size="sm" variant="secondary" onClick={() => { if (rule.enabled) { toggle.mutate(rule); return; } void confirmAction({ title: `¿Activar «${rule.name}»?`, description: "A partir de ahora la regla se ejecutará sola cada vez que ocurra el evento que la dispara.", consequence: "Actuará sobre datos de personas reales sin volver a preguntar. Simúlala antes si no estás seguro de qué hace.", confirmLabel: "Activar la regla" }).then((ok) => ok && toggle.mutate(rule)); }}>{rule.enabled ? <Pause className="size-4" /> : <Play className="size-4" />}{rule.enabled ? uiText("Desactivar") : uiText("Activar")}</Button><Button size="sm" variant="ghost" onClick={() => duplicate.mutate(rule.id)}><Copy className="size-4" />{uiText("Duplicar")}</Button><Button size="icon" variant="ghost" aria-label={`Eliminar ${rule.name}`} onClick={() => void confirmAction({ title: `¿Eliminar «${rule.name}»?`, description: "La regla deja de existir y de ejecutarse.", consequence: "Su historial de ejecuciones se conserva, pero la regla no se puede recuperar.", confirmLabel: "Eliminar la regla", irreversible: true }).then((ok) => ok && remove.mutate(rule.id))}><Trash2 className="size-4 text-destructive" /></Button></div></CardContent></Card>;
        })}</div>
        <Pagination page={rules.data.meta.page} totalPages={rules.data.meta.totalPages} onPage={setRulePage} />
        {!rules.data.data.length ? <Card><CardContent className="flex flex-col items-center py-16 text-center"><Bot className="mb-4 size-10 text-brand" /><h2 className="text-xl font-semibold">{uiText("Crea tu primera automatización")}</h2><p className="mt-2 max-w-lg text-sm text-muted-foreground">{uiText("Conecta eventos reales del sistema con acciones auditables. La regla inicia como borrador para que puedas simularla.")}</p><Button className="mt-5" onClick={() => setEditing("new")}><Plus className="size-4" />{uiText("Crear regla")}</Button></CardContent></Card> : null}
      </TabsContent>
      <TabsContent value="executions" className="space-y-4 pt-5"><div className="flex flex-col gap-3 rounded-2xl border bg-card/80 p-4 lg:flex-row"><div className="relative flex-1"><Search className="absolute left-3 top-2.5 size-4 text-muted-foreground" /><Input value={executionSearch} onChange={(event) => { setExecutionSearch(event.target.value); setExecutionPage(1); }} className="pl-9" placeholder={uiText("Buscar regla, empleado o candidato")} /></div><FormSelect className="min-w-48" value={executionStatus} onValueChange={(value) => { setExecutionStatus(value); setExecutionPage(1); }} options={[{ value: "ALL", label: uiText("Todos los estados") }, { value: "COMPLETED", label: uiText("Completadas") }, { value: "FAILED", label: uiText("Fallidas") }, { value: "PARTIAL", label: "Parciales" }, { value: "IN_PROGRESS", label: uiText("En curso") }, { value: "PENDING", label: uiText("Pendientes") }]} /><Button variant="secondary" disabled={!selectedExecutions.length || retrySelected.isPending} onClick={() => retrySelected.mutate()}><RefreshCw className="size-4" />{uiText("Reintentar (")}{selectedExecutions.length})</Button></div><Card><CardContent className="p-0"><div className="divide-y">{executions.data.data.map((item) => { const retryable = item.status === "FAILED" || item.status === "PARTIAL"; const atLimit = selectedExecutions.length >= overview.data.capacity.retryBatchLimit && !selectedExecutions.includes(item.id); return <div key={item.id} className="flex items-center gap-3 p-5"><input type="checkbox" disabled={!retryable || atLimit} className="size-4 accent-primary" checked={selectedExecutions.includes(item.id)} onChange={() => setSelectedExecutions((items) => items.includes(item.id) ? items.filter((id) => id !== item.id) : [...items, item.id])} aria-label={`Seleccionar ejecución de ${item.rule.name}`} /><button type="button" onClick={() => setExecution(item)} className="flex min-w-0 flex-1 items-center justify-between gap-4 text-left"><div className="min-w-0"><p className="truncate font-semibold">{item.rule.name}</p><p className="text-sm text-muted-foreground">{item.steps.length} {uiText(" acciones · ")}{formatDate(item.startedAt)}</p></div><Badge variant={executionTone(item.status)}>{executionStatusLabel(item.status)}</Badge></button>{retryable ? <Button size="icon" variant="ghost" aria-label={uiText("Reintentar ejecución")} disabled={retryOne.isPending} onClick={() => retryOne.mutate(item.id)}><RefreshCw className="size-4" /></Button> : null}</div>; })}</div>{!executions.data.data.length ? <p className="p-12 text-center text-sm text-muted-foreground">{uiText("No hay ejecuciones para estos filtros.")}</p> : null}</CardContent></Card><Pagination page={executions.data.meta.page} totalPages={executions.data.meta.totalPages} onPage={setExecutionPage} /></TabsContent>
    </Tabs>
...
function RuleEditor({ open, rule, catalog, onClose, onSaved, onSimulation }: { open: boolean; rule: NoCodeAutomationRuleDto | null; catalog: NoCodeAutomationCatalogDto; onClose: () => void; onSaved: () => Promise<void>; onSimulation: (result: NoCodeAutomationSimulationDto) => void }) {
  ...
  return <Dialog open={open} onOpenChange={(next) => !next && onClose()}><DialogContent className="max-h-[94vh] max-w-4xl overflow-y-auto"><DialogHeader><DialogTitle>{rule ? "Editar automatización" : uiText("Nueva automatización")}</DialogTitle><DialogDescription>{uiText("Construye la secuencia de arriba hacia abajo. Las reglas nuevas se guardan desactivadas.")}</DialogDescription></DialogHeader><div className="space-y-6">
    <section className="min-w-0 grid gap-4 rounded-2xl border p-5 md:grid-cols-2"><label className="space-y-2 text-sm font-medium md:col-span-2">{uiText("Nombre")}<Input value={draft.name} maxLength={160} onChange={(event) => setDraft({ ...draft, name: event.target.value })} placeholder={uiText("Ej. Preparar incorporación al contratar")} /></label><label className="space-y-2 text-sm font-medium">{uiText("Cuando ocurra")}<FormSelect value={draft.triggerEvent} onValueChange={(value) => setDraft({ ...draft, triggerEvent: value as typeof draft.triggerEvent, conditions: [] })} options={catalog.triggers.map((item) => ({ value: item.value, label: item.label }))} /></label><label className="space-y-2 text-sm font-medium">{uiText("Alcance")}<FormSelect value={draft.scope} onValueChange={(value) => setDraft({ ...draft, scope: value as typeof draft.scope, branchId: value === "TENANT" ? undefined : draft.branchId })} options={catalog.scopes} /></label>{draft.scope === "BRANCH" ? <label className="space-y-2 text-sm font-medium md:col-span-2">{uiText("Sucursal")}<FormSelect value={draft.branchId} placeholder={uiText("Selecciona una sucursal")} onValueChange={(branchId) => setDraft({ ...draft, branchId })} options={(branches.data ?? []).map((item) => ({ value: item.id, label: item.name }))} /></label> : null}<p className="text-sm text-muted-foreground md:col-span-2">{activeTrigger.description}</p></section>
    <AutomationFlowPreview draft={draft} catalog={catalog} triggerLabel={activeTrigger.label} />
    <section className="space-y-3"><div className="flex items-center justify-between"><div><h3 className="font-semibold">{uiText("Si se cumplen estas condiciones")}</h3><p className="text-sm text-muted-foreground">{uiText("Sin condiciones, la regla se ejecuta siempre.")}</p></div><Button size="sm" variant="secondary" onClick={() => setDraft({ ...draft, conditions: [...draft.conditions, { field: activeTrigger.fields[0]?.value ?? "branchId", operator: "equals", value: "" }] })}><Plus className="size-4" />{uiText("Condición")}</Button></div>{draft.conditions.map((condition, index) => <div key={`${condition.field}-${index}`} className="grid gap-2 rounded-xl border p-3 md:grid-cols-[1fr_1fr_1fr_auto]"><FormSelect value={condition.field} onValueChange={(field) => updateCondition(index, { ...condition, field })} options={activeTrigger.fields.map((item) => ({ value: item.value, label: item.label }))} /><FormSelect value={condition.operator} onValueChange={(operator) => updateCondition(index, { ...condition, operator: operator as NoCodeAutomationCondition["operator"] })} options={catalog.conditionOperators} />{condition.operator === "exists" ? <div className="flex items-center text-sm text-muted-foreground">{uiText("No requiere valor")}</div> : <Input value={Array.isArray(condition.values) ? condition.values.join(", ") : String(condition.value ?? "")} placeholder={condition.operator.includes("in") ? "valor 1, valor 2" : "Valor"} onChange={(event) => updateCondition(index, condition.operator.includes("in") ? { ...condition, value: undefined, values: event.target.value.split(",").map((value) => value.trim()).filter(Boolean) } : { ...condition, value: event.target.value, values: undefined })} />}<Button size="icon" variant="ghost" aria-label={uiText("Eliminar condición")} onClick={() => setDraft({ ...draft, conditions: draft.conditions.filter((_, itemIndex) => itemIndex !== index) })}><Trash2 className="size-4" /></Button></div>)}</section>
    <section className="space-y-3"><div className="flex items-center justify-between"><div><h3 className="font-semibold">{uiText("Entonces ejecuta estas acciones")}</h3><p className="text-sm text-muted-foreground">{uiText("El orden visual es el orden de ejecución.")}</p></div><Button size="sm" variant="secondary" onClick={() => setDraft({ ...draft, consequences: [...draft.consequences, { type: catalog.actions[0].value }] })}><Plus className="size-4" />{uiText("Acción")}</Button></div>{draft.consequences.map((action, index) => <ActionEditor key={`${action.type}-${index}`} action={action} position={index} catalog={catalog} onChange={(value) => updateAction(index, value)} onMove={moveAction} onDelete={() => setDraft({ ...draft, consequences: draft.consequences.filter((_, itemIndex) => itemIndex !== index) })} total={draft.consequences.length} />)}</section>
    {rule ? <section className="space-y-3 rounded-2xl border border-dashed p-5"><div className="flex items-center gap-2"><Beaker className="size-5 text-brand" /><h3 className="font-semibold">{uiText("Simulación segura")}</h3></div><p className="text-sm text-muted-foreground">{uiText("El sistema construirá un evento de prueba con las condiciones configuradas y validará la secuencia sin ejecutar acciones ni modificar datos.")}</p><Button variant="secondary" onClick={() => simulate.mutate()} disabled={simulate.isPending}><Beaker className="size-4" />{uiText("Simular regla")}</Button></section> : null}
    <div className="min-w-0 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end"><Button variant="secondary" onClick={onClose}>{uiText("Cancelar")}</Button><Button disabled={!valid || save.isPending} onClick={() => save.mutate()}>{rule ? uiText("Guardar nueva versión") : uiText("Guardar borrador")}</Button></div>
...
function FlowConnector() { return <span aria-hidden="true" className="text-lg font-semibold text-brand">-&gt;</span>; }
...
function ActionEditor({ action, position, catalog, total, onChange, onMove, onDelete }: { action: NoCodeAutomationAction; position: number; catalog: NoCodeAutomationCatalogDto; total: number; onChange: (action: NoCodeAutomationAction) => void; onMove: (index: number, direction: -1 | 1) => void; onDelete: () => void }) {
  ...
  return <div className="rounded-xl border p-4"><div className="flex items-start gap-3"><span className="flex size-8 shrink-0 items-center justify-center rounded-full bg-primary text-sm font-bold text-primary-foreground">{position + 1}</span><div className="min-w-0 flex-1 space-y-3"><FormSelect value={action.type} onValueChange={(type) => onChange({ type: type as NoCodeAutomationAction["type"] })} options={catalog.actions.map((item) => ({ value: item.value, label: item.label }))} /><p className="text-sm text-muted-foreground">{definition.description}</p><div className="grid gap-3 md:grid-cols-2">{definition.fields.map((field) => field === "stepKey" ? <label key={field} className="space-y-1 text-xs font-medium">{uiText("Etapa")}<FormSelect value={action.stepKey} placeholder={uiText("Selecciona una etapa")} onValueChange={(stepKey) => onChange({ ...action, stepKey })} options={catalog.workflowStages} /></label> : <label key={field} className="space-y-1 text-xs font-medium">{actionFieldLabel(field)}<Input type={field === "dueDate" ? "date" : field === "quantity" ? "number" : "text"} value={String(action[field as keyof NoCodeAutomationAction] ?? "")} onChange={(event) => onChange({ ...action, [field]: field === "quantity" ? Number(event.target.value) : event.target.value || undefined })} /></label>)}</div></div><div className="flex shrink-0 flex-col"><Button size="icon" variant="ghost" disabled={position === 0} aria-label={uiText("Subir acción")} onClick={() => onMove(position, -1)}><ArrowUp className="size-4" /></Button><Button size="icon" variant="ghost" disabled={position === total - 1} aria-label={uiText("Bajar acción")} onClick={() => onMove(position, 1)}><ArrowDown className="size-4" /></Button><Button size="icon" variant="ghost" aria-label={uiText("Eliminar acción")} onClick={onDelete}><Trash2 className="size-4 text-destructive" /></Button></div></div></div>;
}

function SimulationDialog({ result, onClose }: { result: NoCodeAutomationSimulationDto | null; onClose: () => void }) {
  const uiText = useUiText();
  return <Dialog open={Boolean(result)} onOpenChange={(open) => !open && onClose()}><DialogContent><DialogHeader><DialogTitle>{uiText("Resultado de la simulación")}</DialogTitle><DialogDescription>{result?.message}</DialogDescription></DialogHeader>{result ? <div className="space-y-4"><Badge variant={result.willExecute ? "success" : "warning"}>{result.willExecute ? "Lista para ejecutarse" : "No se ejecutaría"}</Badge><div className="space-y-2"><h3 className="font-semibold">{uiText("Condiciones")}</h3>{result.conditions.length ? result.conditions.map((item, index) => <div key={index} className="flex justify-between rounded-lg bg-muted/50 p-3 text-sm"><span>{item.condition.field} · {item.condition.operator}</span><Badge variant={item.matches ? "success" : "secondary"}>{item.matches ? "Cumple" : "No cumple"}</Badge></div>) : <p className="text-sm text-muted-foreground">{uiText("Se ejecutaría para cualquier evento.")}</p>}</div><div className="space-y-2"><h3 className="font-semibold">{uiText("Acciones")}</h3>{result.actions.map((item) => <div key={item.position} className="rounded-lg bg-muted/50 p-3 text-sm"><p>{item.position}. {item.type}</p>{item.valid.reason ? <p className="mt-1 text-xs text-destructive">{item.valid.reason}</p> : null}</div>)}</div></div> : null}</DialogContent></Dialog>;
}
...
function Pagination({ page, totalPages, onPage }: { page: number; totalPages: number; onPage: (page: number) => void }) {
  ...
  return <div className="mt-5 flex items-center justify-between rounded-xl border bg-card/70 px-4 py-3"><p className="text-sm text-muted-foreground">{uiText("Página ")}{page} {uiText(" de ")}{totalPages}</p><div className="flex gap-2"><Button size="sm" variant="secondary" disabled={page <= 1} onClick={() => onPage(page - 1)}>{uiText("Anterior")}</Button><Button size="sm" variant="secondary" disabled={page >= totalPages} onClick={() => onPage(page + 1)}>{uiText("Siguiente")}</Button></div></div>;
}
