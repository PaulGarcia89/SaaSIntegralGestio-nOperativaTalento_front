...
function Metric({ icon: Icon, label, value }: { icon: typeof CreditCard; label: string; value: string }) { return <Card level={2}><CardContent className="p-5"><Icon className="size-5 text-brand" /><p className="mt-4 text-sm text-text-secondary">{label}</p><p className="mt-1 font-semibold">{value}</p></CardContent></Card>; }
