...
function AnalyticsContent({ data, candidateJourney }: { data: AtsAnalyticsDto; candidateJourney?: Awaited<ReturnType<typeof fetchCandidateConversionMetrics>> }) {
  ...
  return <div className="space-y-7">
    <section className="flex flex-col gap-2 rounded-2xl border bg-surface-section p-4 text-sm text-text-secondary lg:flex-row lg:items-center lg:justify-between">
      <span><strong className="text-text-primary">{uiText("Periodo:")}</strong> {formatDate(data.period.from)} – {formatDate(data.period.to)}</span>
      ...
      <span>{uiText("Actualizado ")}{formatDateTime(data.generatedAt)}</span>
    </section>
    ...
    <section className="grid gap-5 xl:grid-cols-[1.35fr_1fr]">
      ...
      <Card level={2}><CardHeader><CardTitle>{t("analytics.volumeAndDecisions")}</CardTitle><p className="text-sm text-text-secondary">{uiText("Evolución ")}{granularityLabel(data.filters.granularity).toLocaleLowerCase()} {uiText(" de nuevas postulaciones.")}</p></CardHeader><CardContent><Trend rows={data.trends} /></CardContent></Card>
    </section>

    <CandidateJourneyCard data={candidateJourney} />

    <section className="grid gap-5 xl:grid-cols-3">
      <OperationalCard title={t("analytics.interviews")} value={`${data.interviews.completionRate}%`} label={uiText("completadas")} rows={[["Sesiones", data.interviews.total], ["Ausencias", `${data.interviews.noShowRate}%`], [t("analytics.avgRating"), data.interviews.averageScore || "—"], ["Anticipación", duration(data.interviews.averageSchedulingLeadHours)]]} />
      <OperationalCard title={uiText("Ofertas")} value={`${data.offers.acceptanceRate}%`} label={uiText("aceptadas")} rows={[["Emitidas", data.offers.total], ["Aceptadas", data.offers.accepted], ["Contrapropuestas", `${data.offers.counterOfferRate}%`], [t("analytics.approvalTime"), duration(data.offers.averageApprovalHours)]]} />
...
function Funnel({ rows, total }: { rows: AtsAnalyticsDto["funnel"]; total: number }) {
  ...
  return <ol className="space-y-4">{rows.map((row, index) => <li key={row.stageCode} className="grid gap-2 sm:grid-cols-[9rem_1fr_7rem]"><div><p className="truncate text-sm font-medium">{row.stageName}</p><p className="text-xs text-text-secondary">{duration(row.averageHours)} {uiText(" promedio")}</p></div><div className="self-center"><div className="h-8 overflow-hidden rounded-lg bg-surface-section"><div className="flex h-full min-w-10 items-center justify-end rounded-lg bg-[linear-gradient(90deg,var(--color-primary),color-mix(in_srgb,var(--color-primary)_65%,var(--color-status-success)))] px-2 text-xs font-semibold text-surface-dark-ink transition-[width]" style={{ width: `${Math.max(4, total ? row.reached / total * 100 : 0)}%` }}>{row.reached}</div></div></div><div className="text-right text-xs"><p className="font-semibold">{index ? `${row.conversionRate}% continúa` : `${row.conversionFromApplications}% entra`}</p><p className={row.dropOff ? "text-status-danger" : "text-text-secondary"}>{row.dropOff} {uiText(" pérdidas")}</p></div></li>)}</ol>;
}
...
function Trend({ rows }: { rows: AtsAnalyticsDto["trends"] }) {
  ...
  return <div><div className="min-w-0 flex h-56 items-end gap-1.5 border-b border-border-default px-1">{rows.map((row) => <div key={row.period} className="group relative flex min-w-1 flex-1 items-end" title={`${formatDate(row.period)}: ${row.applications} postulaciones, ${row.hires} contrataciones`}><div className="w-full rounded-t bg-primary/75 transition-colors group-hover:bg-primary" style={{ height: `${Math.max(3, row.applications / max * 100)}%` }}><div className="w-full bg-status-success" style={{ height: `${row.applications ? row.hires / row.applications * 100 : 0}%` }} /></div></div>)}</div><div className="mt-3 flex items-center justify-between text-xs text-text-secondary"><span>{formatDate(rows[0].period)}</span><span className="flex gap-3"><i className="size-2 rounded-full bg-primary" />{uiText("Postulaciones ")}<i className="size-2 rounded-full bg-status-success" />{t("analytics.hires")}</span><span>{formatDate(rows.at(-1)!.period)}</span></div></div>;
}
