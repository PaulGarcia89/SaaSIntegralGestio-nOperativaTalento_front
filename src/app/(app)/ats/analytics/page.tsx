  ...
  return <div className="space-y-7">
    <PageHeader
      ...
      actions={<div className="flex flex-wrap gap-2">
        {can("candidates.update") ? <Button variant="secondary" onClick={() => setCostOpen(true)}><DollarSign className="size-4" />{uiText("Registrar coste")}</Button> : null}
        {can("candidates.update") ? <Button variant="secondary" onClick={() => setQualityOpen(true)}><Star className="size-4" />{uiText("Calidad 30/60/90")}</Button> : null}
    ...
    <Card level={2} className="overflow-hidden">
      ...
      <CardContent className="grid gap-4 p-5 md:grid-cols-2 xl:grid-cols-6">
        ...
        <Filter label={t("analytics.grouping")}><FormSelect value={filters.granularity ?? "week"} onValueChange={(value) => setFilters({ ...filters, granularity: value as "day" | "week" | "month" })} options={[{ value: "day", label: "Diaria" }, { value: "week", label: uiText("Semanal") }, { value: "month", label: uiText("Mensual") }]} /></Filter>
        <div className="flex flex-wrap gap-2 md:col-span-2 xl:col-span-6">{[7, 30, 90, 180].map((days) => <Button key={days} size="sm" variant="secondary" onClick={() => setPeriod(days)}>{days} {uiText(" días")}</Button>)}</div>
      </CardContent>
    </Card>
...
function AnalyticsContent({ data, candidateJourney }: { data: AtsAnalyticsDto; candidateJourney?: Awaited<ReturnType<typeof fetchCandidateConversionMetrics>> }) {
  ...
  return <div className="space-y-7">
    <section className="flex flex-col gap-2 rounded-2xl border bg-surface-section p-4 text-sm text-text-secondary lg:flex-row lg:items-center lg:justify-between">
      <span><strong className="text-text-primary">{uiText("Periodo:")}</strong> {formatDate(data.period.from, uiText.locale)} – {formatDate(data.period.to, uiText.locale)}</span>
      <span><strong className="text-text-primary">{uiText("Alcance:")}</strong> {data.scope.branchName ?? (data.scope.type === "GLOBAL" ? "Global" : t("analytics.wholeCompany"))}</span>
      <span><strong className="text-text-primary">{uiText("Fuente:")}</strong> {uiText(" datos persistentes")}</span>
      <span>{uiText("Actualizado ")}{formatDateTime(data.generatedAt, uiText.locale)}</span>
    </section>

    <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      <Metric icon={Users} label={t("analytics.applications")} value={data.summary.applications} change={data.summary.changes.applications} detail={uiText("{{count}} candidatos únicos", { count: data.summary.uniqueCandidates })} />
      ...
      <Metric icon={Clock3} label={t("analytics.avgTimeToHire")} value={duration(data.summary.averageTimeToHireHours, uiText.locale)} change={data.summary.changes.averageTimeToHireHours} detail={`Mediana ${duration(data.summary.medianTimeToHireHours, uiText.locale)}`} inverse />
    ...
    <section className="grid gap-5 xl:grid-cols-[1.35fr_1fr]">
      ...
      <Card level={2}><CardHeader><CardTitle>{t("analytics.volumeAndDecisions")}</CardTitle><p className="text-sm text-text-secondary">{uiText("Evolución ")}{uiText(granularityLabel(data.filters.granularity)).toLocaleLowerCase(uiText.locale)} {uiText(" de nuevas postulaciones.")}</p></CardHeader><CardContent><Trend rows={data.trends} /></CardContent></Card>
    </section>

    <CandidateJourneyCard data={candidateJourney} />

    <section className="grid gap-5 xl:grid-cols-3">
      <OperationalCard title={t("analytics.interviews")} value={`${data.interviews.completionRate}%`} label={uiText("completadas")} rows={[["Sesiones", data.interviews.total], ["Ausencias", `${data.interviews.noShowRate}%`], [t("analytics.avgRating"), data.interviews.averageScore || "—"], [uiText("Anticipación"), duration(data.interviews.averageSchedulingLeadHours, uiText.locale)]]} />
      <OperationalCard title={uiText("Ofertas")} value={`${data.offers.acceptanceRate}%`} label={uiText("aceptadas")} rows={[["Emitidas", data.offers.total], ["Aceptadas", data.offers.accepted], ["Contrapropuestas", `${data.offers.counterOfferRate}%`], [t("analytics.approvalTime"), duration(data.offers.averageApprovalHours, uiText.locale)]]} />
      <OperationalCard title={t("analytics.slaPipeline")} value={data.sla.breached} label={uiText("casos vencidos")} rows={[["Medibles", data.sla.measurable], ["Advertencias", data.sla.warningSent], ["Escalados", data.sla.escalated], ["Reasignados", data.sla.reassigned]]} danger={data.sla.breached > 0} />
    </section>

    <section className="grid gap-5 xl:grid-cols-2">
      <TableCard title={t("analytics.efficiencyAndCost")} headers={["Fuente", t("analytics.applications"), t("analytics.hires"), "Coste", uiText("Coste/contratación")]} rows={data.sources.map((item) => [item.source, item.applications, item.hires, money(item.cost, item.currency, uiText.locale), item.costPerHire === null ? "—" : money(item.costPerHire, item.currency, uiText.locale)])} />
      <TableCard title={t("analytics.hireQuality")} headers={["Hito", "Revisiones", t("analytics.avgPerformance"), t("analytics.retention")]} rows={data.qualityOfHire.byCheckpoint.map((item) => [uiText("{{count}} días", { count: item.checkpointDays }), item.reviews, item.averagePerformanceScore ? `${item.averagePerformanceScore}/5` : "—", `${item.retentionRate}%`])} />
      ...
      <TableCard title={t("analytics.recruiterLoad")} headers={[t("analytics.owner"), "Activas", t("analytics.outOfSla"), t("analytics.conversion"), "Edad media"]} rows={data.recruiters.map((item) => [item.name, item.active, item.overdue, `${item.conversionRate}%`, duration(item.averageActiveStageHours, uiText.locale)])} />
    </section>
  </div>;
}
...
function Funnel({ rows, total }: { rows: AtsAnalyticsDto["funnel"]; total: number }) {
  ...
  return <ol className="space-y-4">{rows.map((row, index) => <li key={row.stageCode} className="grid gap-2 sm:grid-cols-[9rem_1fr_7rem]"><div><p className="truncate text-sm font-medium">{row.stageName}</p><p className="text-xs text-text-secondary">{duration(row.averageHours, uiText.locale)} {uiText(" promedio")}</p></div><div className="self-center"><div className="h-8 overflow-hidden rounded-lg bg-surface-section"><div className="flex h-full min-w-10 items-center justify-end rounded-lg bg-[linear-gradient(90deg,var(--color-primary),color-mix(in_srgb,var(--color-primary)_65%,var(--color-status-success)))] px-2 text-xs font-semibold text-surface-dark-ink transition-[width]" style={{ width: `${Math.max(4, total ? row.reached / total * 100 : 0)}%` }}>{row.reached}</div></div></div><div className="text-right text-xs"><p className="font-semibold">{index ? uiText("{{rate}}% continúa", { rate: row.conversionRate }) : uiText("{{rate}}% entra", { rate: row.conversionFromApplications })}</p><p className={row.dropOff ? "text-status-danger" : "text-text-secondary"}>{row.dropOff} {uiText(" pérdidas")}</p></div></li>)}</ol>;
}
...
function Trend({ rows }: { rows: AtsAnalyticsDto["trends"] }) {
  ...
  return <div><div className="min-w-0 flex h-56 items-end gap-1.5 border-b border-border-default px-1">{rows.map((row) => <div key={row.period} className="group relative flex min-w-1 flex-1 items-end" title={uiText("{{date}}: {{applications}} postulaciones, {{hires}} contrataciones", { date: formatDate(row.period, uiText.locale), applications: row.applications, hires: row.hires })}><div className="w-full rounded-t bg-primary/75 transition-colors group-hover:bg-primary" style={{ height: `${Math.max(3, row.applications / max * 100)}%` }}><div className="w-full bg-status-success" style={{ height: `${row.applications ? row.hires / row.applications * 100 : 0}%` }} /></div></div>)}</div><div className="mt-3 flex items-center justify-between text-xs text-text-secondary"><span>{formatDate(rows[0].period, uiText.locale)}</span><span className="flex gap-3"><i className="size-2 rounded-full bg-primary" />{uiText("Postulaciones ")}<i className="size-2 rounded-full bg-status-success" />{t("analytics.hires")}</span><span>{formatDate(rows.at(-1)!.period, uiText.locale)}</span></div></div>;
}
...
function OperationalCard({ title, value, label, rows, danger }: { title: string; value: string | number; label: string; rows: Array<[string, string | number]>; danger?: boolean }) {
  const uiText = useUiText();
  return <Card level={2}><CardHeader><div className="min-w-0 flex items-start justify-between"><div><CardTitle>{title}</CardTitle><p className={`mt-2 text-4xl font-semibold tabular-nums ${danger ? "text-status-danger" : ""}`}>{value}</p><p className="text-xs text-text-secondary">{label}</p></div>{danger ? <AlertTriangle className="size-5 text-status-danger" /> : <BriefcaseBusiness className="size-5 text-brand" />}</div></CardHeader><CardContent><dl className="grid grid-cols-2 gap-2">{rows.map(([name, metric]) => <div key={name} className="rounded-xl bg-surface-section p-3"><dt className="text-xs text-text-secondary">{uiText(name)}</dt><dd className="mt-1 font-semibold tabular-nums">{metric}</dd></div>)}</dl></CardContent></Card>;
}

function TableCard({ title, headers, rows }: { title: string; headers: string[]; rows: Array<Array<string | number>> }) {
  const uiText = useUiText();
  return <Card level={2}><CardHeader><CardTitle>{title}</CardTitle></CardHeader><CardContent>{rows.length ? <><div className="min-w-0 grid gap-3 md:hidden">{rows.map((row, index) => <article key={`${row[0]}-${index}`} className="rounded-xl border border-border-default p-3"><p className="font-semibold">{row[0]}</p><dl className="mt-3 grid grid-cols-2 gap-3">{row.slice(1).map((cell, cellIndex) => <div key={headers[cellIndex + 1]}><dt className="text-xs text-text-secondary">{uiText(headers[cellIndex + 1])}</dt><dd className="mt-1 text-sm font-medium">{cell}</dd></div>)}</dl></article>)}</div><div tabIndex={0} role="group" aria-label={title} className="hidden max-h-80 overflow-auto rounded-xl border focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-border-focus md:block"><table className="w-full text-left text-sm"><thead className="sticky top-0 bg-surface-interactive"><tr>{headers.map((header) => <th key={header} className="whitespace-nowrap px-3 py-2 font-medium">{uiText(header)}</th>)}</tr></thead><tbody>{rows.map((row, index) => <tr key={`${row[0]}-${index}`} className="border-t">{row.map((cell, cellIndex) => <td key={cellIndex} className="whitespace-nowrap px-3 py-2">{cellIndex === 0 ? <span className="font-medium">{cell}</span> : cell}</td>)}</tr>)}</tbody></table></div></> : <Empty />}</CardContent></Card>;
}
...
function HiringQualityDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  const mutation = useMutation({ mutationFn: () => saveAtsHiringQuality({ employeeId, checkpointDays, performanceScore: Number(performanceScore), retained: retained === "yes", managerComment: comment || undefined }), onSuccess: async () => { setComment(""); await employees.refetch(); toast.success(uiText("Revisión de calidad registrada")); }, onError: () => toast.error(uiText("No fue posible registrar la revisión")) });
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent><DialogHeader><DialogTitle>{t("analytics.hireQuality")}</DialogTitle><DialogDescription>{t("analytics.hireQualityBody")}</DialogDescription></DialogHeader><div className="space-y-4"><FormSelect value={String(checkpointDays)} onValueChange={(value) => { setCheckpointDays(Number(value) as 30 | 60 | 90); setEmployeeId(""); }} options={[{ value: "30", label: t("analytics.days30") }, { value: "60", label: t("analytics.days60") }, { value: "90", label: t("analytics.days90") }]} /><FormSelect value={employeeId} placeholder={t("analytics.selectEmployee")} onValueChange={setEmployeeId} options={eligible.map((employee) => ({ value: employee.id, label: `${employee.name}${employee.jobTitle ? ` · ${employee.jobTitle}` : ""}` }))} /><div className="grid grid-cols-2 gap-3"><FormSelect value={performanceScore} onValueChange={setPerformanceScore} options={[1, 2, 3, 4, 5].map((value) => ({ value: String(value), label: uiText("{{score}}/5 desempeño", { score: value }) }))} /><FormSelect value={retained} onValueChange={setRetained} options={[{ value: "yes", label: t("analytics.staysActive") }, { value: "no", label: t("analytics.doesNotStay") }]} /></div><textarea className="field min-h-20" value={comment} onChange={(event) => setComment(event.target.value)} placeholder={t("analytics.evidencePlaceholder")} /><Button className="w-full" disabled={!employeeId || mutation.isPending} onClick={() => mutation.mutate()}>{mutation.isPending ? uiText("Guardando…") : t("analytics.saveReview")}</Button></div></DialogContent></Dialog>;
}
function Empty() {
  const { t } = useLocale();
...
function formatDate(value: string, locale: "es" | "en") { return new Intl.DateTimeFormat(locale, { dateStyle: "medium", timeZone: "UTC" }).format(new Date(value)); }
function formatDateTime(value: string, locale: "es" | "en") { return new Intl.DateTimeFormat(locale, { dateStyle: "medium", timeStyle: "short" }).format(new Date(value)); }
function duration(hours: number, locale: "es" | "en") { return new Intl.NumberFormat(locale, { style: "unit", unit: hours < 24 ? "hour" : "day", unitDisplay: "short", maximumFractionDigits: 1 }).format(hours < 24 ? hours : hours / 24); }
function money(amount: number, currency: string | null, locale: "es" | "en") { return new Intl.NumberFormat(locale, { style: "currency", currency: currency || "USD", maximumFractionDigits: 2 }).format(amount); }
