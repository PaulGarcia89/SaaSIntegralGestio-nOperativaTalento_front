...
import { createDocuSealHiringBundle, createDocuSealHiringBundleForApplication, createHiringContract, fetchApplication, fetchApplicationDecisionEvidence, fetchAtsCommunicationHistory, fetchDocuSealHiringBundleStatus, fetchHiringContext, fetchInterviewScorecardComparison, fetchInterviewerProfiles, fetchJobOffers, fetchRejectionReasons, fetchResumeAccess, hireCandidate, retryAtsCommunication, scheduleRecruitmentInterview, updateApplication, getApiErrorMessage } from "@/lib/backend";
...
function CandidateProfile({ application }: { application: VacancyApplicationDto }) {
  ...
  return <div className="space-y-6"><PageHeader eyebrow={uiText("Perfil 360°")} title={application.candidate.fullName} description={`${application.vacancy.title} · ${application.vacancy.branch?.name ?? t("p360.noBranch")}`} actions={<div className="flex flex-wrap gap-2"><Badge variant="secondary">{currentStageLabel}</Badge>{can("reports.export") ? <Button variant="secondary" onClick={() => decisionEvidence.mutate()} disabled={decisionEvidence.isPending}><Download className="size-4" />{decisionEvidence.isPending ? "Preparando evidencia…" : t("p360.exportRecord")}</Button> : null}</div>} />
    ...
    {save.isSuccess ? <InlineFeedback tone="success" title={uiText("Cambios guardados")}>{save.data.pendingTransitions?.length ? t("p360.stagePending") : t("p360.stageUpdated")}</InlineFeedback> : null}{save.isError ? <InlineFeedback tone="danger" title={t("p360.saveFailed")}>{getApiErrorMessage(save.error, t("p360.changesNotApplied"))}</InlineFeedback> : null}
    <div className="grid gap-5 xl:grid-cols-[minmax(0,2fr)_minmax(300px,1fr)]"><div className="space-y-5">
      ...
      <Card level={2}><CardHeader><CardTitle>{t("p360.application")}</CardTitle></CardHeader><CardContent className="space-y-5"><Section title={t("p360.coverLetter")} empty={t("p360.noCoverLetter")}>{application.coverLetter}</Section><section><h3 className="font-semibold">{t("p360.formAnswers")}</h3>{responses.length ? <dl className="mt-3 grid gap-3 sm:grid-cols-2">{responses.map(([key, value]) => <div key={key} className="rounded-xl bg-surface-section p-3"><dt className="text-xs font-medium text-text-secondary">{key.replaceAll("_", " ")}</dt><dd className="mt-1 text-sm">{Array.isArray(value) ? value.join(", ") : typeof value === "boolean" ? value ? uiText("Sí") : "No" : String(value)}</dd></div>)}</dl> : <p className="mt-2 text-sm text-text-secondary">{t("p360.noAnswers")}</p>}</section></CardContent></Card>
      </div> : null}
    ...
    {hire.isSuccess ? <InlineFeedback tone={hiringDocuments.isError ? "warning" : "success"} title={t("p360.hiringFormalized")}>{uiText("El empleado fue creado y la incorporación se activó automáticamente. Flujo: ")}{hire.data.id}{hiringDocuments.isPending ? ` ${t("p360.docusealSending")}` : hiringDocuments.isError ? ` La contratación quedó creada, pero DocuSeal requiere atención: ${getApiErrorMessage(hiringDocuments.error, t("p360.checkTemplates"))}.` : ` ${t("p360.docusealDone")}`}</InlineFeedback> : null}
...
function EvaluationStatusCard({ application, onComplete, canEvaluate }: { application: VacancyApplicationDto; onComplete: (interviewId: string) => void; canEvaluate: boolean }) {
  ...
  if (upcoming) return <InlineFeedback tone="info" title={t("p360.nextInterview")}>{formatApplicationDate(upcoming.startsAt)} · {upcoming.timezone}{uiText(". Al terminar, registra la evaluación desde esta misma sección.")}</InlineFeedback>;
...
function HiringProgressGuide({ application, offers, docuSealSent, docuSealStatus, canHire, canCreateContract, canSendDocuments, canManage, onHire, onCreateContract, creatingContract, onSendDocuments, sendingDocuments }: { application: VacancyApplicationDto; offers: JobOfferDto[]; docuSealSent: boolean; docuSealStatus?: { allSent: boolean; allCompleted: boolean } | null; canHire: boolean; canCreateContract: boolean; canSendDocuments: boolean; canManage: boolean; onHire: () => void; onCreateContract: () => void; creatingContract: boolean; onSendDocuments: () => void; sendingDocuments: boolean }) {
  ...
  const steps: Array<{ label: string; state: GuideStepState; detail: string }> = [
    ...
    { label: uiText("Oferta"), state: offerAccepted ? "complete" : activeOffer ? "attention" : "pending", detail: offerAccepted ? t("p360.step.offerAccepted") : activeOffer ? t("p360.step.offerNeedsAction") : t("p360.step.prepareOffer") },
  ...
  return <Card level={1} className="overflow-hidden border-primary/25 bg-primary/[0.03]"><CardContent className="space-y-5 p-5 sm:p-6">
    <div><p className="text-sm font-medium text-brand">{t("p360.transferToHiring")}</p><h2 className="mt-1 text-xl font-semibold">{t("p360.transferHint")}</h2><p className="mt-1 text-sm text-text-secondary">{uiText("Paso actual: ")}<span className="font-medium text-text-primary">{next.label}</span></p></div>
    ...
    <div className="flex flex-col gap-4 rounded-xl border border-border-default bg-surface-elevated p-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-medium text-brand">{uiText("Siguiente paso recomendado")}</p><p className="mt-1 font-semibold">{next.detail}</p></div>{"onClick" in action ? <Button onClick={action.onClick} disabled={sendingDocuments}>{action.label}<ArrowRight className="size-4" /></Button> : <Button asChild><a href={action.href}>{action.label}<ArrowRight className="size-4" /></a></Button>}</div>
