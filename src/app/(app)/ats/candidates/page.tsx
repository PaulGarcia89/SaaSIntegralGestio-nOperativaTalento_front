...
function CandidatesContent() {
  ...
  const stages = useMemo(() => {
    const source = vacancyId === ALL ? allItems : allItems.filter((item) => item.vacancy.id === vacancyId);
    return Array.from(new Map(source.flatMap((item) => item.vacancy.stages ?? []).map((item) => [item.id, item])).values()).sort((a, b) => a.position - b.position);
  }, [allItems, vacancyId]);
  const items = useMemo(() => {
    const query = search.trim().toLocaleLowerCase("es");
    return allItems.filter((item) => (!query || `${item.candidate.fullName} ${item.candidate.email} ${item.vacancy.title}`.toLocaleLowerCase("es").includes(query)) && (stage === ALL || item.currentStageId === stage || item.currentStage?.id === stage) && (vacancyId === ALL || item.vacancy.id === vacancyId));
