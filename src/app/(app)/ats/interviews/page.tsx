...
// Los rótulos salen de `interviews.day.<n>`; aquí solo el orden de la semana.
...
export default function InterviewsPage() {
  ...
  const [form, setForm] = useState<ScheduleInterviewInput>({ applicationId: "", interviewerUserId: "", title: uiText("Entrevista"), type: "VIRTUAL", timezone: defaultTimezone, startsAt: "", endsAt: "" });
  ...
  const schedule = useMutation({ onError: (error) => toast.error(getApiErrorMessage(error, t("interviews.scheduleErrorBody"))), mutationFn: async (input: ScheduleInterviewInput) => {
    ...
    if (extraRounds.length) return scheduleInterviewSequence({ applicationId: input.applicationId, title: `${input.title} · ${t("interviews.sequenceSuffix")}`, rounds: [input, ...extraRounds.map((round) => ({ ...input, title: round.title, startsAt: round.startsAt, endsAt: round.endsAt }))] });
    return scheduleRecruitmentInterview(input);
  }, onSuccess: async (result) => { await queryClient.invalidateQueries({ queryKey: ["recruitment-interviews"] }); await queryClient.invalidateQueries({ queryKey: ["interview-coordination-queue"] }); if ("url" in result) { await navigator.clipboard?.writeText(result.url); toast.success(t("interviews.linkCreated")); } else { toast.success(extraRounds.length ? t("interviews.sequenceScheduled") : t("interviews.scheduled")); } setScheduleOpen(false); setExtraRounds([]); } });
  const updateInterview = useMutation({
    ...
    onSuccess: async (_result, variables) => {
      ...
      toast.success(variables.input.status === "CANCELED" ? t("interviews.cancelledToast") : t("interviews.rescheduledToast"));
      setEditingInterview(null);
    },
    onError: (error) => toast.error(getApiErrorMessage(error, t("interviews.updateErrorBody"))),
  });
  const connectCalendar = useMutation({ onError: (error) => toast.error(getApiErrorMessage(error, t("interviews.authorizeErrorBody"))), mutationFn: async (provider: CalendarProvider) => { const redirectUri = `${window.location.origin}/ats/interviews?calendarProvider=${provider}`; const authorization = await fetchCalendarAuthorizationUrl(provider, redirectUri); window.location.assign(authorization.authorizationUrl); } });
  const disconnect = useMutation({
    mutationFn: disconnectCalendar,
    onSuccess: async () => { toast.success(t("interviews.calendarDisconnected")); await queryClient.invalidateQueries({ queryKey: ["calendar-connections"] }); },
    onError: (error) => toast.error(getApiErrorMessage(error, t("interviews.disconnectErrorBody"))),
  });
  const retrySync = useMutation({
    mutationFn: retryInterviewCalendarSync,
    onSuccess: async () => { toast.success(t("interviews.syncRetried")); await queryClient.invalidateQueries({ queryKey: ["recruitment-interviews"] }); },
    onError: (error) => toast.error(getApiErrorMessage(error, t("interviews.retrySyncErrorBody"))),
  });
  const invitationResponse = useMutation({ onError: (error) => toast.error(getApiErrorMessage(error, t("interviews.responseErrorBody"))), mutationFn: ({ interviewId, accepted }: { interviewId: string; accepted: boolean }) => respondInterviewInvitation(interviewId, accepted), onSuccess: async (_, variables) => { toast.success(variables.accepted ? t("interviews.participationConfirmed") : t("interviews.invitationDeclined")); await queryClient.invalidateQueries({ queryKey: ["recruitment-interviews"] }); await queryClient.invalidateQueries({ queryKey: ["interview-coordination-queue"] }); } });
  const saveAvailability = useMutation({ mutationFn: updateAvailabilitySettings, onSuccess: async (saved) => { queryClient.setQueryData(["availability-settings"], saved); toast.success(t("interviews.availabilitySaved")); setAvailabilityOpen(false); } });
  const downloadIcs = useMutation({ onError: (error) => toast.error(getApiErrorMessage(error, t("interviews.downloadErrorBody"))), mutationFn: async (interviewId: string) => ({ interviewId, blob: await downloadInterviewInvitation(interviewId) }), onSuccess: ({ interviewId, blob }) => { const url = URL.createObjectURL(blob); const anchor = document.createElement("a"); anchor.href = url; anchor.download = `entrevista-${interviewId}.ics`; anchor.click(); URL.revokeObjectURL(url); } });
   ...
   */
  return <div className="space-y-7"><PageHeader eyebrow={t("interviews.eyebrow")} title={t("interviews.title")} description={t("interviews.description")} actions={can("interviews.schedule") ? <Button onClick={() => setScheduleOpen(true)}><Plus className="size-4" />{t("interviews.schedule")}</Button> : undefined} />
    ...
    {myPending.length ? <section aria-labelledby="my-invitations-title" className="space-y-3 rounded-lg border border-status-warning/40 bg-status-warning/5 p-4 sm:p-5"><h2 id="my-invitations-title" className="flex items-center gap-2 text-lg font-semibold text-ink-1"><BellRing className="size-5 text-status-warning" aria-hidden="true" />{t("interviews.myInvitations", { count: myPending.length })}</h2><ul className="space-y-2">{myPending.map((interview) => <li key={interview.id} className="flex flex-col justify-between gap-3 rounded-md bg-surface-1 p-3 sm:flex-row sm:items-center"><div className="min-w-0"><p className="truncate font-medium text-ink-1">{interview.application?.candidate.fullName ?? interview.title}</p><p className="text-sm text-ink-2">{new Date(interview.startsAt).toLocaleString(uiText.locale, { dateStyle: "medium", timeStyle: "short" })} · {interview.title}</p></div><div className="flex gap-2"><Button onClick={() => invitationResponse.mutate({ interviewId: interview.id, accepted: true })} disabled={invitationResponse.isPending}><CircleCheck className="size-4" />{t("interviews.accept")}</Button><Button variant="secondary" onClick={() => invitationResponse.mutate({ interviewId: interview.id, accepted: false })} disabled={invitationResponse.isPending}>{t("interviews.decline")}</Button></div></li>)}</ul></section> : null}
    ...
    <details className="group rounded-lg border border-line bg-surface-1" open={Boolean(oauthFeedback)}>
      ...
      <div className="space-y-4 border-t border-line px-4 py-4 sm:px-5"><p className="text-sm text-ink-2">{t("interviews.calendarBody")}</p>{providerConfiguration.data?.some((item) => !item.configured) ? <InlineFeedback tone="warning" title={t("interviews.oauthPending")}>{t("interviews.oauthBody")}</InlineFeedback> : null}<ul className="grid gap-3 sm:grid-cols-3 [&>li]:min-w-0">{(["GOOGLE", "MICROSOFT", "ZOOM"] as CalendarProvider[]).map((provider) => { const connection = connections.data?.find((item) => item.provider === provider && item.status === "ACTIVE"); const configured = providerConfiguration.data?.find((item) => item.provider === provider)?.configured === true; return <li key={provider} className="rounded-lg border border-line p-3"><div className="flex items-center justify-between gap-2"><p className="min-w-0 truncate font-medium text-ink-1">{provider === "GOOGLE" ? "Google Calendar / Meet" : provider === "MICROSOFT" ? "Outlook / Teams" : "Zoom"}</p><StatusBadge tone={connection ? "success" : configured ? "neutral" : "warning"} label={connection ? t("interviews.connected") : configured ? t("interviews.readyToAuthorize") : t("interviews.setupRequired")} /></div><p className="mt-1 truncate text-xs text-ink-3">{connection?.externalEmail || (configured ? t("interviews.authorizeAccount") : t("interviews.missingCredentials"))}</p>{connection ? <Button className="mt-3" size="sm" variant="ghost" disabled={disconnect.isPending} onClick={() => void confirmAction({ title: t("interviews.disconnectTitle", { provider: provider === "GOOGLE" ? "Google Calendar" : provider === "MICROSOFT" ? "Outlook" : "Zoom" }), description: t("interviews.disconnectBody"), consequence: t("interviews.disconnectConsequence"), confirmLabel: t("interviews.disconnectConfirm") }).then((ok) => ok && disconnect.mutate(provider))}>{t("interviews.disconnect")}</Button> : <Button className="mt-3" size="sm" variant="secondary" onClick={() => connectCalendar.mutate(provider)} disabled={!configured || providerConfiguration.isLoading || connectCalendar.isPending}><Link2 className="size-4" />{configured ? t("interviews.connect") : t("interviews.unavailable")}</Button>}</li>; })}</ul><Button variant="secondary" onClick={() => { setAvailabilityForm(availabilitySettings.data ?? defaultAvailability); setAvailabilityOpen(true); }}><Clock3 className="size-4" />{t("interviews.configureAvailability")}</Button></div>
    </details>
...
function SelectField({ label, value, onValueChange, options }: { label: string; value: string; onValueChange: (value: string) => void; options: Array<{ value: string; label: string }> }) { const { t } = useLocale(); return <label className="block space-y-2"><span className="text-sm font-medium text-foreground">{label}</span><Select value={value} onValueChange={onValueChange}><SelectTrigger><SelectValue placeholder={t("interviews.selectPlaceholder", { label: label.toLowerCase() })} /></SelectTrigger><SelectContent>{options.map((option) => <SelectItem key={option.value} value={option.value}>{option.label}</SelectItem>)}</SelectContent></Select></label>; }
...
function InterviewCard({ interview, canScore, canSchedule, busy, onScore, onReschedule, onDownload, onRetrySync }: { interview: RecruitmentInterviewDto; canScore: boolean; canSchedule: boolean; busy: boolean; onScore: () => void; onReschedule: () => void; onDownload: () => void; onRetrySync: () => void }) {
  ...
  const modality = [technicalLabel(interview.type, uiText.locale), interview.videoProvider && interview.videoProvider !== "NONE" ? technicalLabel(interview.videoProvider, uiText.locale) : null].filter(Boolean).join(" · ");
  ...
  return (
    <li className={cn("flex flex-col gap-4 rounded-lg border bg-surface-1 p-4 sm:p-5", interview.calendarSyncStatus === "FAILED" ? "border-status-danger/50" : "border-line")}>
      <div className="flex flex-wrap items-start gap-3">
        ...
        <div className="order-last basis-full sm:order-none sm:basis-auto sm:shrink-0"><StatusBadge tone={tone} label={technicalLabel(interview.status, uiText.locale)} /></div>
      </div>
      ...
      <div className="flex items-start gap-3 rounded-md bg-surface-2 p-3">
        ...
        <div className="min-w-0">
          <p className="font-medium text-ink-1">{sentenceCase(start.toLocaleString(uiText.locale, { timeZone: interview.timezone, dateStyle: "full" }))}</p>
          <p className="text-sm text-ink-2">{start.toLocaleString(uiText.locale, { timeZone: interview.timezone, timeStyle: "short" })} – {new Date(interview.endsAt).toLocaleString(uiText.locale, { timeZone: interview.timezone, timeStyle: "short" })} · {modality}</p>
        </div>
      </div>
      ...
      <dl className="grid gap-x-6 gap-y-1 text-sm sm:grid-cols-2 [&>div]:min-w-0">
        ...
        {interview.participants?.length ? <div className="sm:col-span-2"><dt className="text-xs text-ink-3">{t("interviews.panel")}</dt><dd><ul className="mt-1 flex flex-wrap gap-x-4 gap-y-1">{interview.participants.map((item) => { const state = PARTICIPANT_ICON[item.status] ?? PARTICIPANT_ICON.PENDING; const Icon = state.icon; return <li key={item.id} className="inline-flex items-center gap-1.5 text-ink-1"><Icon className={cn("size-4 shrink-0", state.className)} aria-hidden="true" />{item.user.firstName} {item.user.lastName}<span className="sr-only">, {technicalLabel(item.status, uiText.locale)}</span></li>; })}</ul></dd></div> : null}
      </dl>
      ...
      <div className="flex flex-wrap items-center gap-2 border-t border-line pt-4">
        ...
        <details className="group ml-auto basis-full sm:basis-auto">
          ...
          <dl className="mt-2 grid gap-x-6 gap-y-1 text-sm sm:grid-cols-2 [&>div]:min-w-0">
            ...
            <Summary label={t("interviews.calendar")} value={interview.calendarProvider ? `${technicalLabel(interview.calendarProvider, uiText.locale)} · ${calendarSyncLabel(interview.calendarSyncStatus, t)}` : t("interviews.atsIcsOnly")} />
            ...
            {interview.participants?.length ? <div className="sm:col-span-2"><dt className="text-xs text-ink-3">{t("interviews.panel")}</dt><dd className="text-ink-1">{interview.participants.map((item) => `${item.user.firstName} ${item.user.lastName} · ${technicalLabel(item.role, uiText.locale)} · ${technicalLabel(item.status, uiText.locale)}`).join("; ")}</dd></div> : null}
          </dl>
