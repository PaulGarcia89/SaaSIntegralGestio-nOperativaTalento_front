...
function PipelineContent() {
  ...
  const decide = useMutation({
    ...
    onSuccess: async (_result, variables) => {
      toast.success(variables.approved ? uiText("Aprobación registrada") : "Cambio rechazado");
      await queryClient.invalidateQueries({ queryKey: ["applications"] });
    },
      ...
      <Card level={2} key={application.id}>
        ...
        >
          ...
          <div className="space-y-1 text-xs text-text-secondary">
            <p>{uiText("Recibida: ")}{formatApplicationDate(application.appliedAt, uiText.locale)}</p>
            <p><span className="font-medium text-text-primary">{t("adv.next")}</span> {applicationNextAction(application.status, uiText.locale)}</p>
            {application.stageDueAt ? <p className={application.isStageOverdue ? "font-medium text-status-danger" : ""}><Clock3 className="mr-1 inline size-3.5" />{application.isStageOverdue ? t("adv.slaBreached") : `SLA: ${formatApplicationDate(application.stageDueAt, uiText.locale)}`}</p> : null}
          </div>
    ...
    <div className="space-y-6">
      ...
      <Dialog open={Boolean(automationStage)} onOpenChange={(open) => { if (!open && !createStageAutomation.isPending) setAutomationStage(null); }}>
        <DialogContent>
          ...
          <div className="space-y-4"><label className="block space-y-2 text-sm font-medium">{t("ats.title")}<Input value={automationTitle} maxLength={160} onChange={(event) => setAutomationTitle(event.target.value)} /></label><label className="block space-y-2 text-sm font-medium">{t("ats.instruction")}<textarea value={automationMessage} maxLength={1000} rows={4} onChange={(event) => setAutomationMessage(event.target.value)} className="w-full rounded-xl border border-border-default bg-surface-elevated p-3 text-base sm:text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-border-focus" /></label><p className="text-xs text-text-secondary">{t("ats.manageAutomation")}</p><Button className="w-full" onClick={() => void confirmAction({ title: uiText("¿Activar la automatización de «{{stage}}»?", { stage: automationStage?.name ?? "" }), description: uiText("Quedará encendida y se ejecutará sola cada vez que una candidatura llegue a esa etapa."), consequence: uiText("Actuará sobre candidaturas de personas reales sin volver a preguntar. Puedes apagarla después en Administración › Automatizaciones."), confirmLabel: uiText("Activar la automatización") }).then((ok) => ok && createStageAutomation.mutate())} disabled={createStageAutomation.isPending || automationTitle.trim().length < 3 || automationMessage.trim().length < 3}>{createStageAutomation.isPending ? t("ats.updating") : t("ats.activateAutomation")}</Button></div>
        </DialogContent>
      </Dialog>
    </div>
  );
