...
export default function ScorecardsPage() {
  ...
  return <div className="space-y-7">
    <PageHeader eyebrow="Reclutamiento estructurado" title={t("scorecards.designer")} description={t("scorecards.description")} />
    <section className="grid gap-5 xl:grid-cols-[minmax(0,1.6fr)_minmax(300px,.7fr)]">
      <Card level={1}><CardHeader><CardTitle>{t("scorecards.newVersion")}</CardTitle></CardHeader><CardContent className="space-y-5">
        <div className="grid gap-4 sm:grid-cols-2">
          <SelectField label="Alcance" value={scope} onChange={(value) => { setScope(value as "VACANCY" | "TENANT"); if (value === "TENANT") setStageId("ALL"); }} options={[{ value: "VACANCY", label: t("scorecards.scope.vacancy") }, { value: "TENANT", label: t("scorecards.scope.shared") }]} />
          <SelectField label={t("scorecards.feedbackVisibility")} value={feedbackVisibility} onChange={(value) => setFeedbackVisibility(value as typeof feedbackVisibility)} options={[{ value: "IMMEDIATE", label: "Inmediata" }, { value: "AFTER_OWN_SUBMISSION", label: t("scorecards.visibility.afterOwn") }, { value: "AFTER_ALL_SUBMITTED", label: t("scorecards.visibility.whenAllDone") }, { value: "HIRING_MANAGER_ONLY", label: t("scorecards.visibility.managerOnly") }]} />
        ...
        <div className="space-y-4">{criteria.map((criterion, index) => <div key={criterion.localId} className="rounded-md border border-line p-4">
          <div className="mb-4 flex items-center justify-between"><p className="font-semibold">{index + 1}. {criterion.label || t("scorecards.unnamedCriterion")}</p><div className="flex"><Button size="icon" variant="ghost" aria-label="Subir criterio" onClick={() => move(criterion.localId, -1)} disabled={index === 0}><ArrowUp className="size-4" /></Button><Button size="icon" variant="ghost" aria-label="Bajar criterio" onClick={() => move(criterion.localId, 1)} disabled={index === criteria.length - 1}><ArrowDown className="size-4" /></Button><Button size="icon" variant="ghost" aria-label="Eliminar criterio" onClick={() => setCriteria((current) => current.filter((item) => item.localId !== criterion.localId))}><Trash2 className="size-4" /></Button></div></div>
      ...
      <aside><Card level={2}><CardHeader><CardTitle>Versiones publicadas</CardTitle></CardHeader><CardContent>{templates.isLoading ? <p className="text-sm text-text-secondary">Cargando versiones…</p> : templates.data?.length ? <ol className="space-y-3">{templates.data.map((template) => <li key={template.id} className="rounded-xl border border-border-default p-3"><div className="flex items-start justify-between gap-2"><div><p className="font-medium">{template.name}</p><p className="text-xs text-text-secondary">{template.scope === "TENANT" ? "Compartida" : template.stage?.name ?? t("scorecards.visibility.wholeVacancy")} · {template.criteria.length} criterios</p><p className="mt-1 text-xs text-text-secondary">Visibilidad de la retroalimentación: {technicalLabel(template.feedbackVisibility)}</p></div><span className="rounded-full bg-secondary px-2 py-1 text-xs">v{template.version}{template.isActive ? " · Activa" : ""}</span></div><div className="mt-3 flex gap-2"><Button size="sm" variant="ghost" onClick={() => duplicate.mutate(template.id)}><Copy className="size-3.5" />Duplicar</Button><Button size="sm" variant="ghost" onClick={() => templateAdmin.mutate({ id: template.id, isActive: !template.isActive })}><Power className="size-3.5" />{template.isActive ? "Desactivar" : "Activar"}</Button><Button size="sm" variant="ghost" onClick={() => { setName(template.name); setInstructions(template.instructions ?? ""); setFeedbackVisibility(template.feedbackVisibility ?? "AFTER_ALL_SUBMITTED"); setCriteria(template.criteria.map((item) => ({ ...item, localId: crypto.randomUUID(), lowAnchor: item.ratingAnchors?.["1"] ?? "", highAnchor: item.ratingAnchors?.["5"] ?? "" }))); }}>{t("scorecards.editAsNewVersion")}</Button></div></li>)}</ol> : <p className="text-sm text-text-secondary">{t("scorecards.noTemplates")}</p>}</CardContent></Card></aside>
    </section>
