...
export default function TalentCrmPage() {
  ...
  const addPool = useMutation({ mutationFn: ({ candidateId, selectedPoolId }: { candidateId: string; selectedPoolId: string }) => addCandidateToTalentPool(selectedPoolId, candidateId), onSuccess: async () => { toast.success("Listo, ya está en la lista"); await refreshCrm(); }, onError: showError });
  ...
  return <div className="space-y-6">
    <PageHeader eyebrow="Reclutamiento" title="Base de talento" description="Todas las personas que alguna vez se postularon. Guárdalas en listas, ponles etiquetas y vuelve a contactarlas cuando abras un puesto nuevo." actions={<div className="flex gap-2"><Button variant="secondary" onClick={() => setTagOpen(true)}><Tags className="size-4" />Nueva etiqueta</Button><Button onClick={() => setPoolOpen(true)}><Plus className="size-4" />Nueva lista</Button></div>} />
    ...
    {view === "crm" ? <>
      <section aria-label="Filtros de la base de talento" className="grid gap-3 rounded-2xl border border-border-default bg-surface-elevated p-4 md:grid-cols-3">
        ...
        <label className="space-y-1.5 text-sm font-medium">Lista de talento<Select value={poolId} onValueChange={(value) => { setPoolId(value); setPage(1); }}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value={ALL}>Todas las listas</SelectItem>{pools.data?.filter((item) => item.isActive).map((pool) => <SelectItem key={pool.id} value={pool.id}>{pool.name} · {pool.memberCount ?? 0}</SelectItem>)}</SelectContent></Select></label>
      ...
      {candidates.isError ? <AsyncState state="error" title="No pudimos cargar la base de talento" onRetry={() => void candidates.refetch()} /> : null}
      {candidates.isSuccess && !candidates.data.data.length ? (
        search || poolId !== ALL || tagId !== ALL
          ? <InlineFeedback tone="info" title="No encontramos a nadie con esos criterios">Prueba a quitar la lista o la etiqueta, o busca con otro nombre.</InlineFeedback>
          : <InlineFeedback tone="info" title={`Todavía no hay nadie en ${currentBranch?.name ?? "esta sucursal"}`}>Aquí aparecerán las personas que se postulen a tus puestos. También puedes ampliar la vista a toda la empresa desde el selector de sucursal.</InlineFeedback>
      ) : null}
...
function CandidateCard({ candidate, pools, tags, onAddPool, onAddTag, onActivity }: { candidate: TalentCandidateDto; pools: Awaited<ReturnType<typeof fetchTalentPools>>; tags: Awaited<ReturnType<typeof fetchTalentTags>>; onAddPool: (id: string) => void; onAddTag: (id: string) => void; onActivity: () => void }) {
  ...
  return <Card level={2}><CardContent className="space-y-4 p-5">
    ...
    <div className="grid gap-2 sm:grid-cols-2"><Select onValueChange={onAddPool}><SelectTrigger aria-label={`Agregar ${candidate.fullName} a un pool`}><SelectValue placeholder="Añadir a una lista" /></SelectTrigger><SelectContent>{availablePools.length ? availablePools.map((pool) => <SelectItem key={pool.id} value={pool.id}>{pool.name}</SelectItem>) : <SelectItem value="none" disabled>Todavía no has creado ninguna lista</SelectItem>}</SelectContent></Select><Select onValueChange={onAddTag}><SelectTrigger aria-label={`Etiquetar a ${candidate.fullName}`}><SelectValue placeholder="Agregar etiqueta" /></SelectTrigger><SelectContent>{availableTags.length ? availableTags.map((tag) => <SelectItem key={tag.id} value={tag.id}>{tag.name}</SelectItem>) : <SelectItem value="none" disabled>Sin etiquetas disponibles</SelectItem>}</SelectContent></Select></div>
...
function CreatePoolDialog({ open, onOpenChange, branchId, onCreated }: { open: boolean; onOpenChange: (open: boolean) => void; branchId?: string; onCreated: () => Promise<void> }) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent><DialogHeader><DialogTitle>Nueva lista de talento</DialogTitle><DialogDescription>Crea un segmento reutilizable para búsquedas futuras, campañas o vacantes próximas.</DialogDescription></DialogHeader><div className="space-y-4"><Input value={name} onChange={(event) => setName(event.target.value)} placeholder="Ej. Supervisores de operaciones" /><textarea className="min-h-28 w-full rounded-xl border border-border-default bg-surface-elevated p-3 text-base sm:text-sm" value={description} onChange={(event) => setDescription(event.target.value)} placeholder="Para qué te sirve esta lista. Por ejemplo: cajeros con experiencia en Orlando." /><Button className="w-full" disabled={name.trim().length < 2 || mutation.isPending} onClick={() => mutation.mutate()}>{mutation.isPending ? "Creando…" : "Crear la lista"}</Button></div></DialogContent></Dialog>;
}
