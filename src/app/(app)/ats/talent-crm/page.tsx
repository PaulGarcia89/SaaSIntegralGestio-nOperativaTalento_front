...
export default function TalentCrmPage() {
  ...
  return <div className="space-y-6">
    ...
    <div className="flex w-fit rounded-xl border border-border-default bg-surface-elevated p-1">
      ...
      <Button variant={view === "duplicates" ? "default" : "ghost"} size="sm" onClick={() => setView("duplicates")}><Merge className="size-4" />{uiText("Duplicados")}</Button>
...
function CampaignAudienceDialog({ campaign, onOpenChange, onChanged }: { campaign: TalentCampaignDto | null; onOpenChange: (open: boolean) => void; onChanged: () => Promise<void> }) {
  ...
  return <Dialog open={Boolean(campaign)} onOpenChange={onOpenChange}><DialogContent className="max-w-2xl"><DialogHeader><DialogTitle>{t("talent.audienceReview")}</DialogTitle><DialogDescription>{campaign?.name}{uiText(". Esta pantalla no envía correos.")}</DialogDescription></DialogHeader><div className="space-y-4">{!audience.data?.review.audiencePreparedAt ? <InlineFeedback tone="warning" title={t("talent.audienceNotPrepared")}>{t("talent.audienceReviewBody")}</InlineFeedback> : <><div className="grid grid-cols-2 gap-3"><div className="rounded-xl bg-primary/10 p-3"><p className="text-xs text-text-secondary">{t("talent.withConsent")}</p><p className="text-xl font-semibold">{audience.data.summary.eligible}</p></div><div className="rounded-xl bg-status-warning/10 p-3"><p className="text-xs text-text-secondary">{uiText("Excluidos")}</p><p className="text-xl font-semibold">{audience.data.summary.excluded}</p></div></div><div className="max-h-56 space-y-2 overflow-auto">{audience.data.data.map((recipient) => <div key={recipient.id} className="rounded-lg border border-border-default p-3 text-sm"><p className="font-medium">{recipient.candidate.fullName}</p><p className="text-xs text-text-secondary">{recipient.candidate.email}</p>{recipient.skipReason ? <p className="mt-1 text-xs text-status-warning">{uiText("Excluido: ")}{recipient.skipReason}</p> : <p className="mt-1 text-xs text-status-success">{t("talent.eligibleWithConsent")}</p>}</div>)}</div>{audience.data.review.audienceReviewedAt ? <InlineFeedback tone="success" title={t("talent.audienceConfirmed")}>{t("talent.reviewRecorded")}</InlineFeedback> : <Button className="w-full" disabled={!audience.data.review.audienceFingerprint || review.isPending} onClick={() => review.mutate()}><ShieldCheck className="size-4" />{uiText("Confirmar audiencia revisada")}</Button>}</>}</div><Button className="w-full" variant="secondary" disabled={prepare.isPending} onClick={() => prepare.mutate()}>{prepare.isPending ? "Calculando…" : t("talent.prepareOrRecalc")}</Button></DialogContent></Dialog>;
}
...
function CandidateCard({ candidate, pools, tags, onAddPool, onAddTag, onActivity }: { candidate: TalentCandidateDto; pools: Awaited<ReturnType<typeof fetchTalentPools>>; tags: Awaited<ReturnType<typeof fetchTalentTags>>; onAddPool: (id: string) => void; onAddTag: (id: string) => void; onActivity: () => void }) {
  ...
  return <Card level={2}><CardContent className="space-y-4 p-5">
    <div className="flex items-start justify-between gap-4"><div className="min-w-0"><h2 className="truncate text-lg font-semibold">{candidate.fullName}</h2><p className="truncate text-sm text-text-secondary">{candidate.email}</p><p className="mt-1 text-xs text-text-secondary">{candidate.phone || t("talent.noPhone")} · {candidate.city || t("talent.noCity")}</p></div>{candidate.doNotContact ? <Badge variant="destructive">{t("talent.doNotContact")}</Badge> : <Badge variant="secondary">{uiText("Disponible")}</Badge>}</div>
    ...
    <div className="grid gap-2 rounded-xl bg-surface-section p-3 text-sm sm:grid-cols-2"><div><p className="text-xs text-text-secondary">{t("talent.lastRun")}</p><p className="font-medium">{latestApplication?.vacancy.title ?? t("talent.noApplications")}</p></div><div><p className="text-xs text-text-secondary">{uiText("Actividad")}</p><p className="font-medium">{candidate.talentActivities[0]?.subject ?? t("talent.noFollowUp")}</p></div></div>
...
function DuplicateQueue({ query, onMerge }: { query: ReturnType<typeof useQuery<{ data: DuplicateCandidateMatchDto[]; scannedCandidates: number; truncated: boolean; ignoredSharedValues: number }>>; onMerge: (match: DuplicateCandidateMatchDto) => void }) {
  const uiText = useUiText();
  const { t } = useLocale();
  if (query.isLoading) return <AsyncState state="loading" title={uiText("Comparando identidades")} />;
  if (query.isError) return <AsyncState state="error" title={t("talent.duplicatesError")} onRetry={() => void query.refetch()} />;
  if (!query.data?.data.length) return <InlineFeedback tone="success" title={t("talent.noDuplicates")}>{uiText("Se analizaron ")}{query.data?.scannedCandidates ?? 0} {uiText(" perfiles activos.")}</InlineFeedback>;
  return <div className="space-y-4"><p className="text-sm text-ink-2"><strong className="font-semibold text-ink-1">{query.data.data.length} {uiText(" coincidencias para revisión.")}</strong> {uiText(" Ninguna se fusiona sola: cada una la confirma una persona.")}{query.data.ignoredSharedValues ? ` Se ignoraron ${query.data.ignoredSharedValues} valores repetidos.` : ""} <details className="inline"><summary className="inline cursor-pointer font-medium text-ink-1 [&::-webkit-details-marker]:hidden">{uiText("¿Cómo se calcula?")}</summary> {uiText(" El puntaje combina teléfono, LinkedIn, CV, nombre, correo y ciudad; los valores compartidos por más de dos perfiles se excluyen.")}</details></p>{query.data.truncated ? <InlineFeedback tone="warning" title={t("talent.partialAnalysis")}>{t("talent.partialAnalysisBody")}</InlineFeedback> : null}<div className="grid gap-4 xl:grid-cols-2">{query.data.data.map((match) => <Card key={match.id} level={2}><CardContent className="space-y-4 p-5"><div className="flex items-center justify-between"><Badge variant={match.score >= 75 ? "destructive" : "secondary"}>{match.score}{uiText("% coincidencia")}</Badge>{match.conflictingVacancyIds.length ? <span className="flex items-center gap-1 text-xs font-medium text-status-warning"><CircleAlert className="size-4" />{t("talent.vacancyConflict")}</span> : null}</div><div className="grid gap-3 sm:grid-cols-[1fr_auto_1fr]"><Identity candidate={match.source} /><ArrowRightLeft className="m-auto size-5 text-text-secondary" /><Identity candidate={match.target} /></div><div className="flex flex-wrap gap-2">{match.signals.map((signal) => <Badge key={signal} variant="outline">{signal}</Badge>)}</div><Button className="w-full" disabled={Boolean(match.conflictingVacancyIds.length)} onClick={() => onMerge(match)}><Merge className="size-4" />{t("talent.reviewAndMerge")}</Button>{match.conflictingVacancyIds.length ? <p className="text-xs text-text-secondary">{t("talent.resolveFirst")}</p> : null}</CardContent></Card>)}</div></div>;
}

function Identity({ candidate }: { candidate: DuplicateCandidateMatchDto["source"] }) {
  const uiText = useUiText();
  const { t } = useLocale();
  return <div className="min-w-0 rounded-xl bg-surface-section p-3"><p className="truncate font-semibold">{candidate.fullName}</p><p className="truncate text-xs text-text-secondary">{candidate.email}</p><p className="mt-2 text-xs">{candidate.applications} {uiText(" procesos · ")}{candidate.city || t("talent.noCity")}</p></div>; }
...
function CreatePoolDialog({ open, onOpenChange, branchId, onCreated }: { open: boolean; onOpenChange: (open: boolean) => void; branchId?: string; onCreated: () => Promise<void> }) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent><DialogHeader><DialogTitle>{t("talent.newTalentList")}</DialogTitle><DialogDescription>{t("talent.newListBody")}</DialogDescription></DialogHeader><div className="space-y-4"><Input value={name} onChange={(event) => setName(event.target.value)} placeholder={t("talent.listNamePlaceholder")} /><textarea className="field min-h-28" value={description} onChange={(event) => setDescription(event.target.value)} placeholder={t("talent.listPurposePlaceholder")} /><Button className="w-full" disabled={name.trim().length < 2 || mutation.isPending} onClick={() => mutation.mutate()}>{mutation.isPending ? t("talent.creating") : t("talent.createList")}</Button></div></DialogContent></Dialog>;
}
