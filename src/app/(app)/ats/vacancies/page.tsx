...
import { VacancyReview } from "@/components/recruitment/vacancy-review";
...
export function VacanciesPage({ createPage = false, editId }: { createPage?: boolean; editId?: string }) {
  ...
  const selectVacancyImage = (file?: File) => {
    ...
    if (!["image/jpeg", "image/png", "image/webp"].includes(file.type) || file.size > 5 * 1024 * 1024) {
      setErrors((current) => [...current.filter((item) => item.fieldId !== "vacancy-image"), { fieldId: "vacancy-image", label: uiText("Imagen del cargo"), message: t("vacancies.imageInvalid") }]);
      return;
    }
  ...
  const submit = (status: CreateVacancyInput["status"]) => { const nextErrors = (status === "PUBLISHED" || status === "OPEN" ? validateAll() : []).filter((item) => !(editingId && item.fieldId === "vacancy-image")); setErrors(nextErrors); if (nextErrors.length) setStep(VACANCY_STEPS.findIndex((entry) => entry.id === stepForVacancyError(nextErrors[0].fieldId))); if (!nextErrors.length) save.mutate({ ...form, status }); };
  ...
  const editVacancy = (vacancy: PublicVacancyDto) => {
    const vacancyStages = vacancy.stages?.length ? vacancy.stages : initialStages(locale);
    ...
    setInitialStagesForEdit(vacancy.stages ?? []); setInitialResponsiblesForEdit(vacancyResponsibles);
  ...
  return <div className="space-y-7">{isWorkspacePage ? <PageHeader eyebrow={t("vacancies.eyebrow")} title={editId ? t("vacancies.edit") : t("vacancies.create")} description={editId ? t("vacancies.editDescription") : t("vacancies.createDescription")} actions={<Button variant="secondary" asChild><Link href="/ats/vacancies">{t("vacancies.back")}</Link></Button>} /> : null}{!isWorkspacePage ? <><PageHeader eyebrow={uiText("Reclutamiento")} title={t("vacancies.list")} description={t("vacancies.listDescription")} actions={can("jobs.create") ? <Button asChild><Link href="/ats/vacancies/new"><Plus className="size-4" />{t("vacancies.new")}</Link></Button> : undefined} />
    ...
    <VacancyList items={items} canCreate={can("jobs.create")} cloning={clone.isPending} restoring={restore.isPending} onClone={(id) => clone.mutate(id)} onRestore={(vacancy) => restore.mutate(vacancy)} onArchive={setArchiveTarget} onHistory={setHistoryId} /></> : null}
      ...
      */}
    {isWorkspacePage || open ? <VacancyWizardSurface isWorkspacePage={isWorkspacePage} open={wizardOpen} onOpenChange={setOpen}><Wizard steps={localizedSteps} current={step} onStepChange={(target) => { if (target <= step) { setErrors([]); setStep(target); } }}><div className="space-y-5 text-sm leading-relaxed"><p className="mb-5 text-text-secondary">{VACANCY_STEPS[step] ? vacancyStepHelp(VACANCY_STEPS[step].id, locale) : ""}</p><FormErrorSummary subject="la vacante" errors={errors} serverError={save.error ?? replaceImage.error} />{step === 0 ? <div className="grid items-start gap-6 xl:grid-cols-[minmax(0,1.55fr)_minmax(0,1fr)]"><div className="min-w-0 space-y-5"><div className="grid gap-4 md:grid-cols-2"><label className="space-y-2 text-sm font-medium" htmlFor="vacancy-branch">{t("vacancies.branch")}<Select value={form.branchId} onValueChange={(value) => update("branchId", value)}><SelectTrigger id="vacancy-branch"><SelectValue placeholder={t("vacancies.selectBranch")} /></SelectTrigger><SelectContent>{branches.map((branch) => <SelectItem key={branch.id} value={branch.id}>{branch.name}</SelectItem>)}</SelectContent></Select></label><Field id="vacancy-title" label={t("vacancies.title")} value={form.title} onChange={(value) => update("title", value)} /><Field id="vacancy-openings" label={t("vacancies.openings")} type="number" value={String(form.openings ?? 1)} onChange={(value) => update("openings", Math.max(1, Number(value)))} /></div><SimpleSection title={t("vacancies.moreDetails")} hint={t("vacancies.moreDetailsHint")}><div className="grid gap-4 md:grid-cols-2"><Field id="vacancy-department" label={t("vacancies.department")} value={form.department ?? ""} onChange={(value) => update("department", value)} /><Field id="vacancy-seniority" label={t("vacancies.seniority")} value={form.seniority ?? ""} onChange={(value) => update("seniority", value)} /><Field id="vacancy-city" label={t("vacancies.city")} value={form.city ?? ""} onChange={(value) => update("city", value)} /><Field id="vacancy-country" label={t("vacancies.country")} value={form.country ?? ""} onChange={(value) => update("country", value)} /><Field id="vacancy-summary" label={t("vacancies.summary")} value={form.summary ?? ""} maxLength={240} onChange={(value) => update("summary", value)} className="md:col-span-2" /></div></SimpleSection><SimpleSection title={t("vacancies.multiBranch")} hint={t("vacancies.multiBranchHint")}><MultiLocationEditor branches={branches} primaryBranchId={form.branchId} selected={form.locationBranchIds ?? [form.branchId]} onChange={(ids) => update("locationBranchIds", ids)} /></SimpleSection></div><aside className="min-w-0 rounded-xl border border-border-default bg-surface-section/40 p-4 sm:p-5"><div className="grid gap-4"><div id="vacancy-image" className="space-y-4"><div><p className="text-sm font-medium">{t("vacancies.image")} <span className="text-status-danger">*</span></p><p className="mt-1 text-xs text-text-secondary">{t("vacancies.imageHelp")}</p></div>{imagePreview ? <><div className="relative h-52 w-full overflow-hidden rounded-xl border border-border-default sm:h-64 xl:h-72"><Image src={imagePreview} alt={t("vacancies.imagePreviewAlt", { title: form.title || t("jobs.job") })} fill unoptimized className="object-cover" /><div className="absolute right-3 top-3 flex items-center gap-2"><Button type="button" size="sm" variant="secondary" onClick={() => imageInputRef.current?.click()} disabled={replaceImage.isPending}><Upload className="size-4" />{replaceImage.isPending ? t("vacancies.uploadingImage") : t("vacancies.replaceImage")}</Button>{imageFile && !replaceImage.isPending ? <Button type="button" size="icon" variant="destructive" aria-label={t("vacancies.discardNewImage")} onClick={discardSelectedImage}><Trash2 className="size-4" /></Button> : null}</div>{imageFile ? <p className="absolute bottom-3 left-3 rounded-lg bg-surface-1 px-3 py-2 text-xs font-medium shadow-sm">{editingId ? t("vacancies.imageSavingNow") : t("vacancies.imageSaveOnSubmit")}</p> : null}{imageUploadMessage ? <p className="absolute bottom-3 left-3 rounded-lg bg-surface-1 px-3 py-2 text-xs font-medium text-status-success shadow-sm">{imageUploadMessage}</p> : null}</div><input ref={imageInputRef} type="file" accept="image/jpeg,image/png,image/webp" className="hidden" disabled={replaceImage.isPending} onChange={(event) => { selectVacancyImage(event.target.files?.[0]); event.target.value = ""; }} /></> : <FileUpload accept="image/jpeg,image/png,image/webp" maxFiles={1} maxSizeBytes={5 * 1024 * 1024} onFiles={(selected) => selectVacancyImage(selected[0])} />}</div></div></aside></div> : null}
      {step === 1 ? <div className="grid items-start gap-6 xl:grid-cols-2"><div className="min-w-0 space-y-5"><div className="grid gap-4"><TextArea label={t("vacancies.description")} value={form.description ?? ""} onChange={(value) => update("description", value)} /></div><SimpleSection title={t("vacancies.dutiesAndRequirements")} hint={t("vacancies.dutiesHint")}><div className="grid gap-4"><TextArea label={t("vacancies.responsibilities")} value={form.responsibilities ?? ""} onChange={(value) => update("responsibilities", value)} /><TextArea label={t("vacancies.requirements")} value={form.requirements ?? ""} onChange={(value) => update("requirements", value)} /></div></SimpleSection></div><div className="grid min-w-0 gap-4 rounded-xl border border-border-default bg-surface-section/40 p-4 md:grid-cols-2 sm:p-5"><label className="space-y-2 text-sm font-medium">{t("vacancies.workMode")}<Select value={form.workMode} onValueChange={(value) => update("workMode", value as CreateVacancyInput["workMode"])}><SelectTrigger><SelectValue placeholder={t("vacancies.workMode")} /></SelectTrigger><SelectContent><SelectItem value="REMOTE">{t("vacancies.workMode.REMOTE")}</SelectItem><SelectItem value="HYBRID">{t("vacancies.workMode.HYBRID")}</SelectItem><SelectItem value="ONSITE">{t("vacancies.workMode.ONSITE")}</SelectItem></SelectContent></Select></label><label className="space-y-2 text-sm font-medium">{t("vacancies.employmentType")}<Select value={form.employmentType} onValueChange={(value) => update("employmentType", value as CreateVacancyInput["employmentType"])}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="FULL_TIME">{t("vacancies.employmentType.FULL_TIME")}</SelectItem><SelectItem value="PART_TIME">{t("vacancies.employmentType.PART_TIME")}</SelectItem><SelectItem value="CONTRACT">{t("vacancies.employmentType.CONTRACT")}</SelectItem><SelectItem value="TEMPORARY">{t("vacancies.employmentType.TEMPORARY")}</SelectItem><SelectItem value="INTERNSHIP">{t("vacancies.employmentType.INTERNSHIP")}</SelectItem></SelectContent></Select></label><Field id="vacancy-currency" label={t("vacancies.currency")} value={form.currency ?? "USD"} maxLength={8} onChange={(value) => update("currency", value.toUpperCase())} /><Field id="vacancy-salary-min" label={t("vacancies.salaryMin")} type="number" value={form.salaryMin == null ? "" : String(form.salaryMin)} onChange={(value) => update("salaryMin", value ? Number(value) : undefined)} /><Field id="vacancy-salary-max" label={t("vacancies.salaryMax")} type="number" value={form.salaryMax == null ? "" : String(form.salaryMax)} onChange={(value) => update("salaryMax", value ? Number(value) : undefined)} /><Field id="vacancy-benefits" label={t("vacancies.benefits")} value={form.benefits ?? ""} onChange={(value) => update("benefits", value)} className="md:col-span-2" /></div></div> : null}
      {step === 2 ? <VacancyReview form={form} imagePreview={imagePreview} branchName={branches.find((branch) => branch.id === form.branchId)?.name} stages={stages} onEdit={setStep} /> : null}
...
function VacancyWizardSurface({ children, isWorkspacePage, onOpenChange, open }: { children: ReactNode; isWorkspacePage: boolean; onOpenChange: (open: boolean) => void; open: boolean }) {
  const { t } = useLocale();
  if (isWorkspacePage) {
    return <Card level={1} className="w-full"><CardContent className="space-y-6 p-4 sm:p-6">{children}</CardContent></Card>;
  }
