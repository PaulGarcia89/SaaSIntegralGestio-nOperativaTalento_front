...
export function VacanciesPage({ createPage = false, editId }: { createPage?: boolean; editId?: string }) {
  ...
  return <div className="space-y-7">{isWorkspacePage ? <PageHeader eyebrow="Reclutamiento / Vacantes" title={editId ? "Editar vacante" : "Crear vacante"} description={editId ? "Actualiza contenido, condiciones, ubicaciones, pipeline y responsables." : "Completa la información, revisa el proceso y guarda un borrador o publica cuando esté listo."} actions={<Button variant="secondary" asChild><Link href="/ats/vacancies">Volver a vacantes</Link></Button>} /> : null}{!isWorkspacePage ? <><PageHeader eyebrow="Reclutamiento" title="Vacantes" description="Crea borradores, revisa el contenido y publica únicamente después de validar el resumen." actions={can("jobs.create") ? <Button asChild><Link href="/ats/vacancies/new"><Plus className="size-4" />Nueva vacante</Link></Button> : undefined} />
    ...
    {isWorkspacePage || open ? <VacancyWizardSurface isWorkspacePage={isWorkspacePage} open={wizardOpen} onOpenChange={setOpen}><Wizard steps={steps} current={step} onStepChange={(target) => { if (target <= step) { setErrors([]); setStep(target); } }}><FormErrorSummary errors={errors} serverError={save.error ?? replaceImage.error} />{step === 0 ? <MultiLocationEditor branches={branches} primaryBranchId={form.branchId} selected={form.locationBranchIds ?? [form.branchId]} onChange={(ids) => update("locationBranchIds", ids)} /> : null}
