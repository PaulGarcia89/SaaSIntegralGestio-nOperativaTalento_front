...
export function VacanciesPage({ createPage = false, editId }: { createPage?: boolean; editId?: string }) {
  ...
  const { t } = useLocale();
  const localizedSteps = [t("vacancies.information"), t("vacancies.content"), t("vacancies.conditions"), t("vacancies.form"), t("vacancies.process"), t("vacancies.responsibles"), t("vacancies.review")];
  ...
  return <div className="space-y-7">{isWorkspacePage ? <PageHeader eyebrow="Reclutamiento / Vacantes" title={editId ? t("vacancies.edit") : t("vacancies.create")} description={editId ? "Actualiza contenido, condiciones, ubicaciones, pipeline y responsables." : "Completa la información, revisa el proceso y guarda un borrador o publica cuando esté listo."} actions={<Button variant="secondary" asChild><Link href="/ats/vacancies">{t("vacancies.back")}</Link></Button>} /> : null}{!isWorkspacePage ? <><PageHeader eyebrow="Reclutamiento" title={t("vacancies.list")} description="Crea borradores, revisa el contenido y publica únicamente después de validar el resumen." actions={can("jobs.create") ? <Button asChild><Link href="/ats/vacancies/new"><Plus className="size-4" />{t("vacancies.new")}</Link></Button> : undefined} />
    ...
    {isWorkspacePage || open ? <VacancyWizardSurface isWorkspacePage={isWorkspacePage} open={wizardOpen} onOpenChange={setOpen}><Wizard steps={localizedSteps} current={step} onStepChange={(target) => { if (target <= step) { setErrors([]); setStep(target); } }}><FormErrorSummary errors={errors} serverError={save.error ?? replaceImage.error} />{step === 0 ? <MultiLocationEditor branches={branches} primaryBranchId={form.branchId} selected={form.locationBranchIds ?? [form.branchId]} onChange={(ids) => update("locationBranchIds", ids)} /> : null}
    ...
    </Wizard></VacancyWizardSurface> : null}
