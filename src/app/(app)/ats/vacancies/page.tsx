  ...
  return <div className="space-y-4">
    ...
    {stages.map((stage, index) => <Card level={3} key={`${stage.code}-${index}`}><CardContent className="space-y-5 p-4">
      <div className="grid gap-3 lg:grid-cols-[80px_1fr_1fr_200px_auto]">
        <Field id={`stage-position-${index}`} label={uiText("Orden")} type="number" value={String(stage.position + 1)} onChange={(value) => updateStage(index, { position: Math.max(0, Number(value) - 1) })} />
      ...
      <div className="grid items-end gap-4 border-t border-border-default pt-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        {stage.applicationStatus === "REJECTED" ? <label className="flex min-h-11 items-center gap-2 text-sm font-medium"><input type="checkbox" checked={Boolean(stage.allowReopen)} onChange={(event) => updateStage(index, { allowReopen: event.target.checked })} />{uiText("Permitir reapertura")}</label> : null}
      </div>
    </CardContent></Card>)}
  </div>;
}
