...
import { VACANCY_STEPS, stepForVacancyError } from "@/lib/vacancy-wizard";
...
const steps = VACANCY_STEPS.map((step) => step.title);
...
export function VacanciesPage({ createPage = false, editId }: { createPage?: boolean; editId?: string }) {
  ...
  return <div className="space-y-7">{isWorkspacePage ? <PageHeader eyebrow="Reclutamiento / Vacantes" title={editId ? t("vacancies.edit") : t("vacancies.create")} description={editId ? "Actualiza contenido, condiciones, ubicaciones, pipeline y responsables." : "Completa la información, revisa el proceso y guarda un borrador o publica cuando esté listo."} actions={<Button variant="secondary" asChild><Link href="/ats/vacancies">{t("vacancies.back")}</Link></Button>} /> : null}{!isWorkspacePage ? <><PageHeader eyebrow="Reclutamiento" title={t("vacancies.list")} description="Crea borradores, revisa el contenido y publica únicamente después de validar el resumen." actions={can("jobs.create") ? <Button asChild><Link href="/ats/vacancies/new"><Plus className="size-4" />{t("vacancies.new")}</Link></Button> : undefined} />
    ...
    <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{items.map((vacancy) => <Card level={2} key={vacancy.id} className="overflow-hidden"><VacancyImage imageUrl={vacancy.imageUrl} title={vacancy.title} /><CardHeader><div className="flex items-start justify-between gap-3"><CardTitle>{vacancy.title}</CardTitle><Badge variant="secondary">{vacancy.status ?? "PAUSED"}</Badge></div></CardHeader><CardContent className="space-y-3 text-sm text-text-secondary"><p>{vacancy.department || "Sin área"} · {vacancy.workMode ?? "Sin modalidad"}</p><p>{vacancy.locations?.map((item) => item.branch.name).join(", ") || [vacancy.city, vacancy.country].filter(Boolean).join(", ") || "Sin ubicación"}</p>{vacancy.requisition ? <p className="rounded-lg bg-surface-section px-3 py-2"><ClipboardCheck className="mr-2 inline size-4" />{vacancy.requisition.title}</p> : null}<div className="flex flex-wrap gap-2 pt-2">{vacancy.status === "ARCHIVED" ? <Button size="sm" variant="secondary" onClick={() => restore.mutate(vacancy)} disabled={restore.isPending}><RotateCcw className="size-4" />Restaurar</Button> : <><Button size="sm" variant="secondary" asChild><Link href={`/ats/vacancies/${vacancy.id}/edit`}><Pencil className="size-4" />Editar</Link></Button><Button size="sm" variant="secondary" onClick={() => clone.mutate(vacancy.id)} disabled={clone.isPending}><Copy className="size-4" />Clonar</Button><Button size="sm" variant="ghost" onClick={() => setArchiveTarget(vacancy)}><Archive className="size-4" />Archivar</Button></>}<Button size="sm" variant="ghost" onClick={() => setHistoryId(vacancy.id)}><History className="size-4" />Historial</Button></div></CardContent></Card>)}</section></> : null}
      ...
      */}
    {isWorkspacePage || open ? <VacancyWizardSurface isWorkspacePage={isWorkspacePage} open={wizardOpen} onOpenChange={setOpen}><Wizard steps={localizedSteps} current={step} onStepChange={(target) => { if (target <= step) { setErrors([]); setStep(target); } }}><div className={SIMPLE_TEXT}><p className="mb-5 text-text-secondary">{VACANCY_STEPS[step]?.help}</p><FormErrorSummary errors={errors} serverError={save.error ?? replaceImage.error} />{step === 0 ? <><div className="grid gap-4 md:grid-cols-2"><label className="space-y-2 text-sm font-medium" htmlFor="vacancy-branch">Sucursal<Select value={form.branchId} onValueChange={(value) => update("branchId", value)}><SelectTrigger id="vacancy-branch"><SelectValue placeholder="Selecciona una sucursal" /></SelectTrigger><SelectContent>{branches.map((branch) => <SelectItem key={branch.id} value={branch.id}>{branch.name}</SelectItem>)}</SelectContent></Select></label><Field id="vacancy-title" label="Título" value={form.title} onChange={(value) => update("title", value)} /><Field id="vacancy-openings" label="Número de plazas" type="number" value={String(form.openings ?? 1)} onChange={(value) => update("openings", Math.max(1, Number(value)))} /></div><SimpleSection title="Más detalles del puesto" hint="Área, nivel de experiencia, ciudad y resumen"><div className="grid gap-4 md:grid-cols-2"><Field id="vacancy-department" label="Área" value={form.department ?? ""} onChange={(value) => update("department", value)} /><Field id="vacancy-seniority" label="Nivel de experiencia" value={form.seniority ?? ""} onChange={(value) => update("seniority", value)} /><Field id="vacancy-city" label="Ciudad" value={form.city ?? ""} onChange={(value) => update("city", value)} /><Field id="vacancy-country" label="País" value={form.country ?? ""} onChange={(value) => update("country", value)} /><Field id="vacancy-summary" label="Resumen" value={form.summary ?? ""} maxLength={240} onChange={(value) => update("summary", value)} className="md:col-span-2" /></div></SimpleSection><div className="grid gap-4"><div id="vacancy-image" className="space-y-3 md:col-span-2"><div><p className="text-sm font-medium">Imagen del cargo <span className="text-status-danger">*</span></p><p className="mt-1 text-xs text-text-secondary">JPG, PNG o WebP de hasta 5 MB. Se escaneará y almacenará de forma privada.</p></div>{imagePreview ? <><div className="relative aspect-[16/7] overflow-hidden rounded-2xl border border-border-default"><Image src={imagePreview} alt={`Vista previa para ${form.title || "la vacante"}`} fill unoptimized className="object-cover" /><div className="absolute right-3 top-3 flex items-center gap-2"><Button type="button" size="sm" variant="secondary" onClick={() => imageInputRef.current?.click()} disabled={replaceImage.isPending}><Upload className="size-4" />{replaceImage.isPending ? "Subiendo imagen..." : "Reemplazar imagen"}</Button>{imageFile && !replaceImage.isPending ? <Button type="button" size="icon" variant="destructive" aria-label="Descartar la imagen nueva" onClick={discardSelectedImage}><Trash2 className="size-4" /></Button> : null}</div>{imageFile ? <p className="absolute bottom-3 left-3 rounded-lg bg-surface-base/90 px-3 py-2 text-xs font-medium shadow-sm">{editingId ? "Nueva imagen seleccionada. Se está guardando ahora." : "Nueva imagen seleccionada. Se actualizará al guardar."}</p> : null}{imageUploadMessage ? <p className="absolute bottom-3 left-3 rounded-lg bg-surface-base/90 px-3 py-2 text-xs font-medium text-status-success shadow-sm">{imageUploadMessage}</p> : null}</div><input ref={imageInputRef} type="file" accept="image/jpeg,image/png,image/webp" className="hidden" disabled={replaceImage.isPending} onChange={(event) => { selectVacancyImage(event.target.files?.[0]); event.target.value = ""; }} /></> : <FileUpload accept="image/jpeg,image/png,image/webp" maxFiles={1} maxSizeBytes={5 * 1024 * 1024} onFiles={(selected) => selectVacancyImage(selected[0])} />}</div></div></> : null}{step === 0 ? <SimpleSection title="¿El puesto trabaja en más de una sucursal?" hint="Por omisión, solo en la que elegiste arriba"><MultiLocationEditor branches={branches} primaryBranchId={form.branchId} selected={form.locationBranchIds ?? [form.branchId]} onChange={(ids) => update("locationBranchIds", ids)} /></SimpleSection> : null}
      {step === 1 ? <><div className="grid gap-4"><TextArea label="Descripción" value={form.description ?? ""} onChange={(value) => update("description", value)} /></div><SimpleSection title="Responsabilidades y requisitos" hint="Opcional: ayuda a que se postule la persona adecuada"><div className="grid gap-4"><TextArea label="Responsabilidades" value={form.responsibilities ?? ""} onChange={(value) => update("responsibilities", value)} /><TextArea label="Requisitos" value={form.requirements ?? ""} onChange={(value) => update("requirements", value)} /></div></SimpleSection></> : null}
      {step === 1 ? <div className="grid gap-4 md:grid-cols-2"><label className="space-y-2 text-sm font-medium">Modalidad<Select value={technicalLabel(form.workMode)} onValueChange={(value) => update("workMode", value as CreateVacancyInput["workMode"])}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="REMOTE">Remoto</SelectItem><SelectItem value="HYBRID">Híbrido</SelectItem><SelectItem value="ONSITE">Presencial</SelectItem></SelectContent></Select></label><label className="space-y-2 text-sm font-medium">Tipo de empleo<Select value={form.employmentType} onValueChange={(value) => update("employmentType", value as CreateVacancyInput["employmentType"])}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="FULL_TIME">Tiempo completo</SelectItem><SelectItem value="PART_TIME">Tiempo parcial</SelectItem><SelectItem value="CONTRACT">Contrato</SelectItem><SelectItem value="TEMPORARY">Temporal</SelectItem><SelectItem value="INTERNSHIP">Prácticas</SelectItem></SelectContent></Select></label><Field id="vacancy-currency" label="Moneda" value={form.currency ?? "USD"} maxLength={8} onChange={(value) => update("currency", value.toUpperCase())} /><Field id="vacancy-salary-min" label="Salario mínimo" type="number" value={form.salaryMin == null ? "" : String(form.salaryMin)} onChange={(value) => update("salaryMin", value ? Number(value) : undefined)} /><Field id="vacancy-salary-max" label="Salario máximo" type="number" value={form.salaryMax == null ? "" : String(form.salaryMax)} onChange={(value) => update("salaryMax", value ? Number(value) : undefined)} /><Field id="vacancy-benefits" label="Beneficios" value={form.benefits ?? ""} onChange={(value) => update("benefits", value)} className="md:col-span-2" /></div> : null}
