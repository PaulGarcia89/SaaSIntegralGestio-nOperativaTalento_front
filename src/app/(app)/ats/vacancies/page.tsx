...
export function VacanciesPage({ editId }: { editId?: string }) {
  ...
  const openWizard = () => { setEditingId(null); setForm(initial(currentBranch?.id ?? "")); setStages(initialStages()); setResponsibles([]); setInitialStagesForEdit([]); setInitialResponsiblesForEdit([]); setInitialFormForEdit(null); setImageFile(null); setImagePreview(""); setStoredImagePreview(""); setErrors([]); setDraftReady(false); setOpen(true); };
  const editVacancy = (vacancy: PublicVacancyDto) => {
    ...
    setImageFile(null); setImagePreview(vacancy.imageUrl ?? ""); setStoredImagePreview(vacancy.imageUrl ?? ""); setErrors([]); setStep(0); setOpen(true);
  };
