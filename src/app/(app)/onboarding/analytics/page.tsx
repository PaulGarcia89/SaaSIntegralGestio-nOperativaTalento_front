...
function TableCard({ title, icon: Icon, rows }: { title: string; icon: typeof Clock3; rows: Array<{ label: string; averageHours: number; sampleSize: number }> }) { return <Card><CardHeader><CardTitle className="flex items-center gap-2"><Icon className="size-5 text-brand" />{title}</CardTitle></CardHeader><CardContent className="space-y-2">{rows.slice(0, 8).map((row) => <div key={row.label} className="grid grid-cols-[1fr_auto_auto] gap-3 rounded-lg bg-muted/40 p-3 text-sm"><span className="truncate font-medium">{row.label}</span><span>{hours(row.averageHours)}</span><span className="text-muted-foreground">{row.sampleSize} casos</span></div>)}{!rows.length ? <p className="text-sm text-muted-foreground">Aún no hay tareas completadas suficientes.</p> : null}</CardContent></Card>; }
function Comparison({ title, rows }: { title: string; rows: Array<{ label: string; total: number; completionRate: number; averageCompletionHours: number }> }) { return <Card><CardHeader><CardTitle className="flex items-center gap-2"><BarChart3 className="size-5 text-brand" />{title}</CardTitle></CardHeader><CardContent className="space-y-2">{rows.slice(0, 8).map((row) => <div key={row.label} className="rounded-lg border p-3"><div className="flex justify-between gap-3"><span className="truncate font-medium">{row.label}</span><span>{row.completionRate}%</span></div><p className="mt-1 text-xs text-muted-foreground">{row.total} expedientes · cierre en {hours(row.averageCompletionHours)}</p></div>)}{!rows.length ? <p className="text-sm text-muted-foreground">Sin datos para comparar.</p> : null}</CardContent></Card>; }
