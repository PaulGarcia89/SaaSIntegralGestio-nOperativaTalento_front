...
function Metric({ icon: Icon, label, value }: { icon: typeof ShieldCheck; label: string; value: string }) { return <Card level={2}><CardContent className="flex items-center gap-3 p-4"><Icon className="size-5 text-brand" /><div><p className="text-xs text-text-secondary">{label}</p><p className="font-semibold">{value}</p></div></CardContent></Card>; }
