...
export default function ElectronicSignaturesPage() {
  ...
  return <div className="space-y-7">
    <PageHeader
      ...
      actions={canManage ? <div className="flex flex-wrap gap-2"><Button variant="secondary" onClick={() => setTemplateOpen(true)}><Plus className="size-4" />Plantilla</Button><Button onClick={() => setPackageOpen(true)}><FileSignature className="size-4" />Nuevo paquete</Button></div> : undefined}
    />
    ...
    {packages.isSuccess ? (
      <section aria-labelledby="signature-list-title" className="space-y-4">
        ...
        {!all.length ? (
          <EmptyState
            reason="no-records"
            title="No hay paquetes de firma"
            description={canManage ? "Crea una plantilla y genera el primer paquete desde una incorporación activa." : undefined}
            action={canManage ? <Button onClick={() => setPackageOpen(true)}><FileSignature className="size-4" />Nuevo paquete</Button> : undefined}
          />
        ) : !visible.length ? (
    ...
    <li className={cn("flex flex-col gap-4 rounded-lg border bg-surface-1 p-4 sm:p-5", overdue ? "border-status-warning/50" : "border-line")}>
      ...
      <div className="flex flex-wrap items-center gap-3 border-t border-line pt-4">
        {actionable ? (item.sentAt
          ? <Button variant="secondary" onClick={onRemind} disabled={sending}><BellRing className="size-4" />Recordar</Button>
          : <Button onClick={onSend} disabled={sending || !item.template}><Send className="size-4" />Enviar a firma</Button>) : null}
        {due ? (
