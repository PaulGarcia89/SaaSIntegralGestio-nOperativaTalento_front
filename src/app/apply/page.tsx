  ...
  return (
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 pb-14 pt-2">
      ...
      <Card><CardHeader><CardTitle>{steps[step]}</CardTitle></CardHeader><CardContent className="space-y-6">
        ...
        {step === 1 ? <CandidateAuthCard returnPath={`/apply?${params.toString()}`} portalLabel={portal?.company?.name ? `el portal de ${portal.company.name}` : "este portal"} onAuthenticated={() => { setAuthenticated(true); setError(""); setStep(2); }} /> : null}
