  ...
  return (
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 pb-14 pt-2">
      ...
      <Card><CardHeader><CardTitle>{steps[step]}</CardTitle></CardHeader><CardContent className="space-y-6">
        ...
        {step === 3 ? <><DynamicQuestions fields={getApplicationFields(vacancy.applicationFormSchema).filter((field) => !builtInQuestionKeys.has(field.key))} responses={form.dynamicResponses ?? {}} onChange={setResponse} /><EmploymentAndReferenceQuestions responses={form.dynamicResponses ?? {}} onChange={setResponse} /></> : null}
...
function EmploymentAndReferenceQuestions({ responses, onChange }: { responses: Record<string, unknown>; onChange: (key: string, value: unknown) => void }) {
  ...
  return <div className="mt-8 space-y-8">
    <section className="space-y-4 rounded-2xl border bg-muted/20 p-5"><div><h2 className="text-lg font-semibold">1. Elegibilidad</h2><p className="mt-1 text-sm text-muted-foreground">Responde cada pregunta. Si eliges “Sí”, aparecerá inmediatamente el campo para explicar tu respuesta.</p></div><div className="space-y-4"><BinaryField label="¿Tienes 18 años o más?" value={responses.is18OrOlder} onChange={(next) => onChange("is18OrOlder", next)} /><BinaryField label="¿Estás autorizado para trabajar en Estados Unidos?" value={responses.authorizedToWorkInUS} onChange={(next) => onChange("authorizedToWorkInUS", next)} /><BinaryField label="¿Has trabajado antes para esta empresa?" value={responses.workedForCompany} onChange={(next) => onChange("workedForCompany", next)} />{responses.workedForCompany === true ? <Field label="Explica si has trabajado para esta empresa" value={value("workedForCompanyExplanation")} onChange={(next) => onChange("workedForCompanyExplanation", next)} /> : null}<BinaryField label="¿Algún familiar trabaja para esta empresa?" value={responses.familyWorksForCompany} onChange={(next) => onChange("familyWorksForCompany", next)} />{responses.familyWorksForCompany === true ? <Field label="Explica si un familiar trabaja para esta empresa" value={value("familyWorksForCompanyExplanation")} onChange={(next) => onChange("familyWorksForCompanyExplanation", next)} /> : null}<BinaryField label="¿Has sido condenado por un delito grave?" value={responses.felonyConviction} onChange={(next) => onChange("felonyConviction", next)} />{responses.felonyConviction === true ? <Field label="Explica la condena" value={value("felonyConvictionExplanation")} onChange={(next) => onChange("felonyConvictionExplanation", next)} /> : null}</div></section>
