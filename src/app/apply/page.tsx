...
function ApplyWizard() {
  ...
  const localizedSteps = [t("application.access"), t("application.job"), t("application.profile"), t("application.questions"), t("application.resume"), t("application.submitStep"), t("application.confirmation")];
  ...
  return (
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 pb-14 pt-2">
      ...
      <Card><CardHeader><CardTitle>{localizedSteps[step]}</CardTitle></CardHeader><CardContent className="space-y-6">
        ...
        {step === 5 ? <div className="space-y-5"><input tabIndex={-1} aria-hidden="true" autoComplete="off" name="website" value={website} onChange={(event) => setWebsite(event.target.value)} className="absolute -left-[10000px] h-px w-px opacity-0" /><dl className="grid gap-3 rounded-xl bg-secondary/40 p-5 text-sm"><div><dt className="text-muted-foreground">Nombre</dt><dd className="font-medium">{form.fullName}</dd></div><div><dt className="text-muted-foreground">Correo</dt><dd className="font-medium">{form.email}</dd></div><div><dt className="text-muted-foreground">Vacante</dt><dd className="font-medium">{vacancy.title}</dd></div>{resumeFile ? <div><dt className="text-muted-foreground">CV privado</dt><dd className="font-medium">{resumeFile.name}</dd></div> : null}</dl><div className="rounded-xl border bg-muted/20 p-5 text-sm leading-7" aria-labelledby="application-disclaimer-title"><h2 id="application-disclaimer-title" className="font-semibold">Declaración del postulante</h2><p className="mt-3">Autorizo a Cuban Guys / Qban Guys LLC y a las personas o empresas designadas a investigar todas las declaraciones incluidas en esta solicitud. También autorizo a mis empleadores anteriores y a cualquier otra persona, empresa o corporación a proporcionar la información solicitada sobre mi desempeño laboral, calificaciones, antecedentes y aptitud para el puesto.</p><p className="mt-3">Autorizo expresamente la entrega de información relacionada con mi desempeño y aptitud para el empleo, y libero a Cuban Guys / Qban Guys LLC, a mis empleadores anteriores y a las personas o empresas consultadas de toda responsabilidad por proporcionar dicha información.</p><p className="mt-3">Entiendo que cualquier declaración falsa u omisión de información puede causar el rechazo inmediato de mi solicitud o mi despido si llegara a ser contratado. También entiendo que esta solicitud no constituye un contrato de empleo, que el empleo puede terminarse en cualquier momento y que completar esta solicitud no garantiza que exista un puesto disponible ni obliga a Cuban Guys / Qban Guys LLC a ofrecerme un puesto.</p></div><label className="flex items-start gap-3"><input type="checkbox" checked={consent} onChange={(event) => setConsent(event.target.checked)} className="mt-1 size-4" /><span className="text-sm leading-6">Confirmo que la información es correcta y autorizo su tratamiento para este proceso.</span></label></div> : null}
        ...
        {step === 6 && receipt ? <div className="rounded-xl border border-primary/20 bg-primary/5 p-4 text-left"><p className="font-medium">¿Qué sigue?</p><p className="mt-1 text-sm text-muted-foreground">Consulta el estado de esta postulación y revisa aquí las próximas entrevistas o mensajes de {vacancy.tenant?.name ?? "la empresa"}.</p></div> : null}
        ...
        {step < 6 ? <div className="flex flex-col-reverse gap-3 border-t pt-5 sm:flex-row sm:items-center sm:justify-between"><div className="flex gap-2"><Button type="button" variant="secondary" onClick={back} disabled={step === 0}><ChevronLeft className="size-4" />{t("application.previous")}</Button>{step > 0 ? <Button type="button" variant="ghost" onClick={pause}>{t("application.saveExit")}</Button> : null}</div>{step < 5 ? <Button type="button" onClick={next}>{t("application.next")}<ChevronRight className="size-4" /></Button> : <Button type="button" onClick={submit} disabled={!consent || submitMutation.isPending}>{submitMutation.isPending ? t("application.sending") : t("application.send")}<Send className="size-4" /></Button>}</div> : null}
...
function ApplicationProgressSummary({ currentStep, steps, draftLoaded }: { currentStep: number; steps: string[]; draftLoaded: boolean }) {
  const progress = Math.round((currentStep / Math.max(1, steps.length - 1)) * 100);
  return <section aria-label="Progreso de la postulación" className="rounded-2xl border border-border-default bg-surface-elevated p-4"><div className="flex flex-wrap items-center justify-between gap-2 text-sm"><p className="font-medium">Paso {Math.min(currentStep + 1, steps.length)} de {steps.length}: {steps[currentStep]}</p><p className="text-text-secondary">{progress}% completado</p></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-surface-section"><div className="h-full rounded-full bg-primary transition-[width]" style={{ width: `${progress}%` }} /></div><div className="mt-3 flex flex-wrap items-center justify-between gap-2 text-xs text-text-secondary"><span>Los campos obligatorios se validan antes de continuar.</span>{draftLoaded ? <span role="status" className="font-medium text-status-success">Progreso guardado</span> : <span>Guardado automático activo</span>}</div></section>;
}
