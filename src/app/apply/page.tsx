...
function EmploymentAndReferenceQuestions({ responses, onChange }: { responses: Record<string, unknown>; onChange: (key: string, value: unknown) => void }) {
  ...
  return <div className="mt-8 space-y-8">
    ...
    <section className="space-y-5 rounded-2xl border bg-muted/10 p-5" aria-labelledby="candidate-declaration-title"><div className="space-y-3 leading-7"><h2 id="candidate-declaration-title" className="text-lg font-semibold">Declaración del postulante</h2><p>Autorizo a Cuban Guys / Qban Guys LLC y a las personas o empresas designadas a investigar todas las declaraciones incluidas en esta solicitud. También autorizo a mis empleadores anteriores y a cualquier otra persona, empresa o corporación a proporcionar la información solicitada sobre mi desempeño laboral, calificaciones, antecedentes y aptitud para el puesto.</p><p>Autorizo expresamente la entrega de información relacionada con mi desempeño y aptitud para el empleo, y libero a Cuban Guys / Qban Guys LLC, a mis empleadores anteriores y a las personas o empresas consultadas de toda responsabilidad por proporcionar dicha información.</p><p>Entiendo que cualquier declaración falsa u omisión de información puede causar el rechazo inmediato de mi solicitud o mi despido si llegara a ser contratado. También entiendo que esta solicitud no constituye un contrato de empleo, que el empleo puede terminarse en cualquier momento y que completar esta solicitud no garantiza que exista un puesto disponible ni obliga a Cuban Guys / Qban Guys LLC a ofrecerme un puesto.</p></div><label className="flex items-start gap-3 text-sm"><input type="checkbox" checked={responses.applicationDeclaration === true} onChange={(event) => onChange("applicationDeclaration", event.target.checked)} className="mt-1 size-4" /><span>Confirmo que la información es correcta y autorizo su tratamiento para este proceso.</span></label></section>
  </div>;
}
