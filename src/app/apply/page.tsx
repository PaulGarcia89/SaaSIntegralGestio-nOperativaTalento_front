...
function ApplyWizard() {
   ...
   */
  const validate = () => {
    if (!form.fullName.trim()) return "Escribe tu nombre para que sepamos quién eres.";
    if (!/^\S+@\S+\.\S+$/.test(form.email)) return "Revisa tu correo: parece que le falta algo.";
    ...
    if (!getCandidateSession() && candidatePassword.length < 10) return "Crea una contraseña de al menos 10 caracteres para poder seguir tu postulación.";
    if (!consent) return "Marca la casilla del final para confirmar que tus datos son correctos.";
    return "";
  };
  ...
  return (
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 pb-14 pt-2">
      ...
      <Card><CardContent className="space-y-8 pt-6">
        ...
        {!receipt ? <div className="space-y-5"><input tabIndex={-1} aria-hidden="true" autoComplete="off" name="website" value={website} onChange={(event) => setWebsite(event.target.value)} className="absolute -left-[10000px] h-px w-px opacity-0" /><dl className="grid gap-3 rounded-xl bg-secondary/40 p-5 text-sm"><div><dt className="text-muted-foreground">Nombre</dt><dd className="font-medium">{form.fullName}</dd></div><div><dt className="text-muted-foreground">Correo</dt><dd className="font-medium">{form.email}</dd></div><div><dt className="text-muted-foreground">Vacante</dt><dd className="font-medium">{vacancy.title}</dd></div>{resumeFile ? <div><dt className="text-muted-foreground">CV privado</dt><dd className="font-medium">{resumeFile.name}</dd></div> : null}</dl><div className="rounded-xl border bg-muted/20 p-5 text-sm leading-7" aria-labelledby="application-disclaimer-title"><h2 id="application-disclaimer-title" className="font-semibold">Lo que estás aceptando</h2><p className="mt-3">Léelo con calma antes de marcar la casilla. El contenido no cambia; solo está escrito para entenderse.</p><ul className="mt-3 list-disc space-y-3 pl-5"><li><strong>Podemos comprobar lo que nos cuentas.</strong> Autorizas a {empresa} a verificar lo que escribiste aquí, y autorizas a tus empleadores anteriores y a las personas que indiques a darnos información sobre tu trabajo, tu preparación y tu idoneidad para el puesto.</li><li><strong>Nadie responde por dar esa información.</strong> Liberas a {empresa}, a tus empleadores anteriores y a las personas consultadas de cualquier responsabilidad por habérnosla dado.</li><li><strong>La información tiene que ser cierta.</strong> Si algo es falso o se omitió a propósito, tu solicitud puede rechazarse, y si ya estabas contratado puede terminarse tu empleo.</li><li><strong>Esto no es un contrato de trabajo.</strong> Enviar esta solicitud no garantiza que haya un puesto disponible ni obliga a {empresa} a ofrecerte uno, y el empleo puede terminarse en cualquier momento.</li></ul></div><label className="flex min-h-14 items-start gap-3 rounded-xl border p-4"><input type="checkbox" checked={consent} onChange={(event) => setConsent(event.target.checked)} className="mt-1 size-5" /><span className="text-base leading-7">He leído lo anterior, confirmo que lo que escribí es cierto y autorizo que {empresa} use mis datos para este proceso.</span></label></div> : null}
        ...
        {receipt ? <div className="rounded-xl border border-primary/20 bg-primary/5 p-4 text-left"><p className="font-medium">¿Qué sigue?</p><p className="mt-1 text-sm text-muted-foreground">Consulta el estado de esta postulación y revisa aquí las próximas entrevistas o mensajes de {vacancy.tenant?.name ?? "la empresa"}.</p></div> : null}
        ...
        {!receipt ? <div className="flex flex-col gap-3 border-t pt-6 sm:flex-row-reverse sm:items-center sm:justify-between">
          <Button type="button" className="min-h-14 w-full text-base sm:w-auto" onClick={submit} disabled={!consent || submitMutation.isPending}>
            {submitMutation.isPending ? t("application.sending") : "Enviar mi postulación"}<Send className="size-5" />
          </Button>
          <Button type="button" variant="ghost" className="min-h-14 w-full text-base sm:w-auto" onClick={pause}>Guardar y seguir después</Button>
        </div> : null}
      </CardContent></Card>
      </>}
    </div>
...
function CandidateProfileFields({ form, onChange }: { form: PublicApplicationInput; onChange: (key: string, value: unknown) => void }) {
  ...
  return <div className="space-y-6">
    <div className="grid gap-4 md:grid-cols-2">
      <Field label="Nombre" required value={form.fullName} onChange={(value) => onChange("fullName", value)} />
      <Field label="Apellido" required value={response("lastName")} onChange={(value) => onChange("lastName", value)} />
      <Field label="Correo electrónico" type="email" required value={form.email} onChange={(value) => onChange("email", value)} />
      <Field label="Teléfono" type="tel" value={form.phone || ""} onChange={(value) => onChange("phone", value)} />
      <Field label="Ciudad donde vives" value={form.city || ""} onChange={(value) => onChange("city", value)} />
    </div>

    <fieldset className="space-y-4">
      <legend className="text-base font-semibold">Cinco preguntas rápidas</legend>
      <div className="grid gap-4 md:grid-cols-2">
        <BinaryField label="¿Tienes 18 años o más?" value={form.dynamicResponses?.is18OrOlder} onChange={(value) => onChange("is18OrOlder", value)} />
        <BinaryField label="¿Puedes trabajar legalmente en Estados Unidos?" value={form.dynamicResponses?.authorizedToWorkInUS} onChange={(value) => onChange("authorizedToWorkInUS", value)} />
        <div><BinaryField label="¿Habías trabajado antes en esta empresa?" value={form.dynamicResponses?.workedForCompany} onChange={(value) => onChange("workedForCompany", value)} />{form.dynamicResponses?.workedForCompany === true ? <div className="mt-4"><TextAreaField label="Cuéntanos cuándo y en qué puesto" value={response("workedForCompanyExplanation")} onChange={(value) => onChange("workedForCompanyExplanation", value)} /></div> : null}</div>
        <div><BinaryField label="¿Algún familiar tuyo trabaja aquí?" value={form.dynamicResponses?.familyWorksForCompany} onChange={(value) => onChange("familyWorksForCompany", value)} />{form.dynamicResponses?.familyWorksForCompany === true ? <div className="mt-4"><TextAreaField label="Cuéntanos quién y en qué puesto" value={response("familyWorksForCompanyExplanation")} onChange={(value) => onChange("familyWorksForCompanyExplanation", value)} /></div> : null}</div>
        <div><BinaryField label="¿Has sido condenado por un delito grave?" value={form.dynamicResponses?.felonyConviction} onChange={(value) => onChange("felonyConviction", value)} />{form.dynamicResponses?.felonyConviction === true ? <div className="mt-4"><TextAreaField label="Cuéntanos qué ocurrió" value={response("felonyConvictionExplanation")} onChange={(value) => onChange("felonyConvictionExplanation", value)} /></div> : null}</div>
      </div>
    </fieldset>
  </div>;
}
