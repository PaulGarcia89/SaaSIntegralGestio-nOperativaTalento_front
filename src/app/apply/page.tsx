...
function EmploymentAndReferenceQuestions({ responses, onChange }: { responses: Record<string, unknown>; onChange: (key: string, value: unknown) => void }) {
  ...
  const hasWorkedForCompany = responses.workedForCompany === true;
  const hasFamilyAtCompany = responses.familyWorksForCompany === true;
  const hasFelony = responses.felonyConviction === true;
  return <div className="mt-8 space-y-8">
    <section className="space-y-4 rounded-2xl border bg-muted/20 p-5"><div><h2 className="text-lg font-semibold">1. Elegibilidad</h2><p className="mt-1 text-sm text-muted-foreground">Responde estas preguntas para confirmar tu elegibilidad.</p></div><div className="grid gap-5 md:grid-cols-2"><BinaryField label="¿Tienes 18 años o más?" value={responses.is18OrOlder} onChange={(next) => onChange("is18OrOlder", next)} /><BinaryField label="¿Estás autorizado para trabajar en Estados Unidos?" value={responses.authorizedToWorkInUS} onChange={(next) => onChange("authorizedToWorkInUS", next)} /><BinaryField label="¿Has trabajado antes para esta empresa?" value={responses.workedForCompany} onChange={(next) => onChange("workedForCompany", next)} /><BinaryField label="¿Algún familiar trabaja para esta empresa?" value={responses.familyWorksForCompany} onChange={(next) => onChange("familyWorksForCompany", next)} /><BinaryField label="¿Has sido condenado por un delito grave?" value={responses.felonyConviction} onChange={(next) => onChange("felonyConviction", next)} /></div></section>
    ...
    <section className="space-y-4 rounded-2xl border p-5"><div><h2 className="text-lg font-semibold">3. Información adicional</h2><p className="mt-1 text-sm text-muted-foreground">Completa explicaciones solo cuando hayas respondido “Sí”.</p></div>{hasWorkedForCompany ? <Field label="Explica si has trabajado para esta empresa" value={value("workedForCompanyExplanation")} onChange={(next) => onChange("workedForCompanyExplanation", next)} /> : null}{hasFamilyAtCompany ? <Field label="Explica si un familiar trabaja para esta empresa" value={value("familyWorksForCompanyExplanation")} onChange={(next) => onChange("familyWorksForCompanyExplanation", next)} /> : null}{hasFelony ? <Field label="Explica la condena" value={value("felonyConvictionExplanation")} onChange={(next) => onChange("felonyConvictionExplanation", next)} /> : null}</section>
