...
const steps = ["Vacante", "Acceso del postulante", "Preguntas", "CV", "Enviar", "Confirmación"];
...
const normalizeDraftStep = (draft: PublicApplicationDraftState) => draft.flowVersion === 2 ? Math.min(Math.max(draft.step, 0), 5) : [0, 1, 1, 2, 3, 4][Math.min(Math.max(draft.step, 0), 5)] ?? 0;
...
function ApplyWizard() {
  ...
  }, onSuccess: async () => { await deletePublicApplicationDraft(vacancyId).catch(() => undefined); trackProductEvent({ name: "flow_completed", flow: "vacancy" }); setStep(5); }, onError: (cause) => {
  ...
  return (
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 pb-14 pt-2">
      ...
      <Card><CardHeader><CardTitle>{steps[step]}</CardTitle></CardHeader><CardContent className="space-y-6">
        ...
        {step === 1 ? <CandidateAuthCard returnPath={`/apply?${params.toString()}`} portalLabel={portal?.company?.name ? `el portal de ${portal.company.name}` : "este portal"} onAuthenticated={() => { setAuthenticated(true); setError(""); }} /> : null}
        ...
        {step < 5 ? <div className="flex flex-col-reverse gap-3 border-t pt-5 sm:flex-row sm:items-center sm:justify-between"><div className="flex gap-2"><Button type="button" variant="secondary" onClick={back} disabled={step === 0}><ChevronLeft className="size-4" />Anterior</Button>{step > 0 ? <Button type="button" variant="ghost" onClick={pause}>Guardar y salir</Button> : null}</div>{step < 4 ? <Button type="button" onClick={next}>Siguiente<ChevronRight className="size-4" /></Button> : <Button type="button" onClick={submit} disabled={!consent || submitMutation.isPending}>{submitMutation.isPending ? "Enviando…" : "Enviar postulación"}<Send className="size-4" /></Button>}</div> : null}
