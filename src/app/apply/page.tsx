  ...
  return (
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 pb-14 pt-2">
      ...
      <Card><CardHeader><CardTitle>{steps[step]}</CardTitle></CardHeader><CardContent className="space-y-6">
        ...
        {step === 4 ? <div className="space-y-3"><div><h2 className="font-semibold">Currículum</h2><p className="mt-1 text-sm text-muted-foreground">PDF o DOCX de hasta 15 MB. El archivo se escaneará y almacenará de forma privada.</p></div><FileUpload accept="application/pdf,application/vnd.openxmlformats-officedocument.wordprocessingml.document" maxFiles={1} maxSizeBytes={15 * 1024 * 1024} onFiles={(files) => setResumeFile(files[0] ?? null)} />{resumeFile ? <div className="flex items-center justify-between rounded-xl border p-3 text-sm"><span>{resumeFile.name} · {(resumeFile.size / 1024 / 1024).toFixed(2)} MB</span><Button type="button" size="sm" variant="ghost" onClick={() => setResumeFile(null)}>Quitar</Button></div> : null}</div> : null}
