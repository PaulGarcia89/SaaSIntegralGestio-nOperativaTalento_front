...
function ApplyWizard() {
  ...
  if (step === 2) return <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 pb-14 pt-2"><CandidateNav /><Card><CardHeader><CardTitle>Tu perfil</CardTitle></CardHeader><CardContent><CandidateProfileFields form={form} onChange={(key, value) => { if (["fullName", "email", "phone", "city"].includes(key)) setField(key as keyof PublicApplicationInput, String(value)); else setResponse(key, value); }} />{error ? <p role="alert" className="mt-5 text-sm text-destructive">{error}</p> : null}<div className="mt-6 flex justify-between border-t pt-5"><Button type="button" variant="secondary" onClick={back}>Anterior</Button><Button type="button" onClick={next}>Siguiente<ChevronRight className="size-4" /></Button></div></CardContent></Card></div>;
...
function CandidateProfileFields({ form, onChange }: { form: PublicApplicationInput; onChange: (key: string, value: unknown) => void }) {
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-4 md:grid-cols-2"><BinaryField label="¿Tienes 18 años o más?" value={form.dynamicResponses?.is18OrOlder} onChange={(value) => onChange("is18OrOlder", value)} /><BinaryField label="¿Estás autorizado para trabajar en Estados Unidos?" value={form.dynamicResponses?.authorizedToWorkInUS} onChange={(value) => onChange("authorizedToWorkInUS", value)} /><BinaryField label="¿Has trabajado antes para esta empresa?" value={form.dynamicResponses?.workedForCompany} onChange={(value) => onChange("workedForCompany", value)} /><BinaryField label="¿Algún familiar trabaja para esta empresa?" value={form.dynamicResponses?.familyWorksForCompany} onChange={(value) => onChange("familyWorksForCompany", value)} /><BinaryField label="¿Has sido condenado por un delito grave?" value={form.dynamicResponses?.felonyConviction} onChange={(value) => onChange("felonyConviction", value)} /></div>
    <div className="grid gap-4 md:grid-cols-2"><Field label="Si respondiste sí, explica si has trabajado para esta empresa" value={response("workedForCompanyExplanation")} onChange={(value) => onChange("workedForCompanyExplanation", value)} /><Field label="Si respondiste sí, explica si un familiar trabaja para esta empresa" value={response("familyWorksForCompanyExplanation")} onChange={(value) => onChange("familyWorksForCompanyExplanation", value)} /><Field label="Si respondiste sí, explica la condena" value={response("felonyConvictionExplanation")} onChange={(value) => onChange("felonyConvictionExplanation", value)} /><Field label="Preferencia de puesto" value={response("employmentPreference")} onChange={(value) => onChange("employmentPreference", value)} /><Field label="Preferencia de turno" value={response("shiftPreference")} onChange={(value) => onChange("shiftPreference", value)} /><Field label="Tipo de jornada" value={response("employmentType")} onChange={(value) => onChange("employmentType", value)} /><Field label="Salario deseado por hora" type="number" value={response("desiredHourlyWage")} onChange={(value) => onChange("desiredHourlyWage", value)} /></div>
  </div>;
}
...
function EmploymentAndReferenceQuestions({ responses, onChange }: { responses: Record<string, unknown>; onChange: (key: string, value: unknown) => void }) {
  ...
  return <div className="space-y-6">
    <details className="rounded-xl border p-4" open><summary className="cursor-pointer font-semibold">Empleo anterior</summary><div className="mt-5 grid gap-4 md:grid-cols-2"><Field label="Empresa" value={value("previousEmployerCompany")} onChange={(next) => onChange("previousEmployerCompany", next)} /><Field label="Dirección" value={value("previousEmployerAddress")} onChange={(next) => onChange("previousEmployerAddress", next)} /><Field label="Fecha de inicio" type="date" value={value("previousEmployerStartDate")} onChange={(next) => onChange("previousEmployerStartDate", next)} /><Field label="Fecha de finalización" type="date" value={value("previousEmployerEndDate")} onChange={(next) => onChange("previousEmployerEndDate", next)} /><Field label="Puesto" value={value("previousEmployerPosition")} onChange={(next) => onChange("previousEmployerPosition", next)} /><Field label="Salario final" type="number" value={value("previousEmployerEndingSalary")} onChange={(next) => onChange("previousEmployerEndingSalary", next)} /><Field label="Ciudad, estado y código postal" value={value("previousEmployerLocation")} onChange={(next) => onChange("previousEmployerLocation", next)} /><Field label="Nombre del supervisor" value={value("previousEmployerSupervisor")} onChange={(next) => onChange("previousEmployerSupervisor", next)} /><Field label="Teléfono" type="tel" value={value("previousEmployerPhone")} onChange={(next) => onChange("previousEmployerPhone", next)} /><Field label="Motivo de salida" value={value("previousEmployerLeavingReason")} onChange={(next) => onChange("previousEmployerLeavingReason", next)} /><BinaryField label="¿Podemos contactar a tu supervisor anterior?" value={responses.previousEmployerMayContactSupervisor} onChange={(next) => onChange("previousEmployerMayContactSupervisor", next)} /></div></details>
    <details className="rounded-xl border p-4" open><summary className="cursor-pointer font-semibold">Referencias</summary><p className="mt-2 text-sm text-muted-foreground">Incluye hasta tres referencias laborales o personales que podamos contactar.</p><div className="mt-5 space-y-5">{[1, 2, 3].map((number) => <div key={number} className="grid gap-4 rounded-xl border p-4 md:grid-cols-3"><Field label={`Nombre de la referencia ${number}`} value={value(`reference${number}Name`)} onChange={(next) => onChange(`reference${number}Name`, next)} /><Field label="Relación" value={value(`reference${number}Relationship`)} onChange={(next) => onChange(`reference${number}Relationship`, next)} /><Field label="Teléfono" type="tel" value={value(`reference${number}Phone`)} onChange={(next) => onChange(`reference${number}Phone`, next)} /></div>)}</div></details>
    <label className="flex items-start gap-3 text-sm"><input type="checkbox" checked={responses.applicationDeclaration === true} onChange={(event) => onChange("applicationDeclaration", event.target.checked)} className="mt-1 size-4" /><span>Declaro que la información proporcionada es verdadera y autorizo su verificación. Firma: <input aria-label="Nombre para firma" value={value("signatureName")} onChange={(event) => onChange("signatureName", event.target.value)} className="ml-1 border-b bg-transparent px-1 outline-none" /></span></label>
  </div>;
}
