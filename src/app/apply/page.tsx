...
type PublicApplicationDraftState = { step: number; form: PublicApplicationInput };
type PublicApplicationProgress = { step: number; form: PublicApplicationInput; savedAt?: string };
...
function ApplyWizard() {
  ...
  }, onSuccess: async () => { await deletePublicApplicationDraft(vacancyId).catch(() => undefined); trackProductEvent({ name: "flow_completed", flow: "vacancy" }); setStep(6); }, onError: (cause) => setError(cause instanceof Error ? cause.message : "No fue posible enviar la postulación.") });
  ...
  useEffect(() => {
    ...
    trackProductEvent({ name: "flow_step_viewed", flow: "vacancy", step: draft.step ?? 0 });
  }, [draftQuery.data]);

  useEffect(() => {
    if (!vacancyId || !draftQuery.isSuccess) return;
    if (draftTimer.current) window.clearTimeout(draftTimer.current);
    draftTimer.current = window.setTimeout(() => {
      const progress: PublicApplicationProgress = { step, form };
  ...
  const next = () => { const message = validate(); if (message) return setError(message); setError(""); trackProductEvent({ name: "flow_step_viewed", flow: "vacancy", step: step + 1 }); setStep((value) => Math.min(value + 1, 5)); };
  ...
  const pause = () => { void deletePublicApplicationDraft(vacancyId).catch(() => undefined); setError("Tu progreso quedó guardado en Railway y puedes volver después con el mismo enlace."); };
  ...
  return (
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 pb-14 pt-2">
      ...
      <Card><CardHeader><CardTitle>{steps[step]}</CardTitle></CardHeader><CardContent className="space-y-6">
        ...
        {step < 6 ? <div className="flex flex-col-reverse gap-3 border-t pt-5 sm:flex-row sm:justify-between"><Button type="button" variant="secondary" onClick={back} disabled={step === 0}><ChevronLeft className="size-4" />Anterior</Button>{step < 5 ? <Button type="button" onClick={next}>Siguiente<ChevronRight className="size-4" /></Button> : <Button type="button" onClick={() => submitMutation.mutate()} disabled={!consent || submitMutation.isPending}>{submitMutation.isPending ? "Enviando…" : "Enviar postulación"}<Send className="size-4" /></Button>}</div> : null}
