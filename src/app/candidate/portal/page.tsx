...
import { CalendarDays, Download, FileText, Mail, Send, Signature } from "lucide-react";
import { downloadCandidateInterviewInvitation, exchangeCandidateSocialCode, fetchCandidateApplications, fetchCandidateJobOffers, fetchCandidatePortalOverview, fetchCandidateResumeAccess, getCandidateSession } from "@/lib/backend";
...
function CandidatePortalContent() {
  ...
  return <main className="mx-auto flex w-full max-w-[1440px] flex-col gap-6 pb-12 pt-2"><CandidateNav />
    ...
    {applications.isLoading || overview.isLoading || jobOffers.isLoading ? <AsyncState state="loading" title="Preparando tu centro del candidato" /> : null}
    {applications.isError || overview.isError || jobOffers.isError ? <AsyncState state="error" title="No pudimos cargar tu portal" onRetry={() => { void applications.refetch(); void overview.refetch(); void jobOffers.refetch(); }} /> : null}
    {authenticated && applications.data && overview.data && jobOffers.data ? <>
      <header className="rounded-[2rem] border bg-card p-6 md:p-8"><Badge variant="secondary">Área privada del candidato</Badge><h1 className="mt-3 text-3xl font-semibold tracking-tight">Tu centro de reclutamiento</h1><p className="mt-2 max-w-3xl text-muted-foreground">Entrevistas, ofertas, comunicaciones y documentos organizados por postulación.</p><Button asChild className="mt-5"><Link href={"/candidate/profile?lang=" + lang}>Gestionar perfil y privacidad</Link></Button></header>
      ...
      <section aria-labelledby="interviews-title" className="space-y-3"><h2 id="interviews-title" className="text-2xl font-semibold">Entrevistas</h2>{applications.data.flatMap((application) => (application.interviews ?? []).map((interview) => <Card key={interview.id}><CardContent className="grid gap-4 p-5 md:grid-cols-[1fr_auto]"><div><p className="font-semibold">{interview.title}</p><p className="mt-1 text-sm text-muted-foreground">{application.vacancy.title} · {new Date(interview.startsAt).toLocaleString(lang, { timeZone: interview.timezone })}</p><p className="text-sm text-muted-foreground">{interview.timezone} · {technicalLabel(interview.status)}</p></div>{interview.meetingUrl ? <Button asChild><a href={interview.meetingUrl} target="_blank" rel="noreferrer">Entrar a la reunión</a></Button> : null}</CardContent></Card>))}{!applications.data.some((item) => item.interviews?.length) ? <Empty text="No tienes entrevistas programadas actualmente." /> : null}</section>
      ...
      <section aria-labelledby="offers-title" className="space-y-3"><h2 id="offers-title" className="text-2xl font-semibold">Comunicaciones</h2>{overview.data.communications.filter((message) => message.type !== "OFFER").map((message) => <Card key={message.id}><CardHeader><div className="flex items-start justify-between gap-3"><CardTitle>{message.subject}</CardTitle><Badge variant={message.type === "OFFER" ? "default" : "secondary"}>{message.type}</Badge></div></CardHeader><CardContent><p className="whitespace-pre-line text-sm leading-6 text-muted-foreground">{message.body}</p><time className="mt-3 block text-xs text-muted-foreground">{new Date(message.createdAt).toLocaleString(lang)}</time></CardContent></Card>)}{!overview.data.communications.some((message) => message.type !== "OFFER") ? <Empty text="Aún no tienes comunicaciones." /> : null}</section>
      <section aria-labelledby="documents-title" className="space-y-3"><h2 id="documents-title" className="text-2xl font-semibold">Documentos</h2><div className="grid gap-4 lg:grid-cols-2">{overview.data.resumes.map((document) => <Card key={document.id}><CardContent className="flex items-center gap-3 p-5"><FileText className="size-5" /><div className="min-w-0 flex-1"><p className="truncate font-medium">{document.originalName}</p><p className="text-sm text-muted-foreground">CV · v{document.version} · {technicalLabel(document.status)}</p></div><Button size="sm" variant="secondary" onClick={() => void fetchCandidateResumeAccess(document.id).then(({ url }) => window.open(url, "_blank", "noopener,noreferrer"))}>Abrir</Button></CardContent></Card>)}{overview.data.signatureDocuments.map((document) => <Card key={document.id}><CardContent className="flex items-center gap-3 p-5"><Signature className="size-5" /><div><p className="font-medium">{document.signaturePackage.title}</p><p className="text-sm text-muted-foreground">{technicalLabel(document.status)}</p></div></CardContent></Card>)}</div>{!overview.data.resumes.length && !overview.data.signatureDocuments.length ? <Empty text="No hay documentos disponibles." /> : null}</section>
    </> : null}
  </main>;
}
