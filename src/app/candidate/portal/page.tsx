...
function CandidatePortalContent() {
  ...
  return <main className="mx-auto flex w-full max-w-[1440px] flex-col gap-6 pb-12 pt-2"><CandidateNav />
    ...
    {authenticated && applications.data && overview.data && jobOffers.data ? <>
      ...
      <section aria-labelledby="notifications-title" className="space-y-3"><div className="flex items-center justify-between gap-3"><h2 id="notifications-title" className="text-2xl font-semibold">Notificaciones y mensajes</h2><Badge variant="secondary">{overview.data.communications.filter((item) => !item.readAt).length} sin leer</Badge></div><div className="grid gap-4 lg:grid-cols-2">{overview.data.communications.map((message) => <Card key={message.id} className={!message.readAt ? "border-primary/40" : undefined}><CardHeader><div className="flex items-start justify-between gap-3"><CardTitle>{message.subject}</CardTitle><Badge variant="secondary">{message.readAt ? "Leída" : "Nueva"}</Badge></div></CardHeader><CardContent><p className="whitespace-pre-line text-sm leading-6 text-muted-foreground">{message.body}</p><div className="mt-3 flex flex-wrap items-center gap-2 text-xs text-muted-foreground"><span>{new Date(message.createdAt).toLocaleString(lang)}</span><span>{message.status}</span>{message.deliveredAt ? <span>Entregado {new Date(message.deliveredAt).toLocaleString(lang)}</span> : null}{!message.readAt ? <Button size="sm" variant="ghost" disabled={markRead.isPending} onClick={() => markRead.mutate(message.id)}>Marcar como leída</Button> : null}</div></CardContent></Card>)}</div>{!overview.data.communications.length ? <Empty text="Aún no tienes comunicaciones." /> : null}</section>
      <section aria-labelledby="documents-title" id="documents" className="space-y-3"><h2 id="documents-title" className="text-2xl font-semibold">Documentos y versiones</h2><div className="grid gap-4 lg:grid-cols-2">{overview.data.resumes.map((document) => <Card key={document.id}><CardContent className="space-y-3 p-5"><div className="flex items-center gap-3"><FileText className="size-5" /><div className="min-w-0 flex-1"><p className="truncate font-medium">{document.originalName}</p><p className="text-sm text-muted-foreground">CV · versión {document.version} · {technicalLabel(document.status)} · {new Date(document.createdAt).toLocaleDateString(lang)}</p></div><Button size="sm" variant="secondary" onClick={() => void fetchCandidateResumeAccess(document.id).then(({ url }) => window.open(url, "_blank", "noopener,noreferrer"))}><Download className="size-4" />Descargar</Button></div><p className="text-xs text-muted-foreground">Historial conservado según la política de privacidad. El acceso es temporal y privado.</p></CardContent></Card>)}{overview.data.signatureDocuments.map((document) => <Card key={document.id}><CardContent className="space-y-3 p-5"><div className="flex items-center gap-3"><Signature className="size-5" /><div><p className="font-medium">{document.signaturePackage.title}</p><p className="text-sm text-muted-foreground">{technicalLabel(document.status)}</p></div></div><p className="text-xs text-muted-foreground">Firma · vence {document.tokenExpiresAt ? new Date(document.tokenExpiresAt).toLocaleDateString(lang) : "sin fecha"}</p></CardContent></Card>)}</div>{!overview.data.resumes.length && !overview.data.signatureDocuments.length ? <Empty text="No hay documentos disponibles." /> : null}</section>
      <section aria-labelledby="requests-title" className="space-y-3"><h2 id="requests-title" className="text-2xl font-semibold">Privacidad y soporte</h2><div className="grid gap-4 lg:grid-cols-2"><Card><CardContent className="space-y-3 p-5"><p className="font-semibold">Solicitudes de privacidad</p>{overview.data.privacyRequests.map((item) => <div key={item.id} className="rounded-xl border p-3 text-sm"><div className="flex justify-between gap-3"><span>{item.type}</span><Badge variant="secondary">{item.status}</Badge></div><p className="mt-1 text-muted-foreground">{new Date(item.requestedAt).toLocaleDateString(lang)}{item.response ? ` · ${item.response}` : ""}</p></div>)}{!overview.data.privacyRequests.length ? <p className="text-sm text-muted-foreground">No has creado solicitudes de privacidad.</p> : null}<Button asChild variant="secondary"><Link href="/candidate/profile">Gestionar privacidad</Link></Button></CardContent></Card><Card><CardContent className="space-y-3 p-5"><p className="font-semibold">Centro de soporte</p>{overview.data.supportRequests.map((item) => <div key={item.id} className="rounded-xl border p-3 text-sm"><div className="flex justify-between gap-3"><span>{item.subject}</span><Badge variant="secondary">{item.status}</Badge></div><p className="mt-1 text-muted-foreground">{item.response ?? `Enviada el ${new Date(item.requestedAt).toLocaleDateString(lang)}`}</p></div>)}<Input value={supportSubject} onChange={(event) => setSupportSubject(event.target.value)} maxLength={160} placeholder="Asunto" /><textarea value={supportMessage} onChange={(event) => setSupportMessage(event.target.value)} maxLength={4000} className="min-h-24 w-full rounded-xl border bg-background p-3 text-sm" placeholder="¿En qué podemos ayudarte?" /><Button disabled={!supportSubject.trim() || !supportMessage.trim() || support.isPending} onClick={() => support.mutate()}>{support.isPending ? "Enviando..." : "Enviar solicitud"}</Button></CardContent></Card></div></section>
