...
function CandidatePortalContent() {
  ...
  return <main className="mx-auto flex w-full max-w-[1440px] flex-col gap-6 pb-12 pt-2"><CandidateNav />
    ...
    {authenticated && applications.data ? <>
      ...
      <section aria-labelledby="documents-title" id="documents" className="space-y-3"><h2 id="documents-title" className="text-2xl font-semibold">Documentos y versiones</h2>{overview.isError ? <SectionUnavailable onRetry={() => { void overview.refetch(); }} /> : <><div className="grid gap-4 lg:grid-cols-2">{portalOverview.resumes.map((document) => <Card key={document.id}><CardContent className="space-y-3 p-5"><div className="flex items-center gap-3"><FileText className="size-5" /><div className="min-w-0 flex-1"><p className="truncate font-medium">{document.originalName}</p><p className="text-sm text-muted-foreground">CV · versión {document.version} · {technicalLabel(document.status)} · {new Date(document.createdAt).toLocaleDateString(lang)}</p></div><Button size="sm" variant="secondary" onClick={() => void fetchCandidateResumeAccess(document.id).then(({ url }) => window.open(url, "_blank", "noopener,noreferrer"))}><Download className="size-4" />Descargar</Button></div><p className="text-xs text-muted-foreground">Historial conservado según la política de privacidad. El acceso es temporal y privado.</p></CardContent></Card>)}{portalOverview.signatureDocuments.map((document) => <Card key={document.id}><CardContent className="space-y-3 p-5"><div className="flex items-center gap-3"><Signature className="size-5" /><div><p className="font-medium">{document.signaturePackage.title}</p><p className="text-sm text-muted-foreground">{technicalLabel(document.status)}</p></div></div><p className="text-xs text-muted-foreground">Firma · vence {document.tokenExpiresAt ? new Date(document.tokenExpiresAt).toLocaleDateString(lang) : "sin fecha"}</p></CardContent></Card>)}</div>{!portalOverview.resumes.length && !portalOverview.signatureDocuments.length ? <Empty text="No hay documentos disponibles." /> : null}</>}</section>
