...
function CandidateProfileContent() {
  ...
  return <main className="mx-auto flex w-full max-w-5xl flex-col gap-6 pb-12 pt-2"><CandidateNav />
    ...
    {profile.data && overview.data ? <>
      ...
      <Card><CardHeader><CardTitle>Datos personales</CardTitle></CardHeader><CardContent className="grid gap-4 md:grid-cols-2"><Field label="Nombre completo" value={form.fullName ?? ""} onChange={(value) => field("fullName", value)} /><Field label="Teléfono" value={form.phone ?? ""} onChange={(value) => field("phone", value)} /><Field label="Ciudad" value={form.city ?? ""} onChange={(value) => field("city", value)} /><Field label="LinkedIn" type="url" value={form.linkedinUrl ?? ""} onChange={(value) => field("linkedinUrl", value)} /><Field label="Portafolio" type="url" value={form.portfolioUrl ?? ""} onChange={(value) => field("portfolioUrl", value)} /><Field label="Zona horaria" value={form.timezone ?? "UTC"} onChange={(value) => field("timezone", value)} /><label className="space-y-2 text-sm font-medium">Idioma<select className="h-11 w-full rounded-xl border bg-background px-3" value="es" disabled><option value="es">Español</option></select></label><div className="flex items-end"><Button onClick={() => save.mutate()} disabled={save.isPending}>{save.isPending ? "Guardando…" : "Guardar perfil"}</Button></div></CardContent></Card>
