...
export default function PublicSignaturePage() {
  ...
  return <main className="min-h-screen bg-surface-page p-4 sm:p-8"><div className="mx-auto grid max-w-6xl gap-6 lg:grid-cols-[minmax(0,1fr)_380px]">
    <Card><CardHeader><div className="flex items-center gap-3"><FileSignature className="size-6 text-brand" /><div><p className="text-sm text-text-secondary">{data.package.title}</p><CardTitle>{data.document.title}</CardTitle></div></div></CardHeader><CardContent><article className="whitespace-pre-wrap rounded-2xl border bg-surface-section p-6 leading-7">{data.document.content}</article><p className="mt-4 text-xs text-text-secondary">Versión {data.document.version} · El contenido se verifica mediante suma de comprobación al firmar.</p></CardContent></Card>
    <Card level={1} className="h-fit lg:sticky lg:top-8"><CardHeader><CardTitle>Consentimiento y firma</CardTitle><p className="text-sm text-text-secondary">{data.participant.fullName} · {data.participant.email}</p></CardHeader><CardContent className="space-y-5"><InlineFeedback tone="info" title="Firma protegida"><span className="flex gap-2"><ShieldCheck className="size-4 shrink-0" />Se guardarán fecha, versión, suma de comprobación y huellas no reversibles para la auditoría.</span></InlineFeedback><label className="flex items-start gap-3 rounded-xl border p-4 text-sm"><input type="checkbox" checked={accepted} onChange={(event) => setAccepted(event.target.checked)} className="mt-1 size-5" /><span>{data.document.consentText}</span></label><div className="space-y-2"><Label htmlFor="typed-name">Escribe tu nombre completo</Label><Input id="typed-name" autoComplete="name" value={typedName} onChange={(event) => setTypedName(event.target.value)} placeholder={data.participant.fullName} /></div><Button className="w-full" disabled={!accepted || typedName.trim().length < 3 || sign.isPending} onClick={() => sign.mutate()}>{sign.isPending ? "Registrando firma…" : "Aceptar y firmar"}</Button>{sign.isError ? <p role="alert" className="text-sm text-status-danger">No pudimos registrar la firma. Comprueba que el nombre coincida y vuelve a intentarlo.</p> : null}</CardContent></Card>
  </div></main>;
}
