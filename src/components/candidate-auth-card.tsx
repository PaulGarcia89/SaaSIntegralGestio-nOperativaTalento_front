...
export function CandidateAuthCard({ onAuthenticated, returnPath = "/application-status" }: { onAuthenticated: () => void; returnPath?: string; lang?: "es" | "en" }) {
  ...
  return <Card className="mx-auto w-full max-w-lg" aria-labelledby="candidate-access-title"><CardHeader><CardTitle id="candidate-access-title">Acceso seguro del candidato</CardTitle></CardHeader><CardContent className="space-y-4">
    <p className="text-sm text-muted-foreground">Entra para ver tus postulaciones, entrevistas, ofertas, documentos y solicitudes de privacidad. Si olvidaste tu contraseña, te enviamos un enlace de recuperación.</p>
    ...
    {!recovering ? <div className="space-y-2"><Label htmlFor="candidate-password">Contraseña</Label><Input id="candidate-password" autoComplete="current-password" type="password" value={password} onChange={(event) => setPassword(event.target.value)} /><p className="text-xs text-muted-foreground">Si no recuerdas la clave, usa recuperación para crear una nueva.</p></div> : null}
