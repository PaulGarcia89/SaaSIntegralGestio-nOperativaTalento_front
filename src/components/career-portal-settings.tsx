  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-6 xl:grid-cols-[1.15fr_.85fr]">
      <Card><CardHeader><CardTitle>Acceso y publicación</CardTitle></CardHeader><CardContent className="space-y-5"><div className="grid gap-4 sm:grid-cols-2">
        <label className="space-y-2 text-sm font-medium">Tipo de portal<select className="field" value={form.type} onChange={(event) => setForm({ ...form, type: event.target.value as PortalForm["type"] })}><option value="PUBLIC">Público</option><option value="PRIVATE_STANDARD">Privado estándar</option><option value="BRANDED">Branded</option></select></label>
        <label className="space-y-2 text-sm font-medium">Acceso<select className="field" value={form.accessType} onChange={(event) => setForm({ ...form, accessType: event.target.value as PortalForm["accessType"] })}><option value="OPEN">Abierto</option><option value="LOGIN_REQUIRED">Requiere login</option><option value="INVITATION_ONLY">Solo invitación</option></select></label>
    ...
    <Card><CardHeader><CardTitle>Contenido y SEO</CardTitle></CardHeader><CardContent className="grid gap-4 md:grid-cols-2"><label className="space-y-2 text-sm font-medium">Título<Input value={form.branding.title ?? ""} onChange={(event) => updateBranding("title", event.target.value)} /></label><label className="space-y-2 text-sm font-medium">Subtítulo<Input value={form.branding.subtitle ?? ""} onChange={(event) => updateBranding("subtitle", event.target.value)} /></label><label className="space-y-2 text-sm font-medium md:col-span-2">Descripción<textarea className="field min-h-24" value={form.branding.description ?? ""} onChange={(event) => updateBranding("description", event.target.value)} /></label><label className="space-y-2 text-sm font-medium md:col-span-2">CSS personalizado<textarea className="field min-h-32 font-mono" value={form.branding.customCss ?? ""} onChange={(event) => updateBranding("customCss", event.target.value)} placeholder=":root { --brand-color: #0f766e; }" /></label><label className="space-y-2 text-sm font-medium md:col-span-2">Texto del pie<textarea className="field min-h-20" value={form.branding.footerText ?? ""} onChange={(event) => updateBranding("footerText", event.target.value)} /></label></CardContent></Card>
