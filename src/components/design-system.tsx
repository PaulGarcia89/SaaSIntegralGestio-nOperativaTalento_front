...
export function Wizard({ steps, current, onStepChange, children }: { steps: string[]; current: number; onStepChange?: (step: number) => void; children: ReactNode }) {
  return <div className="space-y-6"><nav aria-label="Progreso"><ol className="flex gap-2 overflow-x-auto pb-2">{steps.map((step, index) => <li key={step}><button type="button" disabled={index > current || !onStepChange} onClick={() => onStepChange?.(index)} aria-current={index === current ? "step" : undefined} className={cn("min-h-11 whitespace-nowrap rounded-full border px-4 text-sm", index === current && "border-primary bg-primary text-text-on-accent", index < current && "bg-surface-interactive")} >{index < current ? <Check className="mr-1 inline size-4" /> : null}{index + 1}. {step}</button></li>)}</ol></nav><section aria-labelledby={`wizard-step-${current}`}><h2 id={`wizard-step-${current}`} className="sr-only">{steps[current]}</h2>{children}</section></div>;
}
