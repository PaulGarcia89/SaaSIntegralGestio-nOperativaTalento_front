...
export function EmployeeCreatePage() {
  ...
  const [evidenceFiles, setEvidenceFiles] = useState<Array<{ id: string; label: string; file: File }>>([]);
  ...
  const addEvidenceFile = async (label: string, file: File | null) => {
    if (!file) return;
    ...
    setEvidenceFiles((current) => [...current.filter((item) => item.label !== label), { id: `${label}-${file.name}-${file.size}`, label, file }]);
