                    ...
                    {(Array.isArray(detailQuery.data.history) ? detailQuery.data.history : []).map((item, index) => {
                      ...
                      return (
                        <div key={item.id} className="flex gap-3">
                          ...
                          <div className="flex-1 rounded-2xl bg-surface-elevated p-3">
                            <div className="flex flex-wrap items-center justify-between gap-2">
                              <p className="font-medium">{item.title}</p>
                              <Badge variant={isFirst ? "success" : "secondary"}>{isFirst ? "Reciente" : "Histórico"}</Badge>
                            </div>
                            <p className="mt-1 text-sm text-text-secondary">{item.detail}</p>
                            <p className="mt-2 text-xs text-text-secondary">{item.at}</p>
                          </div>
                        </div>
                      );
                    })}
...
export function EmployeeCreatePage() {
  ...
  const [evidenceFiles, setEvidenceFiles] = useState<Array<{ id: string; section: "tax" | "eligibility" | "floridaNewHire"; label: string; file: File }>>([]);
  const [docuSealTemplateKeys, setDocuSealTemplateKeys] = useState<string[]>([]);
  const docuSealTemplates = useQuery({ queryKey: ["docuseal-templates"], queryFn: fetchDocuSealTemplates });
  const create = useMutation({
    mutationFn: async () => {
      const employee = await createEmployee(form);
      if (evidenceFiles.length) {
        await Promise.all(evidenceFiles.map((draft) => uploadEmployeeDocument(employee.id, { file: draft.file, section: draft.section, documentType: draft.label })));
      }
      const docuSealResults = await Promise.allSettled(docuSealTemplateKeys.map((templateKey) => createDocuSealSubmission(employee.id, templateKey)));
      const failedDocuSealRequests = docuSealResults.filter((result) => result.status === "rejected").length;
      if (failedDocuSealRequests) toast.warning(`${failedDocuSealRequests} solicitud(es) DocuSeal no pudieron enviarse; el empleado sí fue creado.`);
      return employee;
    },
  ...
  const addEvidenceFile = async (section: "tax" | "eligibility" | "floridaNewHire", label: string, file: File | null) => {
    if (!file) return;
    ...
    setEvidenceFiles((current) => [...current.filter((item) => item.label !== label), { id: `${section}-${label}-${file.name}-${file.size}`, section, label, file }]);
    toast.success(`Archivo preparado para ${label.toLowerCase()}`);
  };
  const steps = [t("employees.steps.personal"), t("employees.steps.contact"), t("employees.steps.employment"), t("employees.steps.payroll"), t("employees.steps.tax"), t("employees.steps.documents"), t("employees.steps.training"), t("employees.steps.review")];
          ...
          <CardContent className="p-6">
            ...
            >
              ...
              <div className="flex flex-col-reverse gap-2 pt-2 sm:flex-row sm:justify-end">
                ...
                <Button type="submit" disabled={create.isPending || (step === 0 && !basicValid) || (step === 1 && !contactValid) || (step === 2 && !employmentValid) || (step === 7 && !valid)}>{step < steps.length - 1 ? tx("Continuar", "Continue") : create.isPending ? tx("Creando...", "Creating...") : tx("Crear empleado", "Create employee")}</Button>
              </div>
            </form>
          </CardContent>
        </Card>
