...
export function getServerFormMessage(error: unknown) {
  if (!(error instanceof ApiError)) return "No pudimos guardar los cambios. Inténtalo nuevamente.";
...
export function FormErrorSummary({ errors, serverError, success }: { errors?: FormSummaryError[]; serverError?: unknown; success?: string }) {
  ...
  return <div ref={ref} tabIndex={-1} className="rounded-xl border border-destructive/40 bg-destructive/5 p-4 outline-none focus-visible:ring-2 focus-visible:ring-destructive" role="alert" aria-labelledby="form-errors-title">
    <div className="flex gap-3"><AlertCircle className="size-5 shrink-0 text-destructive" /><div><h2 id="form-errors-title" className="font-semibold">Revisa el formulario</h2>{serverError ? <p className="mt-1 text-sm text-muted-foreground">{getServerFormMessage(serverError)}</p> : null}</div></div>
