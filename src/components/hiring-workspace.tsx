...
function Responsible({ application }: { application: VacancyApplicationDto }) {
  ...
  return <div className="flex items-center gap-3"><div className="flex size-9 items-center justify-center rounded-full bg-primary/10 text-sm font-semibold text-brand">{initials(user ? `${user.firstName} ${user.lastName}` : "RRHH")}</div><div><p className="text-sm font-medium">{user ? `${user.firstName} ${user.lastName}` : "Responsable de reclutamiento"}</p><p className="text-xs text-text-secondary">{user?.email ?? "Sin responsable asignado"}</p></div></div>;
}
