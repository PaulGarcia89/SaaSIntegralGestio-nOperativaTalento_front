...
export function HiringReasonDialog({
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent><DialogHeader><DialogTitle>{title}</DialogTitle><DialogDescription>{description}</DialogDescription></DialogHeader><label className="space-y-2 text-sm font-medium" htmlFor="hiring-action-reason">{t("hiring.dialog.reason")}<span className="text-danger"> *</span><textarea id="hiring-action-reason" value={reason} onChange={(event) => setReason(event.target.value)} className="min-h-28 w-full rounded-xl border border-border-default bg-surface-elevated p-3 text-base sm:text-sm" placeholder={t("hiring.dialog.reasonPlaceholder")} /></label><div className="flex justify-end gap-2"><Button type="button" variant="secondary" onClick={() => onOpenChange(false)}>{t("actions.cancel")}</Button><Button type="button" variant="destructive" disabled={reason.trim().length < 3} onClick={submit}>{confirmLabel}</Button></div></DialogContent></Dialog>;
}
