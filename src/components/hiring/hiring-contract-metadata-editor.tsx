...
export function HiringContractMetadataEditor({ contract }: { contract: HiringContractDto }) {
  ...
  return <Card level={2}><CardHeader><CardTitle>{t("hiring.meta.title")}</CardTitle></CardHeader><CardContent className="space-y-4"><p className="text-sm text-text-secondary">{t("hiring.meta.help")}</p><div className="grid gap-3 sm:grid-cols-2"><label className="space-y-2 text-sm font-medium" htmlFor="hiring-priority">{uiText("Prioridad")}<select id="hiring-priority" value={priority} onChange={(event) => setPriority(event.target.value as typeof priority)} className="min-h-[var(--control-h-touch)] w-full rounded-xl border border-border-default bg-surface-elevated px-3"><option value="URGENT">{hiringPriorityLabel("URGENT")}</option><option value="HIGH">{hiringPriorityLabel("HIGH")}</option><option value="MEDIUM">{hiringPriorityLabel("MEDIUM")}</option><option value="LOW">{hiringPriorityLabel("LOW")}</option></select></label><label className="space-y-2 text-sm font-medium" htmlFor="hiring-deadline">{t("hiring.meta.deadline")}<input id="hiring-deadline" type="date" value={deadlineAt} onChange={(event) => setDeadlineAt(event.target.value)} className="min-h-[var(--control-h-touch)] w-full rounded-xl border border-border-default bg-surface-elevated px-3" /></label></div>{update.isError ? <InlineFeedback tone="danger" title={t("hiring.meta.saveFailed")}>{update.error instanceof Error ? update.error.message : t("hiring.meta.retry")}</InlineFeedback> : null}{update.isSuccess ? <InlineFeedback tone="success" title={t("hiring.meta.savedTitle")}>{t("hiring.meta.saved")}</InlineFeedback> : null}<div className="flex justify-end"><Button onClick={() => update.mutate()} disabled={update.isPending}>{update.isPending ? uiText("Guardando…") : t("hiring.meta.save")}</Button></div></CardContent></Card>;
}
