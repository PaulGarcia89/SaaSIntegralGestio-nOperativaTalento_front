...
function Metric({ icon: Icon, label, value }: { icon: typeof Boxes; label: string; value: number }) { return <Card level={2}><CardContent className="flex items-center gap-4 p-4"><Icon className="size-5 text-brand" /><div><p className="text-sm text-text-secondary">{label}</p><p className="text-2xl font-semibold">{value}</p></div></CardContent></Card>; }
