...
export function JobOfferManager({ applicationId, jobTitle, canManage }: { applicationId: string; jobTitle: string; canManage: boolean }) {
  ...
  return <section id="job-offers" className="space-y-4">
    ...
    {!offers.isLoading && !offers.data?.length && !editing ? <InlineFeedback tone="info" title="Sin oferta preparada">Crea la oferta directamente y envíala a firma cuando esté lista.</InlineFeedback> : null}
