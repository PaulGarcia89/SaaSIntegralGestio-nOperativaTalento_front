...
function ProductPreview() {
  ...
  return <div className="relative mx-auto w-full max-w-xl overflow-hidden rounded-[2rem] border border-surface-dark-ink/15 bg-[radial-gradient(circle_at_90%_7%,hsl(38_94%_52%_/_.18),transparent_28%),radial-gradient(circle_at_5%_100%,hsl(205_80%_55%_/_.16),transparent_35%),linear-gradient(145deg,hsl(213_38%_12%),hsl(213_40%_10%)_58%,hsl(213_34%_15%))] p-4 shadow-[0_30px_80px_hsl(213_40%_10%_/_.45)] sm:p-5">
    ...
    <div className="relative mt-4 overflow-hidden rounded-2xl border border-accent-line/15 bg-surface-dark-1/35 p-4 shadow-[inset_0_1px_hsl(210_22%_96%_/_.05)]"><div className="flex items-center justify-between gap-3"><p className="text-sm font-medium text-surface-dark-ink">{t("landing.preview.cycle")}</p><span className="text-[10px] font-medium text-accent-fill">{t("landing.preview.cycleHint")}</span></div><div className="mt-3 flex min-w-0 items-center overflow-x-auto pb-1 [scrollbar-width:none]">{["vacancy", "candidates", "interviews", "onboarding", "training"].map((step, index) => <div key={step} className="flex shrink-0 items-center"><span className={`rounded-full border px-2.5 py-1 text-[11px] ${index === 0 ? "border-accent-line/40 bg-accent-fill/15 font-medium text-accent-fill" : "border-surface-dark-ink/10 bg-surface-1/[.06] text-surface-dark-ink/85"}`}>{t(`landing.preview.chip.${step}`)}</span>{index < 4 ? <ChevronRight className="mx-1 size-3 text-accent-fill/80" /> : null}</div>)}</div></div>
  </div>;
}
