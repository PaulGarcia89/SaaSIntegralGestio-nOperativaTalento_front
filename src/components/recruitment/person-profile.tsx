...
function InterviewScheduler({ application, canSchedule, abierta, onAbierta }: { application: VacancyApplicationDto; canSchedule: boolean; abierta?: boolean; onAbierta?: (abierta: boolean) => void }) {
  ...
  const retry = useMutation({ mutationFn: retryInterviewCalendarSync, onSuccess: async () => { toast.success("Calendario actualizado."); await queryClient.invalidateQueries({ queryKey: ["application", application.id] }); }, onError: (error) => toast.error(getApiErrorMessage(error, "No se pudo sincronizar el calendario.")) });
  ...
  const schedule = useMutation({
    mutationFn: () => {
      ...
      if (start.getTime() <= Date.now()) throw new Error("Elige una fecha futura.");
      if (type === "PRESENTIAL" && !location.trim()) throw new Error("Introduce la ubicación de la entrevista.");
      if (type === "VIRTUAL" && calendarProvider === "NONE" && !meetingUrl.trim()) throw new Error("Introduce el enlace de la reunión o selecciona Google Meet.");
    ...
    onSuccess: async (result) => {
      ...
      setOpen(false);
      if (result?.calendarSyncStatus === "FAILED") toast.error("Entrevista guardada, pero falló el calendario. Reintenta la sincronización antes de confirmar la invitación.");
      else toast.success(calendarProvider === "NONE" ? "Entrevista agendada. Invitación en cola de envío." : "Entrevista agendada e invitación enviada al calendario.");
