...
import { interviewStartInZone } from "@/lib/interview-time";
import { fetchCompanyCalendarSettings, retryInterviewCalendarSync } from "@/lib/backend";
...
function CandidateProfile({ application }: { application: VacancyApplicationDto }) {
  ...
  const currentStageLabel = application.currentStage ? recruitmentStageLabel(application.currentStage, uiText.locale) : applicationStageLabel(application.status, uiText.locale);
    ...
    <Card id="perfil" level={2}>
      ...
      <CardContent className="space-y-4">
        <dl className="grid gap-3 sm:grid-cols-2">
          ...
          <Detail icon={<CalendarClock />} label={uiText("Se postuló el")} value={formatApplicationDate(application.appliedAt, uiText.locale)} />
        </dl>
...
function ScorecardConsensus({ interviewId, stageName }: { interviewId: string; stageName: string }) {
  ...
  if (comparison.data.feedbackLocked) return <InlineFeedback tone="info" title={uiText("Comparación protegida · {{stage}}", { stage: stageName })}>{t("p360.crossResultsHidden")}</InlineFeedback>;
...
function InterviewScheduler({ application, canSchedule, abierta, onAbierta }: { application: VacancyApplicationDto; canSchedule: boolean; abierta?: boolean; onAbierta?: (abierta: boolean) => void }) {
  ...
  const [title, setTitle] = useState(uiText("Entrevista · {{role}}", { role: application.vacancy.title }));
  ...
  const settings = useQuery({ queryKey: ["company-calendar-defaults", currentTenant?.id], queryFn: () => fetchCompanyCalendarSettings(true), enabled: open && canSchedule });
  useEffect(() => {
    if (!open) { defaultsApplied.current = false; return; }
    if (!settings.data || defaultsApplied.current) return;
    defaultsApplied.current = true;
    const defaults = settings.data.defaults;
    setDuration(String(defaults.durationMinutes)); setTimezone(defaults.timezone); setReminderMinutes(defaults.reminderMinutes);
    setCalendarProvider(defaults.defaultModality === "GOOGLE_MEET" ? "GOOGLE" : defaults.defaultModality === "MICROSOFT_TEAMS" ? "MICROSOFT" : "NONE");
    setType(defaults.defaultModality === "PRESENTIAL" ? "PRESENTIAL" : defaults.defaultModality === "PHONE" ? "PHONE" : "VIRTUAL");
  }, [open, settings.data]);
  const retry = useMutation({ mutationFn: retryInterviewCalendarSync, onSuccess: async () => { toast.success(uiText("Calendario actualizado.")); await queryClient.invalidateQueries({ queryKey: ["application", application.id] }); }, onError: (error) => toast.error(getApiErrorMessage(error, uiText("No se pudo sincronizar el calendario."))) });
  ...
  const schedule = useMutation({
    mutationFn: () => {
      ...
      const start = interviewStartInZone(startsAt, timezone);
      if (start.getTime() <= Date.now()) throw new Error(uiText("Elige una fecha futura."));
      if (type === "PRESENTIAL" && !location.trim()) throw new Error(uiText("Introduce la ubicación de la entrevista."));
      if (type === "VIRTUAL" && calendarProvider === "NONE" && !meetingUrl.trim()) throw new Error(uiText("Introduce el enlace de la reunión o selecciona Google Meet."));
      requestId.current ??= crypto.randomUUID();
      const end = new Date(start.getTime() + Number(duration) * 60_000);
      return scheduleRecruitmentInterview({ clientRequestId: requestId.current, reminderMinutes, additionalAttendees: additionalEmails.split(/[,;\n]/).map((email) => email.trim()).filter(Boolean), videoProvider: type === "VIRTUAL" ? calendarProvider === "GOOGLE" ? "GOOGLE_MEET" : calendarProvider === "MICROSOFT" ? "MICROSOFT_TEAMS" : "MANUAL" : "NONE", applicationId: application.id, interviewerUserId, title: title.trim(), type, timezone, startsAt: start.toISOString(), endsAt: end.toISOString(), calendarProvider: calendarProvider === "NONE" ? undefined : calendarProvider, meetingUrl: type === "VIRTUAL" && calendarProvider === "NONE" ? meetingUrl.trim() || undefined : undefined, location: type === "PRESENTIAL" ? location.trim() || undefined : undefined });
    },
    onSuccess: async (result) => {
      requestId.current = null;
      setOpen(false);
      if (result?.calendarSyncStatus === "FAILED") toast.error(uiText("Entrevista guardada, pero falló el calendario. Reintenta la sincronización antes de confirmar la invitación."));
      else toast.success(calendarProvider === "NONE" ? uiText("Entrevista agendada. Invitación en cola de envío.") : uiText("Entrevista agendada e invitación enviada al calendario."));
  ...
  const interviews = application.interviews ?? [];
  return <Card id="entrevistas" level={2}><CardHeader><div className="flex flex-wrap items-center justify-between gap-3"><CardTitle>{t("p360.interviews")}</CardTitle>{canSchedule && interviews.length ? <Button size="sm" variant="secondary" onClick={() => setOpen(true)}><CalendarPlus className="size-4" />{uiText("Agendar otra")}</Button> : null}</div></CardHeader><CardContent>{interviews.length ? <div className="space-y-3">{interviews.map((interview) => <div key={interview.id} className="rounded-xl border border-border-default p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-semibold">{interview.title}</p><p className="text-sm text-text-secondary">{technicalLabel(interview.type, uiText.locale)} · {formatApplicationDate(interview.startsAt, uiText.locale)} · {interview.timezone}</p><p className="text-sm text-text-secondary">{uiText("Entrevistador: ")}{interview.interviewer ? `${interview.interviewer.firstName} ${interview.interviewer.lastName}` : t("p360.unassigned")}</p></div><Badge variant="secondary">{technicalLabel(interview.status, uiText.locale)}</Badge></div>{interview.calendarSyncStatus === "FAILED" && <div role="alert" className="mt-3 rounded-lg border border-red-200 p-3 text-sm"><p>{uiText("La sincronización del calendario falló. La invitación no está confirmada.")}</p>{canSchedule && <Button variant="outline" size="sm" className="mt-2" disabled={retry.isPending} onClick={() => retry.mutate(interview.id)}>{uiText("Reintentar sincronización")}</Button>}</div>}{interview.meetingUrl ? <Button asChild size="sm" variant="secondary" className="mt-3"><a href={interview.meetingUrl} target="_blank" rel="noreferrer">{uiText("Abrir reunión ")}<ExternalLink className="size-4" /></a></Button> : null}</div>)}</div> : application.interview ? <dl className="grid gap-3 sm:grid-cols-2"><Summary label={t("p360.type")} value={application.interview.type ?? t("p360.undefinedValue")} /><Summary label={uiText("Programada")} value={formatApplicationDate(application.interview.scheduledAt, uiText.locale)} /><Summary label={uiText("Seguimiento")} value={formatApplicationDate(application.interview.followUpAt, uiText.locale)} /><Summary label={t("p360.observations")} value={application.interview.observations ?? t("p360.noObservations")} /></dl> : <p className="text-sm text-text-secondary">{t("p360.noInterviewScheduled")}</p>}
    <Dialog open={open} onOpenChange={setOpen}><DialogContent className="sm:max-w-3xl"><DialogHeader><DialogTitle>{t("p360.scheduleInterview")}</DialogTitle><DialogDescription>{t("p360.scheduleBody")}</DialogDescription></DialogHeader><div className="space-y-4"><label className="block space-y-2 text-sm font-medium" htmlFor="interview-title">{t("p360.titleField")}<Input id="interview-title" value={title} onChange={(event) => setTitle(event.target.value)} /></label><div className="grid gap-4 sm:grid-cols-2"><label className="space-y-2 text-sm font-medium" htmlFor="interview-type">{t("p360.type")}<Select value={type} onValueChange={(value) => setType(value as typeof type)}><SelectTrigger id="interview-type"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="VIRTUAL">{uiText("Virtual")}</SelectItem><SelectItem value="PRESENTIAL">{uiText("Presencial")}</SelectItem><SelectItem value="PHONE">{t("p360.typePhone")}</SelectItem></SelectContent></Select></label><label className="space-y-2 text-sm font-medium" htmlFor="interview-interviewer">{t("p360.interviewer")}<Select value={interviewerUserId} onValueChange={setInterviewerUserId}><SelectTrigger id="interview-interviewer"><SelectValue placeholder={interviewers.isLoading ? uiText("Cargando...") : t("p360.pickPerson")} /></SelectTrigger><SelectContent>{interviewers.data?.map((person) => <SelectItem key={person.id} value={person.id}>{person.firstName} {person.lastName}</SelectItem>)}</SelectContent></Select></label><div className="space-y-2 sm:col-span-2"><span className="text-sm font-medium">{t("p360.dateTime")}</span><div className="grid gap-2 sm:grid-cols-2"><label className="space-y-1 text-xs text-muted-foreground" htmlFor="interview-date">{t("p360.date")}<Input id="interview-date" type="date" value={startsAt.slice(0, 10)} onChange={(event) => setStartsAt(`${event.target.value}T${startsAt.slice(11, 16)}`)} /></label><label className="space-y-1 text-xs text-muted-foreground" htmlFor="interview-time">{uiText("Hora")}<Input id="interview-time" type="time" value={startsAt.slice(11, 16)} onChange={(event) => setStartsAt(`${startsAt.slice(0, 10)}T${event.target.value}`)} /></label></div></div><label className="space-y-2 text-sm font-medium" htmlFor="interview-duration">{t("p360.duration")}<Select value={duration} onValueChange={setDuration}><SelectTrigger id="interview-duration"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="30">{uiText("30 minutos")}</SelectItem><SelectItem value="45">{uiText("45 minutos")}</SelectItem><SelectItem value="60">{uiText("60 minutos")}</SelectItem><SelectItem value="90">{uiText("90 minutos")}</SelectItem></SelectContent></Select></label><label className="space-y-2 text-sm font-medium" htmlFor="interview-calendar">{uiText("Calendario")}<Select value={calendarProvider} onValueChange={(value) => setCalendarProvider(value as typeof calendarProvider)}><SelectTrigger id="interview-calendar"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="NONE">{t("p360.internalOnly")}</SelectItem><SelectItem value="GOOGLE" disabled={!settings.data?.providers.find((p) => p.provider === "GOOGLE")?.connected}>Google Calendar / Meet</SelectItem><SelectItem value="MICROSOFT" disabled={!settings.data?.providers.find((p) => p.provider === "MICROSOFT")?.connected}>Outlook / Teams</SelectItem></SelectContent></Select></label><label className="space-y-2 text-sm font-medium" htmlFor="interview-timezone">{uiText("Zona horaria")}<Input id="interview-timezone" value={timezone} onChange={(event) => setTimezone(event.target.value)} /></label></div>{type === "VIRTUAL" && calendarProvider === "NONE" ? <label className="block space-y-2 text-sm font-medium" htmlFor="interview-meeting-url">{t("p360.meetingLink")}<Input id="interview-meeting-url" type="url" placeholder="https://..." value={meetingUrl} onChange={(event) => setMeetingUrl(event.target.value)} /></label> : null}{type === "PRESENTIAL" ? <label className="block space-y-2 text-sm font-medium" htmlFor="interview-location">{t("p360.location")}<Input id="interview-location" value={location} onChange={(event) => setLocation(event.target.value)} /></label> : null}{type === "VIRTUAL" && calendarProvider !== "NONE" && <p className="rounded-lg bg-muted p-3 text-sm">{uiText("Se generará el enlace de {{provider}} y se enviará la invitación desde el calendario de la empresa.", { provider: calendarProvider === "GOOGLE" ? "Google Meet" : "Microsoft Teams" })}</p>}
    <div className="grid gap-4 sm:grid-cols-2"><label className="space-y-2 text-sm font-medium" htmlFor="interview-extra-emails">{uiText("Participantes adicionales")}<Input id="interview-extra-emails" value={additionalEmails} onChange={(event) => setAdditionalEmails(event.target.value)} placeholder={uiText("correos separados por coma")} /></label><label className="space-y-2 text-sm font-medium" htmlFor="interview-reminder">{uiText("Recordatorio (minutos antes)")}<Input id="interview-reminder" type="number" min={0} max={10080} value={reminderMinutes} onChange={(event) => setReminderMinutes(Number(event.target.value))} /></label></div>
    {can("admin.company") && <Button asChild variant="ghost" size="sm"><Link href="/admin/company/calendar">{uiText("Conectar Google o Microsoft en Administración")}</Link></Button>}
    {settings.isError && <p role="alert" className="text-sm text-red-700">{uiText("No se pudo consultar la conexión del calendario.")}</p>}
    {schedule.isError ? <InlineFeedback tone="danger" title={t("p360.scheduleFailed")}>{getApiErrorMessage(schedule.error, t("p360.checkAndRetry"))}</InlineFeedback> : null}<ActionBar><Button variant="secondary" onClick={() => setOpen(false)}>{t("actions.cancel")}</Button><Button onClick={() => schedule.mutate()} disabled={schedule.isPending || !interviewerUserId || !title.trim()}>{schedule.isPending ? "Programando..." : t("p360.confirmInterview")}</Button></ActionBar></div></DialogContent></Dialog>
  </CardContent></Card>;
}
