...
export function RestaurantPhase2View({ section }: { section: string }) {
  ...
  if (!currentBranch) return <InlineFeedback tone="warning" title={uiText("Sucursal requerida")}>{uiText("Selecciona una sucursal para operar este módulo.")}</InlineFeedback>;
  if (["stock-counts", "transfers", "adjustments"].includes(section) && !can("restaurant_inventory.manage")) return <InlineFeedback tone="warning" title={uiText("Permiso de gestión requerido")}>{uiText("Tu perfil puede consultar, pero no modificar esta sección.")}</InlineFeedback>;
  ...
  if (section === "transfers") return <SecureTransfersScreen branchId={currentBranch.id} />;
...
function QueryState({ loading, error, retry, children }: { loading: boolean; error: unknown; retry: () => void; children: ReactNode }) {
  const uiText = useUiText(); if (loading) return <SkeletonRows rows={5} label={uiText("Cargando información")} />; if (error) return <ErrorState title={uiText("No fue posible cargar la información")} detail={getApiErrorMessage(error, "No fue posible consultar esta sección.")} onRetry={() => { void (retry)(); }} />; return <>{children}</>; }
      ...
      <Table headers={["Origen", "Destino", "Detalle", "Estado", "Siguiente acción"]}>{(q.data ?? []).map((item) => {
        ...
        return <tr key={String(row.id)}>
          ...
          <td className="flex flex-wrap gap-2">
            ...
            {["SENT", "IN_TRANSIT"].includes(status) ? <Button size="sm" onClick={() => { void confirmAction({ title: uiText("¿Confirmar la recepción de la transferencia?"), description: uiText("Da por recibida la mercancía en el almacén de destino."), consequence: uiText("Se descuenta del almacén de origen y se suma al de destino."), confirmLabel: uiText("Confirmar la recepción"), irreversible: true }).then((ok) => ok && action.mutate({ id: String(row.id), type: "receive" })); }}><Truck className="size-4" />{uiText("Confirmar recepción")}</Button> : null}
            {["DRAFT", "SENT"].includes(status) ? <Button size="sm" variant="secondary" onClick={() => { void confirmAction({ title: uiText("¿Cancelar la transferencia?"), description: uiText("La transferencia queda anulada y deja de poder enviarse ni recibirse."), consequence: uiText("Si ya se había enviado, la mercancía no vuelve sola al origen: hay que registrar el movimiento de vuelta."), confirmLabel: uiText("Cancelar la transferencia"), cancelLabel: "Conservarla", irreversible: true }).then((ok) => ok && action.mutate({ id: String(row.id), type: "cancel" })); }}>{uiText("Cancelar transferencia")}</Button> : null}
          </td>
        </tr>;
      })}</Table>
    </QueryState>
