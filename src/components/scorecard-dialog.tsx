...
export function ScorecardDialog({ interviewId, onClose }: { interviewId: string; onClose: () => void }) {
  ...
  return <Dialog open={Boolean(interviewId)} onOpenChange={(open) => !open && onClose()}>
    <DialogContent className="max-h-[92vh] max-w-4xl overflow-y-auto">
      <DialogHeader><DialogTitle>Scorecard estructurado</DialogTitle><DialogDescription>Evalúa evidencia observable. La puntuación se calcula por peso y la firma bloquea cambios posteriores.</DialogDescription></DialogHeader>
      {context.isLoading ? <p className="py-8 text-center text-sm text-text-secondary">Preparando criterios y evaluación…</p> : null}
      {context.isError ? <InlineFeedback tone="danger" title="No fue posible cargar el scorecard">{context.error instanceof Error ? context.error.message : "Intenta nuevamente."}</InlineFeedback> : null}
...
function ScorecardForm({ interviewId, context, onClose }: { interviewId: string; context: ScorecardContextDto; onClose: () => void }) {
  ...
  return <div className="space-y-6">
    ...
    <InlineFeedback tone="warning" title="Firma definitiva"><span className="inline-flex items-center gap-2"><LockKeyhole className="size-4" />Al firmar, el scorecard y sus respuestas quedan bloqueados para preservar la auditoría.</span></InlineFeedback>
...
function ScoreSummary({ context }: { context: ScorecardContextDto }) {
  return <div className="grid gap-3 rounded-xl bg-surface-section p-4 sm:grid-cols-3"><Summary label="Puntuación" value={`${Number(context.scorecard?.weightedScore ?? (context.scorecard?.overallRating ?? 0) * 20).toFixed(1)}/100`} /><Summary label="Recomendación" value={context.scorecard?.recommendation ?? "Sin definir"} /><Summary label="Evaluador" value={context.scorecard?.reviewer ? `${context.scorecard.reviewer.firstName} ${context.scorecard.reviewer.lastName}` : "Usuario actual"} /></div>;
}
