...
export function TaskNavigation({ routes, pathname, role, onSearch }: { routes: AllowedRoute[]; pathname: string; role: string; onSearch: () => void }) {
  ...
  return <><section aria-label="Accesos frecuentes" className="grid gap-3 xl:grid-cols-[1.2fr_1fr_1fr]"><TaskCard icon={<Sparkles className="size-4" />} title="Acciones rápidas">{quick.length ? quick.map((route) => <Button key={route.href} asChild size="sm" variant="secondary" onClick={() => trackProductEvent({ name: "quick_action_opened", action: route.href, role })}><Link href={route.href}>{route.label}</Link></Button>) : <p className="text-sm text-text-secondary">No hay acciones asignadas para este rol.</p>}</TaskCard><TaskCard icon={<Clock3 className="size-4" />} title="Recientes">{recent.map((href) => routeByHref.get(href)).filter((route): route is AllowedRoute => Boolean(route)).map((route) => <Button key={route.href} asChild size="sm" variant="ghost"><Link href={route.href}>{route.label}</Link></Button>)}{!recent.length ? <p className="text-sm text-text-secondary">Tus visitas aparecerán aquí.</p> : null}</TaskCard><TaskCard icon={<Star className="size-4" />} title="Favoritos">{favorites.map((href) => routeByHref.get(href)).filter((route): route is AllowedRoute => Boolean(route)).map((route) => <Button key={route.href} asChild size="sm" variant="ghost"><Link href={route.href}>{route.label}</Link></Button>)}<div className="flex gap-2"><Button size="sm" variant="secondary" onClick={onSearch}><Search className="size-4" />Buscar</Button><Button size="sm" variant="secondary" onClick={() => setHelpOpen(true)}><HelpCircle className="size-4" />Glosario</Button></div></TaskCard></section><div className="flex flex-wrap gap-2" aria-label="Gestionar favoritos">{routes.slice(0, 10).map((route) => <button key={route.href} type="button" aria-pressed={favorites.includes(route.href)} onClick={() => toggleFavorite(route.href)} className="inline-flex min-h-9 items-center gap-1 rounded-full border border-border-default px-3 text-xs font-medium text-text-secondary hover:bg-surface-interactive"><Star className={`size-3.5 ${favorites.includes(route.href) ? "fill-primary text-primary" : ""}`} />{route.label}</button>)}</div><Dialog open={helpOpen} onOpenChange={setHelpOpen}><DialogContent className="max-w-2xl"><DialogHeader><DialogTitle>Guía y glosario</DialogTitle><DialogDescription>Definiciones breves para trabajar con los módulos sin salir de la aplicación.</DialogDescription></DialogHeader><dl className="grid gap-3 sm:grid-cols-2">{glossary.map(([term, definition]) => <div key={term} className="rounded-xl border border-border-default p-3"><dt className="font-semibold">{term}</dt><dd className="mt-1 text-sm text-text-secondary">{definition}</dd></div>)}</dl></DialogContent></Dialog></>;
}
