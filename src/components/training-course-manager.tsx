    ...
    <div className="space-y-6">
      ...
      <Card level={2}>
        <CardContent className="grid gap-3 p-4 lg:grid-cols-[minmax(240px,1fr)_repeat(3,minmax(150px,220px))]">
          <div className="space-y-2">
            <Label htmlFor="course-search">{uiText("Buscar")}</Label>
            <Input
              id="course-search"
              type="search"
              placeholder={uiText("Título, resumen o identificador")}
              value={search}
      ...
      <div className="flex flex-col gap-3 rounded-2xl border border-border-default bg-surface-section px-4 py-3 text-sm sm:flex-row sm:items-center sm:justify-between" aria-live="polite">
        <div className="flex flex-wrap items-center gap-2">
          <span className="font-medium">{coursesQuery.data?.total ?? 0} {uiText(" cursos")}</span>
          {search ? <Badge variant="secondary">{uiText("Búsqueda: ")}{search}</Badge> : null}
          {status ? <Badge variant="secondary">{uiText("Estado: ")}{statusLabels[status as TrainingCourseStatus]}</Badge> : null}
          {scope ? <Badge variant="secondary">{uiText("Alcance: ")}{scope === "TENANT" ? uiText("Empresa") : "Global"}</Badge> : null}
          {categoryId ? <Badge variant="secondary">{uiText("Categoría: ")}{(categoriesQuery.data ?? []).find((category) => category.id === categoryId)?.name ?? "Seleccionada"}</Badge> : null}
        </div>
        {search || status || scope || categoryId ? (
          <Button type="button" variant="ghost" size="sm" className="self-start sm:self-auto" onClick={() => router.replace(pathname, { scroll: false })}>
            ...
            {uiText("Limpiar filtros")}</Button>
        ) : <span className="text-text-secondary">{uiText("Mostrando el catálogo completo")}</span>}
      </div>

      {coursesQuery.isError ? (
        <ErrorState
          title={uiText("No fue posible cargar los cursos")}
          detail={getApiErrorMessage(coursesQuery.error, uiText("Reintenta la consulta para continuar."))}
      ...
      {!coursesQuery.isError ? (
        <div className="space-y-4">
          <DataView
            ...
            caption={uiText("Catálogo de cursos")}
            ...
            emptyAction={
              !hasFilters && canCreate ? (
                <Button asChild>
                  <Link href="/training/content/new">
                    <Plus className="size-4" aria-hidden="true" />
                    {uiText("Crear el primer curso")}</Link>
                </Button>
              ) : undefined
            }
...
function CategoryFormDialog({
  ...
  return (
    <FormDialog open={open} onOpenChange={onOpenChange} title={uiText("Nueva categoría")} description={uiText("Agrupa cursos y crea subcategorías para facilitar su descubrimiento.")}>
      <form className="space-y-4" onSubmit={(event) => { event.preventDefault(); if (name.trim()) create.mutate(); }}>
        ...
        <FormField id="category-name" label={uiText("Nombre")} required>
          {(field) => <Input {...field} value={name} onChange={(event) => setName(event.target.value)} />}
        </FormField>
        <FormField id="category-description" label={uiText("Descripción")}>
          {(field) => <textarea {...field} className="field min-h-24" value={description} onChange={(event) => setDescription(event.target.value)} />}
        </FormField>
        <FormField id="category-parent" label={uiText("Categoría superior")}>
          {(field) => <Select value={parentCategoryId} onValueChange={setParentCategoryId}><SelectTrigger {...field}><SelectValue /></SelectTrigger><SelectContent><SelectItem value="NONE">{uiText("Categoría principal")}</SelectItem>{categories.map((category) => <SelectItem key={category.id} value={category.id}>{category.name}</SelectItem>)}</SelectContent></Select>}
        </FormField>
        {globalAllowed ? (
          <FormField id="category-scope" label={uiText("Alcance")}>
            {(field) => <Select value={scope} onValueChange={(value) => setScope(value as "TENANT" | "GLOBAL")}><SelectTrigger {...field}><SelectValue /></SelectTrigger><SelectContent><SelectItem value="TENANT">{uiText("Empresa activa")}</SelectItem><SelectItem value="GLOBAL">{uiText("Toda la plataforma")}</SelectItem></SelectContent></Select>}
          </FormField>
        ) : null}
        <div className="flex justify-end gap-2"><Button type="button" variant="secondary" onClick={() => onOpenChange(false)}>{uiText("Cancelar")}</Button><Button type="submit" disabled={!name.trim() || create.isPending}>{create.isPending ? uiText("Creando…") : "Crear categoría"}</Button></div>
      </form>
    </FormDialog>
  );
}
    ...
    <div className="flex flex-wrap gap-2">
      ...
      {props.canEdit ? (
        <Button type="button" size="sm" variant="secondary" onClick={() => props.onEdit(props.course.id)}>
          <Pencil className="size-4" />{uiText("Editar")}</Button>
      ) : null}
      {props.canDuplicate ? (
...
function VisualCourseHint({ thresholdPercent = 90 }: { thresholdPercent?: number }) {
  const uiText = useUiText();
  return (
    <InlineFeedback tone="info" title={uiText("Curso visual")}>
      {uiText("Pensado para apoyarse en video. Cuando una lección es audiovisual, se completa al alcanzar el")}{thresholdPercent}{uiText("% de reproducción.")}</InlineFeedback>
  );
}
...
export function TrainingCourseCreatePage() {
  ...
  const errors = !title.trim()
    ? [{ fieldId: "course-title", label: uiText("Título"), message: "Es obligatorio." }]
    : [];

  return <div className="mx-auto max-w-4xl space-y-6">
    <PageHeader
      eyebrow={uiText("Aprendizaje")}
      title={uiText("Crear curso")}
    ...
    <Card level={2}>
      ...
      <CardContent>
        <form className="space-y-5" onSubmit={(event) => { event.preventDefault(); if (!errors.length) create.mutate(); }}>
          ...
          <FormField id="course-title" label={uiText("Título del curso")} description={uiText("Usa un nombre claro para que las personas lo encuentren fácilmente.")} required>
            {(field) => <Input {...field} autoFocus value={title} onChange={(event) => setTitle(event.target.value)} placeholder={uiText("Ej. Seguridad básica en el trabajo")} />}
          </FormField>
          <FormField id="course-summary" label={uiText("Resumen")} description={uiText("Opcional. Una frase breve sobre lo que aprenderán las personas.")}>
            {(field) => <textarea {...field} className="field min-h-28" value={summary} onChange={(event) => setSummary(event.target.value)} placeholder={uiText("Ej. Principios y prácticas para trabajar de forma segura.")} />}
          </FormField>
          <FormField id="course-category" label={uiText("Categoría")} description={uiText("Opcional. Puedes crear o cambiar categorías más adelante.")}>
            {(field) => <Select value={categoryId} onValueChange={setCategoryId} disabled={categories.isLoading}><SelectTrigger {...field}><SelectValue placeholder={uiText("Sin categoría")} /></SelectTrigger><SelectContent><SelectItem value="NONE">{uiText("Sin categoría")}</SelectItem>{(categories.data ?? []).map((category) => <SelectItem key={category.id} value={category.id}>{category.name}</SelectItem>)}</SelectContent></Select>}
          </FormField>
          <div className="space-y-4 rounded-2xl border border-border-default bg-surface-section p-4">
            <FormField id="course-template" label={uiText("Plantilla inicial")} description={uiText("La plantilla visual crea módulos y lecciones base listas para recibir videos locales.")}>
              {(field) => <Select value={courseTemplate} onValueChange={(value) => { const next = value as "BLANK" | "VISUAL"; setCourseTemplate(next); setIsVisualCourse(next === "VISUAL"); }}><SelectTrigger {...field}><SelectValue /></SelectTrigger><SelectContent><SelectItem value="VISUAL">{uiText("Curso visual")}</SelectItem><SelectItem value="BLANK">{uiText("Curso en blanco")}</SelectItem></SelectContent></Select>}
            </FormField>
            <label className="flex items-start gap-3">
              ...
              />
              <span>
                <span className="block font-medium">{uiText("Crear como curso visual")}</span>
                <span className="block text-sm text-text-secondary">
                  {uiText("Prioriza videos de apoyo y deja visible la regla de avance al 90% para completar.")}</span>
              </span>
            </label>
            {isVisualCourse ? (
              <FormField
                id="course-intro-video"
                label={uiText("Video introductorio")}
                description={uiText("Opcional, pero recomendado para cursos visuales. Sirve como apoyo de bienvenida.")}
              >
                {(field) => (
                  <Input
                    {...field}
          ...
          <p className="text-sm text-ink-2">{uiText("Dificultad, duración, idioma, etiquetas, portada y visibilidad se configuran después, en el editor.")}</p>
          <div className="flex flex-col-reverse gap-3 border-t border-border-default pt-5 sm:flex-row sm:justify-end"><Button type="button" variant="secondary" onClick={() => router.push("/training/content")}>{uiText("Cancelar")}</Button><Button type="submit" disabled={create.isPending || Boolean(errors.length)}>{create.isPending ? uiText("Creando…") : "Crear y diseñar curso"}</Button></div>
        </form>
      </CardContent>
    </Card>
  </div>;
...
function CourseMetadataForm({
  ...
  const errors = [
    !form.title.trim() ? { fieldId: "course-title", label: uiText("Título"), message: "Es obligatorio." } : null,
  ...
  return (
    <form className="space-y-4" onSubmit={submit}>
      ...
      <FormField id="course-title" label={uiText("Título")} required>
        {(field) => <Input {...field} value={form.title} onChange={(event) => setForm({ ...form, title: event.target.value })} />}
      </FormField>
      <FormField id="course-category" label={uiText("Categoría")}>
        {(field) => <Select value={form.categoryId ?? "NONE"} onValueChange={(value) => setForm({ ...form, categoryId: value === "NONE" ? undefined : value })}>
          ...
          <SelectContent><SelectItem value="NONE">{uiText("Sin categoría")}</SelectItem>{categories.map((category) => <SelectItem key={category.id} value={category.id}>{category.name}</SelectItem>)}</SelectContent>
        </Select>}
      </FormField>
      <FormField id="course-summary" label={uiText("Resumen")}>
        {(field) => <textarea {...field} className="field min-h-24" value={form.summary} onChange={(event) => setForm({ ...form, summary: event.target.value })} />}
      </FormField>
      <details className="rounded-2xl border border-border-default bg-surface-section" open={advancedOpen} onToggle={(event) => setAdvancedOpen(event.currentTarget.open)}>
        <summary className="cursor-pointer list-none p-4 font-semibold">{uiText("Configuración avanzada ")}<span className="ml-2 text-sm font-normal text-text-secondary">{uiText("Opcional")}</span></summary>
        <div className="space-y-4 border-t border-border-default p-4">
          <p className="text-sm text-text-secondary">{uiText("Estos datos mejoran la organización y la presentación, pero no son necesarios para guardar el contenido básico.")}</p>
          <FormField id="course-description" label={uiText("Descripción")}>
            {(field) => <textarea {...field} className="field min-h-32" value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} />}
          </FormField>
          <div className="grid gap-4 sm:grid-cols-2">
            <FormField id="course-difficulty" label={uiText("Dificultad")}>
              {(field) => <Select value={form.difficulty} onValueChange={(value) => setForm({ ...form, difficulty: value as TrainingCourseInput["difficulty"] })}><SelectTrigger {...field}><SelectValue /></SelectTrigger><SelectContent><SelectItem value="BEGINNER">{uiText("Inicial")}</SelectItem><SelectItem value="INTERMEDIATE">{uiText("Intermedia")}</SelectItem><SelectItem value="ADVANCED">{uiText("Avanzada")}</SelectItem></SelectContent></Select>}
            </FormField>
            <FormField id="course-language" label={uiText("Idioma")}>
              {(field) => <Input {...field} value={form.language ?? ""} onChange={(event) => setForm({ ...form, language: event.target.value })} placeholder="es" />}
            </FormField>
            <FormField id="course-duration" label={uiText("Duración estimada (minutos)")} description={uiText("Puedes ajustarla cuando definas las lecciones.")}>
            ...
            <FormField id="course-cover" label={uiText("Portada")} description={uiText("URL opcional de la imagen del curso.")}>
          ...
          <FormField id="course-intro-video" label={uiText("Video introductorio")} description={uiText("Opcional. Sirve como apoyo visual y entrada rápida al contenido.")}>
          ...
          <FormField id="course-tags" label={uiText("Etiquetas")} description={uiText("Separa las etiquetas con comas para facilitar la búsqueda.")}>
            {(field) => <Input {...field} value={(form.tags ?? []).join(", ")} onChange={(event) => setForm({ ...form, tags: event.target.value.split(",").map((tag) => tag.trim()).filter(Boolean) })} placeholder={uiText("seguridad, operaciones")} />}
          </FormField>
          <VisualCourseHint />
        </div>
      </details>
      <div className="flex justify-end gap-2">
        {showCancel ? <Button type="button" variant="secondary" onClick={onCancel}>{uiText("Cancelar")}</Button> : null}
        <Button type="submit" disabled={save.isPending || Boolean(errors.length)}>{save.isPending ? uiText("Guardando…") : submitLabel ?? "Guardar"}</Button>
      </div>
    </form>
  );
}
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description={uiText("Asistente editorial con guardado por etapa y requisitos de publicación.")}
        actions={<Button type="button" variant="secondary" onClick={() => router.push("/training/content")}><ArrowLeft className="size-4" />{uiText("Volver a cursos")}</Button>}
      />
      {/*
      ...
      */}
      <section className="overflow-hidden rounded-lg border border-line bg-surface-1 shadow-e1">
        ...
        <div className="min-w-0 p-5 sm:p-6">
          {query.isLoading ? <SkeletonRows rows={4} label={uiText("Cargando el curso")} /> : null}
          {query.isError || design.isError ? <ErrorState title={uiText("No fue posible cargar el curso")} detail={getApiErrorMessage(query.error ?? design.error, uiText("Reintenta la consulta para continuar."))} onRetry={() => { void query.refetch(); void design.refetch(); }} /> : null}
          {query.data && design.data && wizard ? (
            <div className="space-y-6">
              <div>
                <h2 className="flex flex-wrap items-center gap-x-3 gap-y-1 text-2xl font-semibold text-ink-1">{currentStep.label}{!currentStep.required ? <StatusBadge tone="neutral" label={uiText("Opcional, recomendado")} /> : null}</h2>
...
function AssessmentWizardStep({ course, onContinue }: { course: TrainingCourseDto; onContinue: () => void }) {
  ...
  return (
    <Card level={2}>
      <CardHeader><CardTitle>{uiText("Comprobación del aprendizaje")}</CardTitle></CardHeader>
          ...
          <div className="space-y-2">
            {course.quizzes.map((quiz) => (
              <div key={quiz.id} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-border-default p-4">
                <div><strong>{quiz.title}</strong><p className="text-sm text-text-secondary">{quiz.questions.length} {uiText(" preguntas · Aprobación ")}{quiz.passingScore}% · {quiz.maxAttempts ?? "∞"} {uiText(" intentos")}</p></div>
                <Badge variant={getTrainingAssessmentReadiness(quiz).ready ? "success" : "warning"}>
                  {getTrainingAssessmentReadiness(quiz).ready ? uiText("Lista") : "Requiere ajustes"}
                </Badge>
              </div>
            ))}
          </div>
...
function CertificationWizardStep({ course, onChanged, onContinue }: { course: TrainingCourseDto; onChanged: () => Promise<void>; onContinue: () => void }) {
  ...
  if (query.isLoading) return <SkeletonRows rows={4} label={uiText("Cargando el curso")} />;
  if (query.isError || !query.data) return <ErrorState title={uiText("No fue posible cargar el curso")} detail={getApiErrorMessage(query.error, uiText("Reintenta la consulta para continuar."))} onRetry={() => void query.refetch()} />;
  const policy = query.data.policy;
  return (
    <Card level={2}>
      <CardHeader><CardTitle>{uiText("Certificación, vigencia y renovación")}</CardTitle></CardHeader>
      <CardContent>
        <form key={policy.version} className="space-y-5" onSubmit={submit}>
          <FormErrorSummary subject="el curso" serverError={save.error} />
          {!query.data.readiness.ready && policy.isEnabled ? <InlineFeedback tone="warning" title={uiText("Configuración incompleta")}>{query.data.readiness.errors.join(" · ")}</InlineFeedback> : null}
          <div className="grid gap-3 sm:grid-cols-2">
            <label className="flex items-center gap-3 rounded-xl border border-border-default p-4"><input type="checkbox" name="isEnabled" defaultChecked={policy.isEnabled} /><span><strong>{uiText("Emitir certificado")}</strong><span className="block text-sm text-text-secondary">{uiText("Habilita una credencial verificable para este curso.")}</span></span></label>
            <label className="flex items-center gap-3 rounded-xl border border-border-default p-4"><input type="checkbox" name="autoIssue" defaultChecked={policy.autoIssue} /><span><strong>{uiText("Emisión automática")}</strong><span className="block text-sm text-text-secondary">{uiText("Se emite al cumplir toda la evidencia.")}</span></span></label>
            <label className="flex items-center gap-3 rounded-xl border border-border-default p-4"><input type="checkbox" name="requireAssessment" defaultChecked={policy.requireAssessment} /><span><strong>{uiText("Exigir evaluación")}</strong><span className="block text-sm text-text-secondary">{uiText("Todas las evaluaciones deben estar aprobadas.")}</span></span></label>
            <label className="flex items-center gap-3 rounded-xl border border-border-default p-4"><input type="checkbox" name="requireAllRequiredLessons" defaultChecked={policy.requireAllRequiredLessons} /><span><strong>{uiText("Exigir lecciones obligatorias")}</strong><span className="block text-sm text-text-secondary">{uiText("Valida progreso antes de emitir.")}</span></span></label>
          </div>
          <div className="grid gap-4 sm:grid-cols-3">
            <FormField id="cert-validity" label={uiText("Vigencia (días)")}>{(field) => <Input {...field} name="validityDays" type="number" min={1} max={3650} defaultValue={policy.validityDays ?? ""} placeholder={uiText("Sin vencimiento")} />}</FormField>
            <FormField id="cert-renewal-window" label={uiText("Ventana de renovación")}>{(field) => <Input {...field} name="renewalWindowDays" type="number" min={0} max={365} defaultValue={policy.renewalWindowDays} />}</FormField>
            <FormField id="cert-reminders" label={uiText("Avisos antes de vencer")}>{(field) => <Input {...field} name="reminderDays" defaultValue={policy.reminderDays.join(",")} placeholder="30,7,1" />}</FormField>
          </div>
          <div className="grid gap-4 sm:grid-cols-2">
            <FormField id="cert-title" label={uiText("Título de la credencial")}>{(field) => <Input {...field} name="certificateTitle" defaultValue={policy.certificateTitle ?? course.title} />}</FormField>
            <FormField id="cert-badge" label={uiText("URL de insignia")}>{(field) => <Input {...field} name="badgeImageUrl" type="url" defaultValue={policy.badgeImageUrl ?? ""} />}</FormField>
            <FormField id="cert-signatory" label={uiText("Nombre del firmante")}>{(field) => <Input {...field} name="signatoryName" defaultValue={policy.signatoryName ?? ""} />}</FormField>
            <FormField id="cert-signatory-title" label={uiText("Cargo del firmante")}>{(field) => <Input {...field} name="signatoryTitle" defaultValue={policy.signatoryTitle ?? ""} />}</FormField>
          </div>
          <FormField id="cert-description" label={uiText("Descripción de la credencial")}>{(field) => <textarea {...field} name="certificateDescription" className="field min-h-24" defaultValue={policy.certificateDescription ?? ""} />}</FormField>
          <div className="flex flex-wrap justify-end gap-2"><Button asChild variant="secondary"><Link href={`/training/certificates?courseId=${encodeURIComponent(course.id)}`}>{uiText("Ver certificados")}</Link></Button><Button type="submit" disabled={save.isPending}>{save.isPending ? uiText("Guardando…") : "Guardar y continuar"}</Button></div>
        </form>
      </CardContent>
    </Card>
  );
...
function PreviewWizardStep({ previewed, onPreview, onContinue }: { previewed: boolean; onPreview: () => void; onContinue: () => void }) {
  ...
  return (
    <Card level={2}>
      <CardHeader><CardTitle>{uiText("Experiencia del participante")}</CardTitle></CardHeader>
      <CardContent className="space-y-4">
        <p className="text-sm text-text-secondary">{uiText("Abre una simulación de solo lectura para revisar portada, secuencia, títulos y recursos. No guarda progreso ni modifica el curso.")}</p>
        {previewed ? <InlineFeedback tone="success" title={uiText("Vista previa revisada")}>{uiText("Esta sesión ya abrió la experiencia del participante.")}</InlineFeedback> : null}
        <div className="flex justify-end gap-2">
          <Button type="button" variant="secondary" onClick={onPreview}><Eye className="size-4" />{uiText("Abrir vista previa")}</Button>
          <Button type="button" onClick={onContinue}>{uiText("Continuar a revisión")}</Button>
        </div>
      </CardContent>
    </Card>
  );
...
function PublishWizardStep({
  ...
  return (
    <div className="space-y-5">
      <div>
        <p className="text-xs font-semibold uppercase tracking-[0.18em] text-brand">{uiText("Paso final")}</p>
        <h2 className="mt-1 text-2xl font-semibold">{uiText("Revisión antes de publicar")}</h2>
        <p className="mt-1 text-sm text-text-secondary">{uiText("Confirma el alcance y completa los requisitos antes de cambiar el estado del curso.")}</p>
      </div>
      <PublicationChecklist course={course} wizard={wizard} onGoTo={onGoTo} />
      <EditorSummary course={course} />
      {missing.length ? (
        <InlineFeedback tone="warning" title={uiText("El curso todavía no puede enviarse a revisión")}>
          <div className="mt-2 flex flex-wrap gap-2">{missing.map((item) => <Button key={item.id} type="button" size="sm" variant="secondary" onClick={() => onGoTo(item.id)}>{uiText("Completar ")}{item.label}</Button>)}</div>
        </InlineFeedback>
      ) : (
        <InlineFeedback tone="success" title={uiText("Requisitos editoriales completos")}>
          {uiText("Información, fundamento pedagógico y estructura están preparados para revisión.")}</InlineFeedback>
      )}
      {!wizard.completed.ASSESSMENT ? <InlineFeedback tone="info" title={uiText("Recomendación")}>{uiText("Añade una evaluación antes de publicar para medir el aprendizaje.")}</InlineFeedback> : null}
...
function PublicationChecklist({
  ...
  return (
    <Card className="border-primary/20 bg-primary/5">
      <CardHeader className="pb-3"><CardTitle className="text-base">{uiText("Checklist de publicación")}</CardTitle><p className="text-sm text-text-secondary">{uiText("La publicación conserva la versión y el historial del curso.")}</p></CardHeader>
      <CardContent className="space-y-2">
        {checks.map((check) => (
          <div key={check.label} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-border-default bg-card p-3">
            ...
            {!check.complete ? <Button type="button" variant="ghost" size="sm" onClick={() => onGoTo(check.step)}>{uiText("Completar")}</Button> : <Badge variant="success">{uiText("Completo")}</Badge>}
          </div>
        ))}
        <p className="pt-2 text-xs text-text-secondary">{uiText("Estado actual: ")}<strong className="font-medium text-foreground">{statusLabels[course.status]}</strong> {uiText(" · La validación de calidad se muestra debajo antes de publicar.")}</p>
      </CardContent>
    </Card>
  );
}
  ...
  return (
    <Card level={2}>
      <CardHeader className="flex-row items-start justify-between gap-4">
        <div><CardTitle>{uiText("Control de calidad · versión ")}{quality.courseVersion}</CardTitle><p className="mt-1 text-sm text-text-secondary">{uiText("Las aprobaciones anteriores no se reutilizan cuando cambia la versión.")}</p></div>
          ...
          {REQUIRED_TRAINING_REVIEW_TYPES.map((type) => {
            ...
            return (
              <div key={type} className="rounded-xl border border-border-default p-4">
                <div className="flex items-center justify-between gap-3"><strong>{qualityReviewLabels[type]}</strong><Badge variant={review?.status === "APPROVED" ? "success" : review?.status === "CHANGES_REQUESTED" ? "destructive" : "secondary"}>{review?.status === "APPROVED" ? uiText("Aprobada") : review?.status === "CHANGES_REQUESTED" ? "Cambios" : review ? uiText("Pendiente") : "No solicitada"}</Badge></div>
                {review?.summary ? <p className="mt-2 text-sm text-text-secondary">{review.summary}</p> : null}
                {review && canApprove && review.status === "PENDING" ? <Button className="mt-3 w-full" size="sm" variant="secondary" onClick={() => setDecisionReview(review)}>{uiText("Revisar gate")}</Button> : null}
              </div>
            );
          })}
        </div>
        ...
        <div className="border-t border-border-default pt-5">
          ...
          {quality.pilots.length ? <div className="space-y-3">{quality.pilots.map((pilot) => (
            <div key={pilot.id} className="rounded-xl border border-border-default p-4">
              <div className="flex flex-wrap items-center justify-between gap-3"><div><strong>{pilot.name}</strong><p className="text-sm text-text-secondary">{pilot.metrics?.responses ?? 0}/{pilot.metrics?.participants ?? pilot.participantIds.length} {uiText(" respuestas · promedio ")}{pilot.metrics?.averageRating ?? 0}/5 · {pilot.metrics?.blockingIssues ?? 0} {uiText(" bloqueos")}</p></div><Badge>{pilot.status}</Badge></div>
              <div className="mt-3 flex flex-wrap gap-2">
                {pilot.status === "DRAFT" ? <Button size="sm" onClick={() => pilotStatus.mutate({ id: pilot.id, status: "ACTIVE" })}>{uiText("Activar piloto")}</Button> : null}
                {pilot.status === "ACTIVE" ? <><Button size="sm" onClick={() => pilotStatus.mutate({ id: pilot.id, status: "COMPLETED" })}>{uiText("Cerrar con éxito")}</Button><Button size="sm" variant="secondary" onClick={() => pilotStatus.mutate({ id: pilot.id, status: "CANCELLED" })}>{uiText("Cancelar")}</Button></> : null}
              </div>
            </div>
          ))}</div> : <p className="text-sm text-text-secondary">{uiText("No se creó un piloto para esta versión.")}</p>}
        </div>
        {!quality.readiness.ready ? <InlineFeedback tone="warning" title={uiText("Pendiente para aprobar")}>{quality.readiness.errors.join(" · ")}</InlineFeedback> : <InlineFeedback tone="success" title={uiText("Go-live autorizado")}>{uiText("Todos los gates de calidad están aprobados.")}</InlineFeedback>}
      </CardContent>
...
function QualityDecisionDialog({ review, onClose, onSaved }: { review: TrainingQualityReviewDto | null; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  return (
    <FormDialog open={Boolean(review)} onOpenChange={(open) => !open && onClose()} title={`Revisión de ${review ? qualityReviewLabels[review.reviewType] : ""}`} description={uiText("Registra evidencia y una decisión explícita para esta versión.")}>
      <form className="space-y-4" onSubmit={submit}>
        ...
        <div><Label>{uiText("Decisión")}</Label><Select name="decision" defaultValue="APPROVED"><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="APPROVED">{uiText("Aprobar")}</SelectItem><SelectItem value="CHANGES_REQUESTED">{uiText("Solicitar cambios")}</SelectItem></SelectContent></Select></div>
        <div><Label htmlFor="quality-summary">{uiText("Observaciones")}</Label><textarea id="quality-summary" name="summary" className="field min-h-28" required /></div>
        <Button className="w-full" disabled={mutation.isPending}>{uiText("Registrar decisión")}</Button>
      </form>
    </FormDialog>
  );
}
...
function PilotDialog({ courseId, open, onOpenChange, onSaved }: { courseId: string; open: boolean; onOpenChange: (open: boolean) => void; onSaved: () => Promise<void> }) {
  ...
  return (
    <FormDialog open={open} onOpenChange={onOpenChange} title={uiText("Crear piloto")} description={uiText("Selecciona participantes y define la evidencia mínima para autorizar el go-live.")}>
      <form className="space-y-4" onSubmit={submit}>
        <div><Label htmlFor="pilot-name">{uiText("Nombre")}</Label><Input id="pilot-name" name="name" required placeholder={uiText("Piloto operaciones · cohorte 1")} /></div>
        <div className="grid gap-3 sm:grid-cols-2"><div><Label>{uiText("Respuestas mínimas")}</Label><Input name="minResponses" type="number" min={1} defaultValue={3} /></div><div><Label>{uiText("Promedio mínimo")}</Label><Input name="minAverageRating" type="number" min={1} max={5} step="0.1" defaultValue={4} /></div></div>
        <div><Label>{uiText("Participantes")}</Label><div className="mt-2 max-h-56 space-y-2 overflow-y-auto">{users.data?.map((user) => <label key={user.id} className="flex items-center gap-3 rounded-xl border border-border-default p-3"><input type="checkbox" checked={participantIds.includes(user.id)} onChange={(event) => setParticipantIds(event.target.checked ? [...participantIds, user.id] : participantIds.filter((id) => id !== user.id))} /><span>{user.fullName}<span className="block text-xs text-text-secondary">{user.email}</span></span></label>)}</div></div>
        <Button className="w-full" disabled={!participantIds.length || mutation.isPending}>{uiText("Crear piloto")}</Button>
      </form>
    </FormDialog>
  );
}
...
function WizardFooter({ step, onPrevious, onNext }: { step: TrainingCourseWizardStep; onPrevious: () => void; onNext: () => void }) {
  ...
  return (
    <div className="flex justify-between border-t border-line pt-4">
      <Button type="button" variant="ghost" disabled={first} onClick={onPrevious}><ChevronLeft className="size-4" />{uiText("Anterior")}</Button>
      <Button type="button" variant="ghost" disabled={last} onClick={onNext}>{uiText("Siguiente")}<ChevronRight className="size-4" /></Button>
    </div>
  );
}
  ...
  return (
    <Card level={1}>
      <CardContent className="grid gap-4 p-5 sm:grid-cols-2 lg:grid-cols-5">
        <Summary label={uiText("Estado")}><CourseStatusBadge status={course.status} /></Summary>
        <Summary label={uiText("Alcance")}>{course.tenantId ? uiText("Empresa") : "Global"}</Summary>
        <Summary label={uiText("Estructura")}>{course.modules.length} {uiText(" módulos · ")}{lessons} {uiText(" lecciones")}</Summary>
        <Summary label={uiText("Contenido")}>{blocks} {uiText(" bloques")}</Summary>
      </CardContent>
      <details className="border-t border-border-default px-5 py-3 text-sm">
        <summary className="cursor-pointer font-medium text-text-secondary">{uiText("Detalles técnicos")}</summary>
        <div className="mt-3 grid gap-3 sm:grid-cols-2">
          <Summary label={uiText("Versión editorial")}>{course.version}</Summary>
          <Summary label={uiText("Identificador del curso")}><span className="break-all font-mono text-xs">{course.id}</span></Summary>
        </div>
      </details>
    </Card>
  );
...
function WorkflowActions({
  ...
  const actions: Array<{ status: TrainingCourseStatus[]; permission: Parameters<typeof can>[0]; action: Parameters<typeof onTransition>[0]; label: string; icon: ReactNode }> = [
    ...
    { status: ["PUBLISHED", "PAUSED", "ARCHIVED"], permission: "courses.archive", action: "retire", label: uiText("Retirar"), icon: <Archive className="size-4" /> },
  ];
  return (
    <ActionBar label={uiText("Flujo editorial")}>
      {can("courses.update") && ["DRAFT", "IN_REVIEW", "APPROVED", "PAUSED"].includes(course.status) ? (
        <Button type="button" variant="secondary" onClick={onEdit}><Pencil className="size-4" />{uiText("Información")}</Button>
      ) : null}
...
function CourseStructure({ course, editable, onChanged }: { course: TrainingCourseDto; editable: boolean; onChanged: () => Promise<void> }) {
  ...
  return (
    <section className="space-y-4" aria-labelledby="course-structure-title">
      <div>
        <h2 id="course-structure-title" className="text-xl font-semibold">{uiText("Estructura del curso")}</h2>
        <p className="text-sm text-text-secondary">{uiText("Construye módulos, lecciones, recursos y práctica en el orden exacto que seguirá el participante.")}</p>
      </div>
      ...
      )) : <InlineFeedback tone="info" title={uiText("Sin módulos")}>{uiText("Añade un módulo para comenzar a estructurar el curso.")}</InlineFeedback>}
      {editable ? (
        <form className="flex flex-col gap-2 rounded-2xl border border-dashed border-border-strong p-4 sm:flex-row" onSubmit={(event) => { event.preventDefault(); if (moduleTitle.trim()) createModule.mutate(); }}>
          <Input aria-label={uiText("Título del nuevo módulo")} placeholder={uiText("Título del nuevo módulo")} value={moduleTitle} onChange={(event) => setModuleTitle(event.target.value)} />
          <Button type="submit" disabled={!moduleTitle.trim() || createModule.isPending}><Plus className="size-4" />{uiText("Añadir módulo")}</Button>
        </form>
      ) : null}
    </section>
  );
    ...
    <Card level={2}>
      <CardHeader className="flex-row items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          {editing ? <div className="space-y-2"><Input aria-label={uiText("Título del módulo")} value={title} onChange={(event) => setTitle(event.target.value)} /><textarea aria-label={uiText("Descripción del módulo")} className="field min-h-20" placeholder={uiText("Qué logrará el participante en este módulo")} value={description} onChange={(event) => setDescription(event.target.value)} /><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={isRequired} onChange={(event) => setIsRequired(event.target.checked)} />{uiText("Módulo obligatorio")}</label></div> : <><CardTitle>{module.title}</CardTitle>{module.description ? <p className="mt-1 text-sm text-text-secondary">{module.description}</p> : null}</>}
          <p className="mt-1 text-xs text-text-secondary">{module.lessons.length} {uiText(" lecciones · ")}{module.isRequired ? uiText("Obligatorio") : uiText("Opcional")}</p>
        </div>
        {editable ? <div className="flex flex-wrap justify-end gap-1"><OrderButtons label={module.title} index={index} total={total} pending={moving} onMove={(direction) => onMove(module.id, direction)} />{editing ? <Button type="button" size="sm" onClick={() => update.mutate()} disabled={!title.trim() || update.isPending}>{uiText("Guardar")}</Button> : <Button type="button" size="icon" variant="ghost" aria-label={`Editar ${module.title}`} onClick={() => setEditing(true)}><Pencil className="size-4" /></Button>}<Button type="button" size="icon" variant="ghost" aria-label={`Duplicar ${module.title}`} onClick={() => duplicate.mutate()} disabled={duplicate.isPending}><Copy className="size-4" /></Button><Button type="button" size="icon" variant="ghost" aria-label={`Eliminar ${module.title}`} onClick={() => remove.mutate()} disabled={remove.isPending}><Trash2 className="size-4 text-status-danger" /></Button></div> : null}
      </CardHeader>
      {expanded ? <CardContent className="space-y-3">{module.lessons.map((lesson, lessonIndex) => <LessonEditor key={lesson.id} courseId={module.courseId} lesson={lesson} index={lessonIndex} total={module.lessons.length} editable={editable} moving={reorder.isPending} onMove={moveLesson} onChanged={onChanged} />)}{editable ? <form className="flex flex-col gap-2 sm:flex-row" onSubmit={(event) => { event.preventDefault(); if (lessonTitle.trim()) createLesson.mutate(); }}><Input aria-label={uiText("Título de la nueva lección")} placeholder={uiText("Nueva lección")} value={lessonTitle} onChange={(event) => setLessonTitle(event.target.value)} /><Button type="submit" variant="secondary" disabled={!lessonTitle.trim() || createLesson.isPending}><Plus className="size-4" />{uiText("Lección")}</Button></form> : null}</CardContent> : null}
    </Card>
  );
}
...
function LessonEditor({ courseId, lesson, index, total, editable, moving, onMove, onChanged }: { courseId: string; lesson: TrainingCourseDto["modules"][number]["lessons"][number]; index: number; total: number; editable: boolean; moving: boolean; onMove: (lessonId: string, direction: -1 | 1) => void; onChanged: () => Promise<void> }) {
  ...
  const upload = useMutation({
    ...
    onError: (error) => toast.error(getApiErrorMessage(error, uiText("No fue posible cargar el video."))),
  });
...
function BlockEditor({ block, index, total, editable, moving, onMove, onChanged }: { block: TrainingContentBlockDto; index: number; total: number; editable: boolean; moving: boolean; onMove: (blockId: string, direction: -1 | 1) => void; onChanged: () => Promise<void> }) {
  ...
  return (
    <div className="flex items-center gap-3 rounded-xl border border-border-default bg-surface-elevated p-3">
      ...
      <div className="min-w-0 flex-1"><p className="font-medium">{block.title || blockLabels[block.type]}</p><p className="text-xs text-text-secondary">{blockLabels[block.type]} · {block.isRequired ? uiText("Obligatorio") : uiText("Opcional")}{summary ? ` · ${summary}` : ""}</p></div>
      {editable ? <div className="flex flex-wrap gap-1"><OrderButtons label={block.title || blockLabels[block.type]} index={index} total={total} pending={moving} onMove={(direction) => onMove(block.id, direction)} /><Button type="button" size="icon" variant="ghost" aria-label={uiText("Editar bloque")} onClick={() => setEditing(true)}><Pencil className="size-4" /></Button><Button type="button" size="icon" variant="ghost" aria-label={uiText("Duplicar bloque")} onClick={() => duplicate.mutate()} disabled={duplicate.isPending}><Copy className="size-4" /></Button><Button type="button" size="icon" variant="ghost" aria-label={uiText("Eliminar bloque")} onClick={() => remove.mutate()}><Trash2 className="size-4 text-status-danger" /></Button></div> : null}
...
function BlockFormDialog({ lessonId, block, open, onOpenChange, onSaved }: { lessonId: string; block?: TrainingContentBlockDto; open: boolean; onOpenChange: (open: boolean) => void; onSaved: () => Promise<void> }) {
  ...
  return (
    <FormDialog open={open} onOpenChange={onOpenChange} title={block ? "Editar contenido" : "Añadir contenido"} description={uiText("Configura el recurso, la actividad o la evaluación con criterios claros para el participante.")}>
      <form className="space-y-4" onSubmit={(event) => { event.preventDefault(); if (value.trim()) save.mutate(); }}>
        <FormErrorSummary subject="el curso" serverError={save.error} />
        <FormField id="block-type" label={uiText("Tipo")}>{(field) => <Select value={type} onValueChange={(next) => { setType(next as TrainingContentBlockType); setValue(""); }}><SelectTrigger {...field}><SelectValue /></SelectTrigger><SelectContent>{Object.entries(blockLabels).map(([key, label]) => <SelectItem key={key} value={key}>{label}</SelectItem>)}</SelectContent></Select>}</FormField>
        <FormField id="block-title" label={uiText("Título")}>{(field) => <Input {...field} value={title} onChange={(event) => setTitle(event.target.value)} />}</FormField>
        <FormField id="block-value" label={valueLabel} required>{(field) => resource || type === "QUIZ" ? <Input {...field} type={resource ? "url" : "text"} value={value} onChange={(event) => setValue(event.target.value)} /> : <textarea {...field} className="field min-h-32" value={value} onChange={(event) => setValue(event.target.value)} />}</FormField>
        {type === "VIDEO" ? <div className="grid gap-4 sm:grid-cols-2"><FormField id="block-transcript" label={uiText("URL de transcripción")}>{(field) => <Input {...field} type="url" value={transcriptUrl} onChange={(event) => setTranscriptUrl(event.target.value)} />}</FormField><FormField id="block-captions" label={uiText("URL de subtítulos")}>{(field) => <Input {...field} type="url" value={captionsUrl} onChange={(event) => setCaptionsUrl(event.target.value)} />}</FormField></div> : null}
        {type === "FILE" || type === "LINK" ? <FormField id="block-accessibility" label={uiText("Nota de accesibilidad")}>{(field) => <Input {...field} placeholder={uiText("Ej. PDF etiquetado y apto para lector de pantalla")} value={accessibilityNote} onChange={(event) => setAccessibilityNote(event.target.value)} />}</FormField> : null}
        {type === "TASK" ? <div className="grid gap-4 sm:grid-cols-2"><FormField id="block-evidence" label={uiText("Evidencia esperada")} required>{(field) => <Select value={evidenceType} onValueChange={setEvidenceType}><SelectTrigger {...field}><SelectValue /></SelectTrigger><SelectContent><SelectItem value="Archivo entregable">{uiText("Archivo entregable")}</SelectItem><SelectItem value="Respuesta escrita">{uiText("Respuesta escrita")}</SelectItem><SelectItem value="Enlace externo">{uiText("Enlace externo")}</SelectItem><SelectItem value="Validación del supervisor">{uiText("Validación del supervisor")}</SelectItem></SelectContent></Select>}</FormField><FormField id="block-rubric" label={uiText("Criterios de aceptación")}>{(field) => <Input {...field} placeholder={uiText("Qué debe demostrar para aprobar")} value={rubric} onChange={(event) => setRubric(event.target.value)} />}</FormField></div> : null}
        <label className="flex items-center gap-2 rounded-xl bg-surface-section p-3 text-sm"><input type="checkbox" checked={isRequired} onChange={(event) => setIsRequired(event.target.checked)} />{uiText("Este contenido es obligatorio para completar la lección")}</label>
        <div className="flex justify-end gap-2"><Button type="button" variant="secondary" onClick={() => onOpenChange(false)}>{uiText("Cancelar")}</Button><Button type="submit" disabled={!value.trim() || save.isPending}>{uiText("Guardar")}</Button></div>
      </form>
    </FormDialog>
  );
}
...
function ScheduleDialog({ open, onOpenChange, pending, onConfirm }: { open: boolean; onOpenChange: (open: boolean) => void; pending: boolean; onConfirm: (input: { scheduledPublishAt: string; scheduledRetireAt?: string }) => void }) {
  ...
  return (
    <FormDialog open={open} onOpenChange={onOpenChange} title={uiText("Programar publicación")} description={uiText("El backend publicará el curso automáticamente en la fecha indicada.")}>
      <form className="space-y-4" onSubmit={(event) => { event.preventDefault(); if (publishAt) onConfirm({ scheduledPublishAt: new Date(publishAt).toISOString(), scheduledRetireAt: retireAt ? new Date(retireAt).toISOString() : undefined }); }}>
        <FormField id="publish-at" label={uiText("Publicar el")} required>{(field) => <Input {...field} type="datetime-local" value={publishAt} onChange={(event) => setPublishAt(event.target.value)} />}</FormField>
        <FormField id="retire-at" label={uiText("Retirar el (opcional)")}>{(field) => <Input {...field} type="datetime-local" value={retireAt} onChange={(event) => setRetireAt(event.target.value)} />}</FormField>
        <div className="flex justify-end gap-2"><Button type="button" variant="secondary" onClick={() => onOpenChange(false)}>{uiText("Cancelar")}</Button><Button type="submit" disabled={!publishAt || pending}>{pending ? "Programando…" : "Programar"}</Button></div>
      </form>
    </FormDialog>
  );
}
...
function AuthenticatedTrainingVideo({ courseId, lessonId, title }: { courseId: string; lessonId: string; title: string }) {
  ...
  if (error) return <p className="mt-4 rounded-xl bg-status-danger/10 p-3 text-sm text-status-danger">{uiText("No fue posible cargar el video.")}</p>;
  if (!url) return <div className="mt-4 aspect-video w-full animate-pulse rounded-xl bg-muted" aria-label={uiText("Cargando video")} />;
