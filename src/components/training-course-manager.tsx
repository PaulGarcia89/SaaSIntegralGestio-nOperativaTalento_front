      ...
      <CardContent>
        <form className="space-y-5" onSubmit={(event) => { event.preventDefault(); if (!errors.length) create.mutate(); }}>
          ...
          <FormField id="course-summary" label="Resumen" description="Opcional. Una frase breve sobre lo que aprenderán las personas.">
            {(field) => <textarea {...field} className="field min-h-28" value={summary} onChange={(event) => setSummary(event.target.value)} placeholder="Ej. Principios y prácticas para trabajar de forma segura." />}
          </FormField>
    ...
    <Card level={2}>
      ...
      <CardContent>
        <form key={policy.version} className="space-y-5" onSubmit={submit}>
          ...
          <FormField id="cert-description" label="Descripción de la credencial">{(field) => <textarea {...field} name="certificateDescription" className="field min-h-24" defaultValue={policy.certificateDescription ?? ""} />}</FormField>
  ...
  return (
    <FormDialog open={Boolean(review)} onOpenChange={(open) => !open && onClose()} title={`Revisión de ${review ? qualityReviewLabels[review.reviewType] : ""}`} description="Registra evidencia y una decisión explícita para esta versión.">
      <form className="space-y-4" onSubmit={submit}>
        ...
        <div><Label htmlFor="quality-summary">Observaciones</Label><textarea id="quality-summary" name="summary" className="field min-h-28" required /></div>
    ...
    <Card level={2}>
      <CardHeader className="flex-row items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          {editing ? <div className="space-y-2"><Input aria-label="Título del módulo" value={title} onChange={(event) => setTitle(event.target.value)} /><textarea aria-label="Descripción del módulo" className="field min-h-20" placeholder="Qué logrará el participante en este módulo" value={description} onChange={(event) => setDescription(event.target.value)} /><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={isRequired} onChange={(event) => setIsRequired(event.target.checked)} />Módulo obligatorio</label></div> : <><CardTitle>{module.title}</CardTitle>{module.description ? <p className="mt-1 text-sm text-text-secondary">{module.description}</p> : null}</>}
...
function LessonEditor({ courseId, lesson, index, total, editable, moving, onMove, onChanged }: { courseId: string; lesson: TrainingCourseDto["modules"][number]["lessons"][number]; index: number; total: number; editable: boolean; moving: boolean; onMove: (lessonId: string, direction: -1 | 1) => void; onChanged: () => Promise<void> }) {
  ...
  return (
      ...
      <div className="flex items-start gap-2">
        ...
        <div className="min-w-0 flex-1">{editing ? <div className="grid gap-2 sm:grid-cols-[1fr_9rem]"><Input aria-label="Título de la lección" value={title} onChange={(event) => setTitle(event.target.value)} /><Input aria-label="Duración estimada en minutos" type="number" min={0} value={estimatedMinutes} onChange={(event) => setEstimatedMinutes(Number(event.target.value))} /><textarea aria-label="Descripción de la lección" className="field min-h-20 sm:col-span-2" placeholder="Objetivo y contexto de la lección" value={description} onChange={(event) => setDescription(event.target.value)} /><label className="flex items-center gap-2 text-sm sm:col-span-2"><input type="checkbox" checked={isRequired} onChange={(event) => setIsRequired(event.target.checked)} />Lección obligatoria</label><label className="flex items-center gap-2 text-sm sm:col-span-2">Porcentaje para completar video<input className="field w-24" type="number" min={1} max={100} value={completionPercentage} onChange={(event) => setCompletionPercentage(Math.min(100, Math.max(1, Number(event.target.value) || 1)))} />%</label></div> : <><p className="font-medium">{lesson.title}</p>{lesson.description ? <p className="text-sm text-text-secondary">{lesson.description}</p> : null}<p className="text-xs text-text-secondary">{lesson.estimatedMinutes || 0} min · {lesson.isRequired ? "Obligatoria" : "Opcional"} · {lesson.blocks.length} bloques · Video: {lesson.requiredCompletionPercentage ?? 90}%</p></>}</div>
