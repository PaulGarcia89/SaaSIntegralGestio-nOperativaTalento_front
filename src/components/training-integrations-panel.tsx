...
export function TrainingIntegrationsPanel() {
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-4 lg:grid-cols-2"><Card><CardHeader><CardTitle>Próximas sesiones</CardTitle></CardHeader><CardContent className="space-y-3">{data.sessions.length?data.sessions.map((item)=><div key={item.id} className="rounded-xl border p-4"><strong>{item.title}</strong><p className="text-sm text-muted-foreground">{new Date(item.startsAt).toLocaleString()} · {item.timeZone}</p>{item.meetingUrl?<a className="text-sm text-brand underline" href={item.meetingUrl} target="_blank" rel="noreferrer">Abrir reunión</a>:null}</div>):<p className="text-sm text-muted-foreground">No hay sesiones virtuales próximas.</p>}</CardContent></Card><Card><CardHeader><CardTitle>Recomendaciones explicables</CardTitle></CardHeader><CardContent className="space-y-3">{data.recommendations.length?data.recommendations.map((item)=><div key={item.id} className="rounded-xl border p-4"><p className="text-sm">{item.reason}</p><p className="my-2 text-xs text-muted-foreground">Es una sugerencia; requiere confirmación humana.</p><div className="flex gap-2"><Button size="sm" onClick={() => decision.mutate({id:item.id,status:"ACCEPTED"})}>Aceptar</Button><Button size="sm" variant="secondary" onClick={() => decision.mutate({id:item.id,status:"DISMISSED"})}>Descartar</Button></div></div>):<p className="text-sm text-muted-foreground">No hay recomendaciones pendientes.</p>}</CardContent></Card></div>
