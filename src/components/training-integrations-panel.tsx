...
export function TrainingIntegrationsPanel() {
  ...
  if (query.isLoading) return <SkeletonRows rows={6} label="Cargando las integraciones formativas" />;
  if (query.isError) return <ErrorState title="No fue posible cargar las integraciones" detail={getApiErrorMessage(query.error, "Reintenta la consulta para continuar.")} onRetry={() => void query.refetch()} />;
  const data = query.data!;
  return <div className="space-y-6">
    <PageHeader eyebrow="Aprendizaje" title="Integraciones formativas" description="Supervisa SCORM, actividad xAPI, sesiones virtuales y webhooks sin exponer secretos." actions={<Button onClick={() => setDialog("scorm")}><Plus className="size-4" aria-hidden="true" />Registrar paquete SCORM</Button>} /><IntegrationGuide />
    <MetricRow>{[{label:"Paquetes SCORM",value:data.packages.length},{label:"Eventos xAPI",value:data.xapiStatements},{label:"Webhooks",value:data.webhooks.length},{label:"Sesiones próximas",value:data.sessions.length},{label:"Recursos publicados",value:data.resources}].map((item)=><Metric key={item.label} label={item.label} value={String(item.value)} />)}</MetricRow>
    ...
    <div className="grid gap-4 lg:grid-cols-2"><Card><CardHeader><CardTitle>SCORM</CardTitle></CardHeader><CardContent className="space-y-3">{data.packages.length?data.packages.map((item)=><div key={item.id} className="rounded-xl border p-4"><div className="flex flex-wrap items-center justify-between gap-3"><strong>{item.title}</strong><StatusBadge size="sm" tone={packageStatusTone(item.status)} label={packageStatusLabel(item.status)} /></div><p className="text-sm text-muted-foreground">{item.course.title} · SCORM {item.version} · {item._count.sessions} sesiones</p><Button size="sm" variant="secondary" className="mt-2" onClick={()=>launchScorm(item.id)}>Abrir reproductor</Button></div>):<EmptyState reason="no-records" title="No hay paquetes SCORM" description="Registra un paquete para reproducir contenido formativo dentro de la plataforma." action={<Button variant="secondary" onClick={()=>setDialog("scorm")}>Registrar el primero</Button>} />}</CardContent></Card><Card><CardHeader className="flex-row items-center justify-between gap-2"><CardTitle>Webhooks</CardTitle><div className="flex flex-wrap gap-2"><Button size="sm" variant="secondary" onClick={()=>webhookAction.mutate(undefined)} disabled={webhookAction.isPending}>Probar entrega</Button><Button size="sm" variant="secondary" onClick={()=>setDialog("webhook")}><Plus className="size-4" aria-hidden="true" />Nuevo webhook</Button></div></CardHeader><CardContent className="space-y-3">{data.webhooks.length?data.webhooks.map((item)=><div key={item.id} className="rounded-xl border p-4"><div className="flex justify-between gap-3"><strong>{item.name}</strong><Badge variant={item.isActive&&item.deliveryReady?"success":"secondary"}>{item.deliveryReady?(item.isActive?"Activo":"Inactivo"):"Reconfigurar"}</Badge></div><p className="truncate text-sm text-muted-foreground">{item.endpointUrl}</p><p className="mt-1 text-xs text-muted-foreground">{item.eventTypes.join(", ")}</p>{!item.deliveryReady?<div className="mt-2"><InlineNote tone="warning" title="Este webhook no puede entregar">La configuración anterior no guardó un secreto recuperable. Crea un webhook nuevo con su secreto.</InlineNote></div>:null}{item.lastError?<p className="mt-2 text-xs text-destructive">{item.lastError}</p>:null}</div>):<EmptyState reason="no-records" title="No hay webhooks configurados" description="Un webhook avisa a otro sistema cuando ocurre algo en la formación." action={<Button variant="secondary" onClick={()=>setDialog("webhook")}>Configurar el primero</Button>} />}</CardContent></Card></div>
    <Card><CardHeader><CardTitle>Entregas recientes</CardTitle></CardHeader><CardContent className="space-y-3">{data.deliveries.length?data.deliveries.map((item)=><div key={item.id} className="flex flex-col gap-3 rounded-xl border p-4 sm:flex-row sm:items-center sm:justify-between"><div><div className="flex items-center gap-2"><strong>{technicalLabel(item.eventType)}</strong><StatusBadge size="sm" tone={deliveryStatusTone(item.status)} label={deliveryStatusLabel(item.status)} /></div><p className="text-xs text-muted-foreground">{item.webhook.name} · {item.attemptCount} {item.attemptCount === 1 ? "intento" : "intentos"} · {formatDateTime(item.createdAt)}</p>{item.lastError?<p className="mt-1 text-xs text-destructive">{item.lastError}</p>:null}</div>{item.status==="FAILED"?<Button size="sm" variant="secondary" onClick={()=>webhookAction.mutate(item.id)}>Reintentar</Button>:null}</div>):<EmptyState reason="no-records" title="Todavía no hay entregas" description="Aquí aparecerán los envíos a los sistemas conectados." />}</CardContent></Card>
    <div className="grid gap-4 lg:grid-cols-2"><Card><CardHeader><CardTitle>Próximas sesiones</CardTitle></CardHeader><CardContent className="space-y-3">{data.sessions.length?data.sessions.map((item)=><div key={item.id} className="rounded-xl border p-4"><strong>{item.title}</strong><p className="text-sm text-muted-foreground">{formatDateTime(item.startsAt)} · {item.timeZone}</p>{item.meetingUrl?<a className="text-sm text-brand underline" href={item.meetingUrl} target="_blank" rel="noreferrer">Abrir reunión</a>:null}</div>):<EmptyState reason="no-records" title="No hay sesiones próximas" description="Programa una sesión virtual para que aparezca en la agenda." action={<Button variant="secondary" onClick={()=>setDialog("session")}>Programar una sesión</Button>} />}</CardContent></Card><Card><CardHeader><CardTitle>Recomendaciones explicables</CardTitle></CardHeader><CardContent className="space-y-3">{data.recommendations.length?data.recommendations.map((item)=><div key={item.id} className="rounded-xl border p-4"><p className="text-sm">{item.reason}</p><p className="my-2 text-xs text-muted-foreground">Es una sugerencia; requiere confirmación humana.</p><div className="flex gap-2"><Button size="sm" onClick={() => decision.mutate({id:item.id,status:"ACCEPTED"})}>Aceptar</Button><Button size="sm" variant="secondary" onClick={() => decision.mutate({id:item.id,status:"DISMISSED"})}>Descartar</Button></div></div>):<EmptyState reason="no-records" title="No hay recomendaciones pendientes" description="Cuando el sistema detecte una oportunidad, aparecerá aquí para que una persona decida." />}</CardContent></Card></div>
...
function TrainingOperationsCenter() {
  ...
  if (query.isLoading) return <SkeletonRows rows={4} label="Comprobando la operación formativa" />;
  if (query.isError) return <ErrorState title="No fue posible consultar la operación" detail={getApiErrorMessage(query.error, "Reintenta la consulta para continuar.")} onRetry={() => void query.refetch()} />;
  const data = query.data!;
  return (
      ...
      <Card className={data.health.status === "CRITICAL" ? "border-destructive/40" : ""}>
        <CardHeader>
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div><CardTitle className="flex items-center gap-2"><Activity className="size-5 text-brand" />Centro de operaciones</CardTitle><p className="mt-1 text-sm text-muted-foreground">Atrasos reales, recuperación tenant-safe y evidencia de cada intervención.</p></div>
            <div className="text-right"><div className="flex items-center justify-end gap-2"><strong className="font-mono text-4xl tabular-figures">{data.health.score}</strong><StatusBadge tone={healthStatusTone(data.health.status)} label={healthStatusLabel(data.health.status)} /></div><p className="text-xs text-muted-foreground">salud operativa / 100</p></div>
          </div>
        </CardHeader>
          ...
          <CardContent className="space-y-2">
            {data.runs.length ? data.runs.slice(0, 12).map((run) => (
              <div key={run.id} className="rounded-xl border p-3">
                ...
                <p className="mt-1 text-xs text-muted-foreground">{run.actor ? `${run.actor.firstName} ${run.actor.lastName}` : "Sistema"} · {formatDateTime(run.startedAt)}{run.durationMs !== null && run.durationMs !== undefined ? ` · ${formatDuration(run.durationMs)}` : ""}</p>
                {run.error ? <p className="mt-1 text-xs text-destructive">{run.error}</p> : null}
              </div>
            )) : <EmptyState reason="no-records" title="Todavía no hay recuperaciones manuales" description="Cada acción de recuperación que ejecutes quedará registrada aquí." />}
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent className="space-y-2">
            ...
            )) : <EmptyState reason="no-records" title="No hay eventos auditados" description="La actividad sobre formación aparecerá aquí en cuanto ocurra." />}
          </CardContent>
        </Card>
      </div>
    </section>
...
function IntegrationDialog({ kind, courses, pending, onClose, onSubmit }: { kind: DialogKind; courses: Array<{id:string;title:string}>; pending:boolean; onClose:()=>void; onSubmit:(kind:Exclude<DialogKind,null>,input:Record<string,unknown>)=>void }) {
  ...
  return <Dialog open={Boolean(kind)} onOpenChange={(open)=>!open&&onClose()}><DialogContent><DialogHeader><DialogTitle>{title}</DialogTitle><DialogDescription>La operación queda aislada a la empresa activa y será auditada.</DialogDescription></DialogHeader>{kind?<form className="space-y-4" onSubmit={submit}>
    ...
    <Button className="w-full" loading={pending} loadingLabel="Guardando…">Guardar</Button>
