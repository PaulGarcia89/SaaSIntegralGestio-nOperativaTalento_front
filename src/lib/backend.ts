...
import { normalizarExistencias, normalizarLotes } from "./restaurant-stock-rows";
import { normalizarConteos, normalizarTransferencias } from "./restaurant-operation-rows";
...
const uiPermissionToBackendCodes: Partial<Record<PermissionKey, string[]>> = {
  ...
  "restaurant_inventory.receipts.create": ["restaurant_inventory.receipts.create"],
  "restaurant_inventory.receipts.confirm": ["restaurant_inventory.receipts.confirm"],
  "restaurant_inventory.recipes.manage": ["restaurant_inventory.recipes.manage"],
  "restaurant_inventory.operations.create": ["restaurant_inventory.operations.create"],
  "restaurant_inventory.operations.confirm": ["restaurant_inventory.operations.confirm"],
  "restaurant_inventory.counts.approve": ["restaurant_inventory.counts.approve"],
  "restaurant_inventory.adjustments.create": ["restaurant_inventory.adjustments.create"],
  "restaurant_inventory.transfers.manage": ["restaurant_inventory.transfers.manage"],
  "restaurant_inventory.settings.manage": ["restaurant_inventory.settings.manage"],
  "restaurant_inventory.expiry_alerts.view": ["restaurant_inventory.expiry_alerts.view"],
  "restaurant_inventory.counts.schedule": ["restaurant_inventory.counts.schedule"],
  "restaurant_inventory.variance.view": ["restaurant_inventory.variance.view"],
  "restaurant_inventory.shrinkage.view": ["restaurant_inventory.shrinkage.view"],
  "restaurant_inventory.audit.read": ["restaurant_inventory.audit.read"],
  "restaurant_inventory.commercial.view": ["restaurant_inventory.commercial.view"],
  "restaurant_inventory.commissary.manage": ["restaurant_inventory.commissary.manage"],
  "restaurant_inventory.budgets.manage": ["restaurant_inventory.budgets.manage"],
...
export function confirmRestaurantReceipt(id: string, idempotencyKey = requestId()) { return request<import("./contracts").RestaurantReceiptDto>(`/restaurant-inventory/receipts/${encodeURIComponent(id)}/confirm`, { method: "POST" }, { idempotencyKey }); }
...
export async function fetchRestaurantMovements(filters: { branchId?: string; warehouseId?: string; ingredientId?: string; from?: string; to?: string } = {}) {
  const query = new URLSearchParams();
  Object.entries(filters).forEach(([key, value]) => { if (value) query.set(key, value); });
  const payload = await request<Array<Record<string, unknown>> | { data?: Array<Record<string, unknown>>; items?: Array<Record<string, unknown>> }>(`/restaurant-inventory/movements${query.size ? `?${query}` : ""}`);
  const rows = normalizeRestaurantList(payload);
  return rows.map((row) => {
...
export async function fetchRestaurantLots(filters: { branchId?: string; warehouseId?: string; search?: string; page?: number; pageSize?: number } = {}) { const query = new URLSearchParams(); Object.entries(filters).forEach(([key, value]) => { if (value !== undefined && value !== "") query.set(key, String(value)); }); const payload = await request<RestaurantCatalogPage<import("./contracts").RestaurantLotDto> | import("./contracts").RestaurantLotDto[]>(`/restaurant-inventory/lots${query.size ? `?${query}` : ""}`); const filas = normalizeRestaurantListPage(payload); const normalizadas = normalizarLotes(filas as unknown as Array<Record<string, unknown>>) as unknown as typeof filas; return Object.assign(normalizadas, { page: filas.page, pageSize: filas.pageSize, total: filas.total, totalPages: filas.totalPages }); }
export async function fetchRestaurantStockCounts(filters: { branchId?: string; warehouseId?: string; status?: string; page?: number; pageSize?: number } = {}) { const query = new URLSearchParams(); Object.entries(filters).forEach(([key, value]) => { if (value !== undefined && value !== "") query.set(key, String(value)); }); const payload = await request<RestaurantCatalogPage<import("./contracts").RestaurantStockCountDto> | import("./contracts").RestaurantStockCountDto[]>(`/restaurant-inventory/stock-counts${query.size ? `?${query}` : ""}`); const filas = normalizeRestaurantListPage(payload); const normalizadas = normalizarConteos(filas as unknown as Array<Record<string, unknown>>) as unknown as typeof filas; return Object.assign(normalizadas, { page: filas.page, pageSize: filas.pageSize, total: filas.total, totalPages: filas.totalPages }); }
...
export async function fetchRestaurantTransfers(filters: { branchId?: string; warehouseId?: string; status?: string; page?: number; pageSize?: number } = {}) { const query = new URLSearchParams(); Object.entries(filters).forEach(([key, value]) => { if (value !== undefined && value !== "") query.set(key, String(value)); }); const payload = await request<RestaurantCatalogPage<import("./contracts").RestaurantTransferDto> | import("./contracts").RestaurantTransferDto[]>(`/restaurant-inventory/transfers${query.size ? `?${query}` : ""}`); const filas = normalizeRestaurantListPage(payload); const normalizadas = normalizarTransferencias(filas as unknown as Array<Record<string, unknown>>) as unknown as typeof filas; return Object.assign(normalizadas, { page: filas.page, pageSize: filas.pageSize, total: filas.total, totalPages: filas.totalPages }); }
...
export function sendRestaurantTransfer(id: string, idempotencyKey = requestId()) { return request<import("./contracts").RestaurantTransferDto>(`/restaurant-inventory/transfers/${encodeURIComponent(id)}/send`, { method: "POST" }, { idempotencyKey }); }
export function receiveRestaurantTransfer(id: string, idempotencyKey = requestId()) { return request<import("./contracts").RestaurantTransferDto>(`/restaurant-inventory/transfers/${encodeURIComponent(id)}/receive`, { method: "POST" }, { idempotencyKey }); }
export function cancelRestaurantTransfer(id: string, reason: string, idempotencyKey = requestId()) { return request<import("./contracts").RestaurantTransferDto>(`/restaurant-inventory/transfers/${encodeURIComponent(id)}/cancel`, { method: "POST", body: JSON.stringify({ reason }) }, { idempotencyKey }); }
export function createRestaurantAdjustment(input: Record<string, unknown>) { requireRestaurantContext(input, "solicitar un ajuste"); return request("/restaurant-inventory/adjustments", { method: "POST", body: JSON.stringify(input) }, { idempotencyKey: String(input.clientOperationId ?? requestId()) }); }
...
export type RestaurantPurchaseOrderDto = { id: string; code: string; status: string; supplierName?: string; warehouseName?: string; currency?: string; totalAmount?: number | string; orderedAt?: string; expectedAt?: string; lines: Array<{ id: string; ingredientId: string; ingredientName?: string; quantity: number; receivedQuantity?: number; unitCost: number | string; unit?: string }> };
export type RestaurantPriceHistoryDto = { ingredientId: string; ingredientName?: string; supplierId?: string; supplierName?: string; unit?: string; lastCost: number; averageCost: number; minimumCost?: number; maximumCost?: number; lastPurchasedAt?: string; variationPercent?: number };
export type RestaurantPurchaseSuggestionDto = { ingredientId: string; ingredientName?: string; supplierId?: string; supplierName?: string; unit?: string; currentStock: number; parLevel: number; coverageDays: number; targetCoverageDays: number; suggestedQuantity: number; estimatedCost: number; reason: string };
export type RestaurantInvoiceDto = { id: string; fileName: string; status: string; supplierName?: string; invoiceNumber?: string; confidence?: number; total?: number; createdAt?: string; errorMessage?: string };
export async function fetchRestaurantPurchaseOrders(filters: Record<string, string | number | undefined> = {}) { const query = new URLSearchParams(); Object.entries(filters).forEach(([key, value]) => { if (value !== undefined && value !== "") query.set(key, String(value)); }); const payload = await request<RestaurantPurchaseOrderDto[] | { data?: RestaurantPurchaseOrderDto[]; items?: RestaurantPurchaseOrderDto[] }>(`/restaurant-inventory/purchase-orders${query.size ? `?${query}` : ""}`); return normalizeRestaurantList(payload); }
export function createRestaurantPurchaseOrder(input: Record<string, unknown>) { requireRestaurantContext(input, "crear una orden de compra"); return request<RestaurantPurchaseOrderDto>("/restaurant-inventory/purchase-orders", { method: "POST", body: JSON.stringify(input) }, { idempotencyKey: String(input.clientOperationId ?? requestId()) }); }
export function submitRestaurantPurchaseOrder(id: string, idempotencyKey = requestId()) { return request<RestaurantPurchaseOrderDto>(`/restaurant-inventory/purchase-orders/${encodeURIComponent(id)}/submit`, { method: "POST" }, { idempotencyKey }); }
export function approveRestaurantPurchaseOrder(id: string, idempotencyKey = requestId()) { return request<RestaurantPurchaseOrderDto>(`/restaurant-inventory/purchase-orders/${encodeURIComponent(id)}/approve`, { method: "POST" }, { idempotencyKey }); }
export function rejectRestaurantPurchaseOrder(id: string, reason: string) { return request<RestaurantPurchaseOrderDto>(`/restaurant-inventory/purchase-orders/${encodeURIComponent(id)}/reject`, { method: "POST", body: JSON.stringify({ reason }) }); }
export function receiveRestaurantPurchaseOrder(id: string, lines: Array<{ lineId: string; quantity: number; unitCost?: number; lotNumber?: string; expirationDate?: string }>, idempotencyKey = requestId()) { return request<RestaurantPurchaseOrderDto>(`/restaurant-inventory/purchase-orders/${encodeURIComponent(id)}/receive`, { method: "POST", body: JSON.stringify({ lines }) }, { idempotencyKey }); }
export async function fetchRestaurantPriceHistory(filters: Record<string, string | number | undefined> = {}) { const query = new URLSearchParams(); Object.entries(filters).forEach(([key, value]) => { if (value !== undefined && value !== "") query.set(key, String(value)); }); const payload = await request<RestaurantPriceHistoryDto[] | { data?: RestaurantPriceHistoryDto[]; items?: RestaurantPriceHistoryDto[] }>(`/restaurant-inventory/price-history${query.size ? `?${query}` : ""}`); return normalizeRestaurantList(payload); }
...
export async function fetchRestaurantInvoices(filters: Record<string, string | number | undefined> = {}) { const query = new URLSearchParams(); Object.entries(filters).forEach(([key, value]) => { if (value !== undefined && value !== "") query.set(key, String(value)); }); const payload = await request<RestaurantInvoiceDto[] | { data?: RestaurantInvoiceDto[]; items?: RestaurantInvoiceDto[] }>(`/restaurant-inventory/invoices${query.size ? `?${query}` : ""}`);
  /*
   * La columna «Total» de la bandeja de facturas salía con un guion en todas
   * las filas: la factura guarda el importe como `totalAmount` y la pantalla
   * leía `total`. Un solo nombre de diferencia entre tener el dato y no
   * tenerlo.
   */
  return normalizeRestaurantList(payload).map((fila) => {
    const registro = fila as unknown as Record<string, unknown>;
    return { ...fila, total: Number(registro.total ?? registro.totalAmount ?? 0) } as RestaurantInvoiceDto;
  });
}
...
export type RestaurantUnitComparisonDto = { metric: string; unit: string; period: string; value: number; rank?: number; changePercent?: number };
export type RestaurantCommissaryDto = { id: string; code: string; name: string; status: string; branchCount: number; itemsCount?: number; totalValue?: number; updatedAt?: string };
export type RestaurantPurchaseBudgetDto = { id: string; period: string; branchId?: string; branchName?: string; budget: number; committed: number; received: number; remaining: number; utilizationPercent: number; status: string };
export type RestaurantPurchaseBudgetInput = { period: string; branchId?: string; budget: number; notes?: string };
function restaurantCommercialQuery(filters: Record<string, string | number | undefined> = {}) { const query = new URLSearchParams(); Object.entries(filters).forEach(([key, value]) => { if (value !== undefined && value !== "") query.set(key, String(value)); }); return query; }
export function fetchRestaurantDemandForecast(filters: Record<string, string | number | undefined> = {}) { const query = restaurantCommercialQuery(filters); return request<RestaurantDemandForecastDto[]>(`/restaurant-inventory/commercial/demand-forecast${query.size ? `?${query}` : ""}`); }
export function fetchRestaurantBranchCosts(filters: Record<string, string | number | undefined> = {}) { const query = restaurantCommercialQuery(filters); return request<RestaurantBranchCostDto[]>(`/restaurant-inventory/commercial/branch-costs${query.size ? `?${query}` : ""}`); }
export function fetchRestaurantRecipeMargins(filters: Record<string, string | number | undefined> = {}) { const query = restaurantCommercialQuery(filters); return request<RestaurantRecipeMarginDto[]>(`/restaurant-inventory/commercial/recipe-margins${query.size ? `?${query}` : ""}`); }
export function fetchRestaurantUnitComparison(filters: Record<string, string | number | undefined> = {}) { const query = restaurantCommercialQuery(filters); return request<RestaurantUnitComparisonDto[]>(`/restaurant-inventory/commercial/unit-comparison${query.size ? `?${query}` : ""}`); }
export function fetchRestaurantCommissaries(filters: Record<string, string | number | undefined> = {}) { const query = restaurantCommercialQuery(filters); return request<RestaurantCommissaryDto[]>(`/restaurant-inventory/commercial/commissaries${query.size ? `?${query}` : ""}`); }
export function createRestaurantCommissary(input: Record<string, unknown>) { return request<RestaurantCommissaryDto>("/restaurant-inventory/commercial/commissaries", { method: "POST", body: JSON.stringify(input) }, { idempotencyKey: String(input.clientOperationId ?? requestId()) }); }
export function fetchRestaurantPurchaseBudgets(filters: Record<string, string | number | undefined> = {}) { const query = restaurantCommercialQuery(filters); return request<RestaurantPurchaseBudgetDto[]>(`/restaurant-inventory/commercial/purchase-budgets${query.size ? `?${query}` : ""}`); }
export function createRestaurantPurchaseBudget(input: RestaurantPurchaseBudgetInput) { return request<RestaurantPurchaseBudgetDto>("/restaurant-inventory/commercial/purchase-budgets", { method: "POST", body: JSON.stringify(input) }, { idempotencyKey: requestId() }); }
...
export type CompanyCalendarProvider = "GOOGLE" | "MICROSOFT";
export type CompanyInterviewDefaults = { defaultModality: "GOOGLE_MEET" | "MICROSOFT_TEAMS" | "PRESENTIAL" | "CUSTOM" | "PHONE"; durationMinutes: number; reminderMinutes: number; timezone: string };
export type CompanyCalendarSettings = {
  defaults: CompanyInterviewDefaults;
  providers: { provider: CompanyCalendarProvider; configured: boolean; connected: boolean; email: string | null; lastSyncedAt: string | null; clientId?: string; hasClientSecret?: boolean; authority?: string; calendarId?: string }[];
};
export function fetchCompanyCalendarSettings(defaultsOnly = false) { return request<CompanyCalendarSettings>(`/company/calendar-settings${defaultsOnly ? "/defaults" : ""}`); }
export function saveCompanyCalendarDefaults(input: CompanyInterviewDefaults) { return request<CompanyInterviewDefaults>("/company/calendar-settings", { method: "PUT", body: JSON.stringify(input) }); }
export function saveCompanyCalendarCredentials(provider: CompanyCalendarProvider, input: { clientId?: string; clientSecret?: string; authority?: string; calendarId?: string }) { return request<CompanyCalendarSettings>(`/company/calendar-settings/${provider}`, { method: "PUT", body: JSON.stringify(input) }); }
export function authorizeCompanyCalendar(provider: CompanyCalendarProvider, redirectUri: string) { return request<{ authorizationUrl: string; state: string }>(`/company/calendar-settings/${provider}/authorize?redirectUri=${encodeURIComponent(redirectUri)}`); }
export function completeCompanyCalendarOAuth(provider: CompanyCalendarProvider, input: { code: string; state: string; redirectUri: string }) { return request<CalendarConnectionDto>(`/company/calendar-settings/${provider}/oauth`, { method: "POST", body: JSON.stringify(input) }); }
export function testCompanyCalendar(provider: CompanyCalendarProvider) { return request<{ success: boolean; email: string }>(`/company/calendar-settings/${provider}/test`, { method: "POST" }); }
export function listCompanyCalendars(provider: CompanyCalendarProvider) { return request<{ id: string; name: string }[]>(`/company/calendar-settings/${provider}/calendars`); }
export function disconnectCompanyCalendar(provider: CompanyCalendarProvider) { return request<{ disconnected: boolean }>(`/company/calendar-settings/${provider}`, { method: "DELETE" }); }
