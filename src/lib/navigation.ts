...
const configuredNavigation: Array<Omit<NavItem, "featureFlag" | "available" | "requiredPermissions" | "section">> = [
  ...
  // «Panel principal» llevaba exactamente a la misma pantalla que
  // «Restaurante»: dos entradas de menú al mismo destino, que es la
  // duplicación que este rediseño vino a eliminar. La ruta y su política de
  // acceso se conservan para no romper enlaces guardados ni las pruebas de
  // permisos; deja de ocupar un sitio en el menú.
  { href: "/inventory/restaurant/dashboard", label: "Panel principal", group: "Operaciones", module: "restaurant_inventory", permission: "restaurant_inventory.view", audience: "shared", subscriptionStates: live, branchRequired: true, showInNavigation: false, icon: "dashboard", roles: ["admin_saas", "admin_empresa", "supervisor", "encargado_inventario"] },
